...
| **Mobile Static Analysis** | APK/IPA manifest/config checks for lab use (requires `--features mobile`; static-only; no execution or device interaction; dynamic under `mobile-dynamic` feature, Phase 1 complete 2026-06-12 per `plans/mobile-dynamic-phase1-implementation-handoff-plan.md` (executed); Phase 1 polish (smoke test script `scripts/test-mobile-dynamic.sh`, `--list-devices` convenience, troubleshooting, docs) complete 2026-06-12 per `plans/mobile-dynamic-post-phase1-polish-and-phase2-planning.md` (executed); Phase 2a: proxy config + traffic summary + runtime permission testing complete 2026-06-12 per `plans/mobile-dynamic-phase2-implementation-handoff-plan.md` (executed) (final polish executed 2026-06-12 per plans/mobile-dynamic-phase2-final-polish-handoff-plan.md (executed); close-out polish executed 2026-06-12 per `plans/mobile-dynamic-phase2-close-out-polish-plan.md` (executed)); see `plans/dynamic-mobile-testing-loadout-design-plan.md` for design + future phases). |

## What Eggsec is not

Eggsec is not an exploitation framework, botnet component, credential attack platform, or tool for unscoped internet scanning. The `auth-test` command exists for defense validation of authentication controls (lockout policies, MFA enforcement, rate limiting, etc.) under strict scope/policy gating — it is not a credential attack platform (see architecture/auth.md for adopted model details: runtime policy gate, local findings only, standalone CLI distinct from pipeline `ScanProfile::Auth`). The `mobile` command performs static-only analysis of user-supplied .apk/.ipa files (manifest, permissions, transport config, secrets, debug/backup/exported components) for authorized lab/defense use only (feature-gated behind `mobile`; no execution, no device interaction, no network traffic to the app). Dynamic (Android ADB + runtime log analysis; standalone defense-lab, MCP-absent; bridge via `to_scan_report_data_dynamic`) is Phase 1 complete 2026-06-12 under `mobile-dynamic` feature (`mobile-dynamic = ["mobile"]`) per `plans/mobile-dynamic-phase1-implementation-handoff-plan.md` (executed); Phase 1 polish (smoke test script `scripts/test-mobile-dynamic.sh`, `--list-devices` convenience, troubleshooting section, updated success criteria) complete 2026-06-12 per `plans/mobile-dynamic-post-phase1-polish-and-phase2-planning.md` (executed); Phase 2a (proxy config + traffic summary + runtime permission testing; lab-only, user-managed mitmproxy/CA for --proxy/--traffic-capture, pm grant/revoke/list for permissions; still standalone defense-lab, MCP/agent absent, same pattern as wireless-active + static-mobile + auth-test) complete 2026-06-12 per `plans/mobile-dynamic-phase2-implementation-handoff-plan.md` (executed) (final polish executed 2026-06-12 per plans/mobile-dynamic-phase2-final-polish-handoff-plan.md (executed); close-out polish executed 2026-06-12 per plans/mobile-dynamic-phase2-close-out-polish-plan.md (executed)); design in `plans/dynamic-mobile-testing-loadout-design-plan.md`. A lightweight `correlate_findings` helper (and `static_correlation` on findings) links high-value static signals (cleartext, dangerous permissions) to dynamic observations. Same standalone pattern as wireless active. Some modules can generate aggressive traffic or security-test payloads, so advanced capabilities are feature-gated and intended for systems you own, operate, or have explicit authorization to test.
...
# Mobile dynamic (Phase 2a: proxy + permissions; lab-only; requires --features mobile-dynamic + --allow-dynamic-mobile for real)
...
| `eggsec mobile <path.{apk,ipa}>` (or `eggsec mobile static ...`) | defense-lab (static) | Standalone static analysis of Android APKs and iOS IPAs (manifest, permissions, transport config, secrets, debug/backup/exported components, signing/provisioning). Pure-Rust offline on user-supplied lab binaries only. Feature-gated `mobile` (default/legacy static path). Policy via SafeActive + required_features:["mobile"]; local MobileScanReport/MobileFinding + optional to_scan_report_data bridge. Native `--json` auto-bridges for `eggsec report convert`. See docs/MOBILE.md (Integration section) and architecture/mobile.md. `eggsec mobile dynamic ...` requires `--features mobile-dynamic` (Phase 1: Android ADB core + runtime log analysis; Phase 2a 2026-06-12: proxy Level-1 device config + --traffic-capture summary + runtime permission grant/revoke/list; standalone defense-lab, MCP-absent; `to_scan_report_data_dynamic` bridge with `mobile-dynamic-*` categories; auto-bridged in `report convert`; no TUI/pipeline/MCP). Design per `plans/dynamic-mobile-testing-loadout-design-plan.md`; Phase 1 per `plans/mobile-dynamic-phase1-implementation-handoff-plan.md` (executed 2026-06-12); Phase 1 polish (smoke test script, `--list-devices` convenience, troubleshooting, docs) complete 2026-06-12 per `plans/mobile-dynamic-post-phase1-polish-and-phase2-planning.md` (executed); Phase 2a per `plans/mobile-dynamic-phase2-implementation-handoff-plan.md` (executed); final polish per `plans/mobile-dynamic-phase2-final-polish-handoff-plan.md` (executed); close-out per `plans/mobile-dynamic-phase2-close-out-polish-plan.md` (executed). Same pattern as wireless active. |
...
| `mobile-dynamic` | Mobile dynamic testing (Phase 1: Android ADB core + runtime log analysis; Phase 2a 2026-06-12: proxy config + traffic summary + runtime permission testing; standalone defense-lab, MCP-absent; `mobile-dynamic = ["mobile"]`; auto-bridge via `to_scan_report_data_dynamic`; Phase 1 complete 2026-06-12 per `plans/mobile-dynamic-phase1-implementation-handoff-plan.md` (executed); Phase 1 polish (smoke test script `scripts/test-mobile-dynamic.sh`, `--list-devices` convenience, troubleshooting section, updated success criteria) complete 2026-06-12 per `plans/mobile-dynamic-post-phase1-polish-and-phase2-planning.md` (executed); Phase 2a per `plans/mobile-dynamic-phase2-implementation-handoff-plan.md` (executed); final polish per `plans/mobile-dynamic-phase2-final-polish-handoff-plan.md` (executed); close-out per `plans/mobile-dynamic-phase2-close-out-polish-plan.md` (executed)). Design in `plans/dynamic-mobile-testing-loadout-design-plan.md`. | Stable |
...
| `wireless` | `wireless-tools` | `sudo apt-get install wireless-tools` (provides `iwlist` scanner). Tests (parsing/analysis, no hardware): `cargo test -p eggsec --features wireless`. TUI tab + MCP/agent note: see architecture/wireless.md and plans/wireless-tui-mcp-agentic-handoff-plan.md. Active design: `plans/wireless-active-attacks-loadout-design-plan.md`. |
