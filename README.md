...
- **Loader policy is closed at Milestone 1.** All script/module filesystem loading flows through `ScriptResolver` with canonical root containment, symlink escape rejection, extension allowlist, and size limits. `ManualPermissive` script-file loading with empty roots is intentionally permissive (manual CLI/TUI discretion); restricted and automated profiles enforce roots strictly or deny script files entirely. Read-path authorization cannot authorize non-existent script/module files. Rust-side blocking helper cancellation remains a Milestone 3 follow-up. See `architecture/nse_integration.md` for the empty-roots semantic table.
