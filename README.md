...
| **Web Proxy / Traffic Interception** | MITM proxy for capturing and inspecting HTTP/HTTPS traffic in authorized lab environments (requires `web-proxy` feature; dry-run always safe; Phase 2 adds interactive TUI with flow inspection, header/body editing, forward/drop/replay, HAR export, and manipulation audit trail; real interception requires `--allow-web-proxy`; see `docs/WEB_PROXY.md`) |
...
| `eggsec proxy-intercept` | defense-lab | Interactive web proxy for HTTP/HTTPS traffic interception with TUI (dry-run; Phase 2 with flow inspection, editing, HAR export) |
...
| `web-proxy` | MITM proxy for capturing and inspecting HTTP/HTTPS traffic in authorized lab environments (Phase 2: interactive TUI with flow inspection, editing, HAR export, manipulation audit trail) | Stable |
...
| `mobile-dynamic` | Mobile dynamic testing (Phase 1: Android ADB core + runtime log analysis; Phase 2 closed 2026-06-12: proxy config + traffic summary + runtime permission testing + correlation; Phase 3a/3b/3c + Phase 4a CorrelationEngine + evidence foundation delivered 2026-06-12 under single mobile-dynamic; standalone defense-lab, MCP-absent; `mobile-dynamic = ["mobile"]`; auto-bridge via `to_scan_report_data_dynamic`; Phase 1 complete 2026-06-12 per `plans/mobile-dynamic-phase1-implementation-handoff-plan.md` (executed); Phase 1 polish complete 2026-06-12 per `plans/mobile-dynamic-post-phase1-polish-and-phase2-planning.md` (executed); Phase 2 closed per closeout+kickoff plan (executed); Phase 4a polish handoff executed per `plans/mobile-dynamic-phase4a-final-polish-handoff-plan.md`. Design in `plans/dynamic-mobile-testing-loadout-design-plan.md`. | Stable |
