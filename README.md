...
- **Milestone 2 is closed.** Run output truthfulness is defined by `NseRunReport`. Rule behavior is defined by `NseRuleEvaluationReport`. Rule evaluation produces structured reports via `evaluate_rule()`. Library reports are populated from registry metadata by `build_library_reports()`. Error paths emit full reports via `build_failure_report()`. The compatibility corpus is representative and local-only by default. See the [Milestone 2 Closure Note](./architecture/nse_integration.md#milestone-2-closure-note).
