...
| **Web Proxy / Traffic Interception** | MITM proxy for capturing and inspecting HTTP/HTTPS traffic in authorized lab environments (requires `web-proxy` feature; dry-run always safe; real interception requires `--allow-web-proxy`; see `docs/WEB_PROXY.md`) |
...
| `eggsec proxy-intercept` | defense-lab | Interactive web proxy for HTTP/HTTPS traffic interception |
...
| `web-proxy` | MITM proxy for capturing and inspecting HTTP/HTTPS traffic in authorized lab environments | Stable |
