...
| **Optional NSE Compatibility** | Curated Nmap NSE script support as an optional layer. **NSE Capability Wrappers (Milestone 3 complete)**: All side-effecting helper operations (filesystem, process, network, DNS, time, randomness, environment, compression, crypto) are now routed through centralized capability enforcement. Profile-specific policies control access for manual, agent, and CI surfaces. **Milestone 4 complete**: Structured evidence reports, upstream-style corpus, context fidelity fixtures, and CLI UX. **Milestone 5 complete**: Runtime harness strictness, flake isolation, local protocol fixtures, deferred library migration, report UX/performance. **Milestone 6 complete**: HTTP method coverage, AgentSafe/CiSafe strict assertions, all HTTP methods zero-hit denial tested. See `architecture/nse_integration.md` and `docs/NSE_COMPATIBILITY.md` for details. |
...
Eggsec provides Python bindings via [PyO3](https://pyo3.rs) and [maturin](https://github.com/PyO3/maturin). The Python package is a **host-language binding** over the Rust engine — not an internal plugin runtime. **Status: pre-1.0 release candidate work — 22 stable operations, not yet published to PyPI.** Windows is outside the primary support scope. The twenty-two stable-core engine operations use a canonical registry, mandatory policy gate, structured errors, and typed results; broader domains remain provisional or experimental. The API convergence release promotes git-secrets, SBOM, consolidated-recon, GraphQL, OAuth, authentication, database, NSE, container, and mobile-static domains to stable. See `crates/eggsec-python/README.md` and [`docs/python/domain-maturity.md`](docs/python/domain-maturity.md) for details.

The final integration pass brings the Python API toward a scoped pre-1.0 release: canonical stable-operation dispatch, mandatory policy/audit decisions, versioned structured errors, reliable-event/backpressure accounting, domain maturity introspection, and release evidence. The pipeline now supports step dependencies, parallel execution groups, retry policy, and configurable failure policy. Broader domain parity, cross-platform wheel validation, and TestPyPI publication remain release gates.
...
- **Milestone 4 is closed.** Structured evidence reports, upstream-style corpus, context fidelity fixtures, and CLI UX are complete. Compatibility matrix: `docs/NSE_COMPATIBILITY.md`.
- **Milestone 5 is closed.** Runtime harness strictness, flake isolation, local protocol fixtures, deferred library migration, report UX/performance, and release closure are complete. 511 tests pass, 52 architecture guards pass.
- **Milestone 6 is closed.** HTTP method coverage complete — all HTTP methods (GET/POST/PUT/DELETE/HEAD/OPTIONS/request) have local fixture scripts with zero-hit denial tests for AgentSafe and CiSafe profiles. HTTP library (`reqwest`) fully migrated to capability-gated operations. Architecture guards 48-50 enforce strict assertions.
