...
./eggsec db pentest oracle://system:pass@127.0.0.1:1521/ORCL --dry-run --json  # requires db-pentest-oracle
...
| `eggsec proxy-intercept` | defense-lab | Interactive web proxy for HTTP/HTTPS traffic interception with TUI (dry-run; Phase 2 with flow inspection, editing, HAR export; Phase 4 adds pipeline profile `ScanProfile::WebProxy`, MCP tools via `web-proxy-mcp`, evidence bundles) |
