...
Legacy implementation plans from the flat era are grandfathered at the `plans/` top level under their original filenames (e.g. `crate-boundary-consolidation-phase-b-report-model-extraction.md`). They already carry appended completion records and MUST NOT be rewritten; `plans/registry.md` maps each of them to its subsystem. All NEW milestone plans live here.
...
State expected behavior for partial failure, duplicate delivery, process restart, cancellation races, stale generations, and concurrent callers. Cancellation goes through `eggsec-runtime::race_with_cancel`; spawned tokio tasks need 30–300s timeout wrappers.
...
make check-features-individual  # deep check only, when feature surface changes
```
...
The corrective plan must reference the original plan and closure record, enumerate unclosed findings, and add regression evidence that would have caught them. It MUST NOT rewrite the original plan's completion record.
