...
**Executed note (Phase 3b)**: Per plan section 4 (2026-06-12). All deliverables under single mobile-dynamic (no sub-feature). No new Cargo deps. frida.rs: added structured_output to FridaScriptResult; redact_frida_evidence (pure); generate_crypto_keystore_script / generate_bypass_validation_script / generate_api_trace_script; run_builtin dispatch (builtin: crypto-keystore|bypass-validation|api-trace|basic-method-trace alias); execute_script now populates structured_output on JSON type lines (sim + real); sim paths populate for new types + redaction. basic_method_trace kept as thin wrapper. Expanded 8+ new unit tests (gen_*, run_builtin each+unknown, structured parse, redact, dry structured). DynamicMobileReport/FridaInstrumentation: +start_time/structured_results/correlation_notes (non-breaking; populated in new()/dry/real). run_dynamic_cli: dry now exercises 3 new builtins + sample frida-* findings + richer carrier (multiple enabled_builtins + structured + corr_notes); real generalized to call run_builtin for builtin: prefix, populates structured_results, applies redact to evidence. correlate_findings extended with 4 new Frida rules (crypto+static-secret, api+traffic/network, bypass+debug-perm, secret-extract+static-secret); populates static_correlation + returns notes. format_dynamic_report + to_scan_report_data_dynamic + build_dynamic_recommendations updated for richer fields + new bridge cats (mobile-dynamic-android-frida-crypto-observation etc.) + correlation mention. ~6 new tests (richer carrier, structured bridge, redaction on Frida evidence, new correlate rules, error robustness, build_recs). CLI: ABOUT cleaned (builtin: convention documented, example for Phase 3b builtin in dry; no new clap arg). Handler: no logic change (frida_script presence still triggers Intrusive for !dry). Smoke: Phase 3b leg added (exercises builtin:crypto-keystore + --traffic-capture + --list-permissions; asserts richer fi, multiple enabled_builtins, structured_results, correlation_notes, new cats, redaction hits, bridge). Docs: plan annotated, docs/MOBILE.md (Phase 3b delivered bullets + examples + updated limitations/success), architecture/mobile.md (Status/Future/Key Types/Files/Integration), root AGENTS.md (mobile-dynamic desc + Key Types + Security Notes + verification), README.md (mobile para), crates/eggsec/src/mobile/AGENTS.override.md (notes/testing), incidental. Verification inside session: cargo check/test/clippy --features mobile-dynamic green (81 mobile:: tests); ./scripts/test-mobile-dynamic.sh full PASS for Phase 3b assertions. No mobile-frida cfg. All per constraints (dry safe, best-effort real, standalone, preserve patterns, minimal, AGENTS style). 3c future. (2026-06-12)
