# Eggsec TUI Theme Load State Cleanup Plan
...
Do not edit generated `crates/eggsec-tui/src/theme/packaged.rs` by hand.
...
```bash
rg "theme_load_rx|theme_load_handle|deferred_theme_name|theme_changed_by_user" crates/eggsec-tui/src/app
rg "spawn_theme_loader|join_theme_loader_handle|handle_theme_install_report" crates/eggsec-tui/src/app
```
...
Add a small state type, preferably in `crates/eggsec-tui/src/app/state.rs` near the other grouped state structs:
...
Update state polling in `crates/eggsec-tui/src/app/state_update.rs` from direct fields to grouped state:
