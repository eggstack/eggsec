...
Legacy closure evidence from the flat era lives inside the grandfathered top-level plan files themselves (completion records appended to each plan, plus standalone `*-closure-report.md` / `*-corrective-*.md` files). That evidence MUST NOT be rewritten; `plans/registry.md` maps it to subsystems. All NEW closure records live here.
...
Record pass, fail, timeout, environmental block, skipped scope, and relevant counts without concealing partial execution. `make check` is the mandatory contract; record deep checks (`make check-full`, `make check-features-individual`, `make clippy-domain`, `make test-tui-pty`) only when actually run.

## 5. Invariant review

For each source-plan invariant, record evidence that it remains true. Enforcement invariants (`EnforcementContext::evaluate()` gate, strict fail-closed, `LoadedScope`, `ApprovedExecution` bundles, `OperationMetadata` as policy source of truth) require explicit evidence, never assumption.
...
- cancellation races (`race_with_cancel` behavior);
...
Record authorization enforcement, scope checks, secret handling (`SensitiveString`), path validation, privilege boundaries, transport checkpoints, denial-of-service bounds, redaction, and audit behavior as applicable.
...
- a security, scope, or migration requirement is unimplemented;
- a strict-surface execution path still bypasses `EnforcedDispatcher::dispatch_execution()`;
