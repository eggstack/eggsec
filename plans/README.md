...
- `001-terminology-and-domain-model.md` — normative language and domain model.
- `002-long-term-roadmap.md` — dependency-ordered long-term roadmap (phases 0–6 closed, phase 7 standing).
...
- `subsystems/` — subsystem specifications and dependency-ordered roadmaps for NEW work. (Flat-era roadmaps are grandfathered at this top level; see below.)
- `implementation/` — focused milestone plans for NEW work, handed to implementation agents.
- `closure/` — verification, evidence, residual-risk, and completion records for NEW milestones.
- `archive/` — completed or superseded interim planning retained for traceability.
- `registry.md` — compact index of subsystem standing, dependency-ready plans, closure work, and blockers.

## Core rule

Long-term documents state **what Eggsec is becoming and what must remain true**. Interim documents state **what an agent should implement next against a specific repository baseline**.
...
6. Implement and verify the milestone (`make check` minimum).
...
## Grandfathered flat-era record

Plans written between 2026-07 and 2026-09 live at this top level under dated filenames with inline `Status: Executed` markers and appended completion records. They predate `003-planning-process.md`, are referenced by `architecture/*.md` at these paths, and are immutable historical records — they MUST NOT be renamed or rewritten. `registry.md` maps each of them to its subsystem. The chronological execution log below is retained as part of that record.

---

# Implementation plan retention (flat-era log retained)
...
## Eggress 1.0.10 adoption and socket-metadata closure (executed 2026-09-25)

Plan:
[eggress-1.0.10-adoption-and-metadata-closure-2026-09-24.md](eggress-1.0.10-adoption-and-metadata-closure-2026-09-24.md)

This narrow Eggress handoff for `eggsec-web-proxy` moves only the
already-approved listener-free dependency edge from exact 1.0.8 to exact
1.0.10, updates Check 106 without widening the allowlist, and closes the
remaining upstream-gated `ProxiedConnection.local_addr` debt now that Eggress
preserves first-hop TCP socket metadata.

Executed: implementation `fe5ec2d2` removes the `0.0.0.0:0`
unknown-local-address sentinel (fail-closed `require_local_addr()` on all
production paths), adds deterministic SOCKS4/SOCKS5/Tor/HTTP CONNECT and
multi-hop first-hop metadata assertions (parity 27 → 30 tests), and documents
that the measured address is the local endpoint of the physical TCP
connection to the first proxy hop—not the final external egress address.
`make check`, feature profiles, dependency policy, and all architecture
guards green locally; completion evidence in the plan. No Eggress capability
expansion: routing, embed/runtime/server, H2, SSH, QUIC/H3, UDP, pproxy
compatibility, insecure TLS, and advanced protocol features remain out of
scope. Reqwest remains the application-level proxy-health owner. Eggress
1.0.10 typed detailed failure classification is a separate future
behavior-change decision and was not adopted.
