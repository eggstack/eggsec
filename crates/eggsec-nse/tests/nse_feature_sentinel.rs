...
//!
//! Without the `nse` feature: this test is marked `#[ignore]` with a message
//! pointing at the right invocation, so plain `cargo test --workspace` runs
//! stay green. Run `cargo test -p eggsec-nse -- --ignored` to surface it.
//! With the `nse` feature: this test passes trivially.

#[cfg(not(feature = "nse"))]
#[test]
#[ignore = "NSE feature-gated tests (local_protocol_tests, runtime_corpus_tests) are skipped. Re-run with: cargo test -p eggsec-nse --features nse"]
