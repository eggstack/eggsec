...
> **Milestone 6 Phase 03 (2026-07-06, TLS/sslcert local fixtures).** `TlsEchoServer` in `local_fixtures.rs` generates self-signed X.509 certs at startup via openssl, creates `native_tls::Identity` via PKCS12, binds `127.0.0.1:0` (ephemeral), accepts TLS connections with per-connection thread spawning. 5 sslcert `.nse` fixture scripts exercise `get_certificate`, `parse_cert`, `get_subject`, `get_chain_certs`, and `is_valid`. All 5 manifest entries with `local_service.type = "tls_echo"`. 5 NSE integration tests + 1 unit test (40 total in local_protocol_tests). Architecture guard Check 53 added. `sslcert.rs` fixed to return actual PEM-encoded certificates. sslcert network functions (`get_certificate`, `get_chain_certs`, `version`) are gated by both `check_crypto` and `check_network_tcp` — denied under AgentSafe/CiSafe. Pure parsing functions (`parse_cert`, `get_subject`, `is_valid`, etc.) have no network gate. 518 eggsec-nse tests pass, 53 architecture guards pass.

> **NSE Expansion Phase 04 (2026-07-07, upstream-style corpus growth).** 6 new upstream-style fixtures added: SSH host key verification (`ssh_hostkey_check.nse`), SMB discovery (`smb_discovery.nse`), LDAP search (`ldap_search.nse`), multi-step discovery (`multi_step_discovery.nse`), conditional portrule (`conditional_portrule.nse`), and timestamp extraction (`timestamp_extract.nse`). Corpus expanded from 39 to 45 fixtures, upstream category from 16 to 22 scripts. New manifest metadata fields: `expected_profile_set`, `expected_evidence_kinds`, `expected_denial_kinds`. Architecture guard Check 54 enforces upstream fixtures are local-only, clean-room, public-network-free. Protocol patterns covered: SSH, SMB, LDAP (approximate — connections may fail locally). Pure-output fixtures: conditional portrule and timestamp extraction are `compatible` under all profiles; multi-step discovery gets `compatible_with_warnings` (hostrule fires with synthetic host context → approximate fidelity). 518 eggsec-nse tests pass, 54 architecture guards pass. Plan: `plans/nse-expansion-phase-04-upstream-style-corpus-growth.md`.
...
10. **NSE Expansion Phase 05 is closed** (2026-07-07). Selective deferred library migration: `creds` library migrated from Deferred to Wrapped. Registry corrected: side effects changed from `FileSystemRead, NetworkAccess` to `None` (pure in-memory credential storage — `FxHashMap<String, Vec<Credential>>` behind `LazyLock<Mutex>`; no filesystem or network operations). `register_creds_library()` signature updated to accept `&NseCapabilityContext` for API consistency (context accepted but unused — no capability checks fire). 4 new runtime corpus fixtures (compatibility_lab, manual_permissive, agent_safe_runtime, ci_safe_runtime) verify creds operations succeed across all profiles with zero capability events. Architecture guard added. 522+ tests pass, 55 architecture guards pass. See `architecture/nse_integration.md#expansion-phase-05`.
