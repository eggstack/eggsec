...
use eggsec::scanner::EndpointScanConfig;
use std::sync::Arc;
...
async fn test_endpoint_scan_basic() {
    ...
    let results = eggsec::scanner::scan_endpoints(config).await.unwrap();
...
async fn test_timing_config_presets() {
    use eggsec::scanner::TimingPreset;
...
async fn test_port_scan_results_display() {
    let results = eggsec::scanner::PortScanResults {
