...
async fn test_session_tester_fixation_detected() {
    ...
    let tester = eggsec::auth::SessionTester::new(5).unwrap();
...
async fn test_mfa_tester_no_mfa() {
    ...
    let tester = eggsec::auth::MfaTester::new(5).unwrap();
    let result = tester.test(&server.uri()).await.unwrap();
...
async fn test_rate_limit_tester_no_limit() {
    ...
    let tester = eggsec::auth::RateLimitTester::new(5).unwrap();
    let result = tester.test(&server.uri()).await.unwrap();
...
async fn test_password_policy_tester_no_policy() {
    ...
    let tester = eggsec::auth::PasswordPolicyTester::new(5).unwrap();
    let result = tester.test(&server.uri()).await.unwrap();
...
async fn test_timing_tester() {
    ...
    let tester = eggsec::auth::TimingTester::new(5).unwrap();
    let result = tester.test(&server.uri()).await.unwrap();
...
async fn test_brute_force_tester_weak_credential() {
    ...
    let tester = eggsec::auth::BruteForceTester::new(10, 1, 5).unwrap();
    let result = tester
        .test(
...
async fn test_lockout_detector_hard_lockout() {
    ...
    let detector = eggsec::auth::LockoutDetector::new(5).unwrap();
    let result = detector
    ...
    assert_eq!(
        result.lockout_type,
        eggsec::auth::lockout::LockoutType::HardLockout
    );
...
async fn test_lockout_detector_soft_lockout() {
    ...
    assert_eq!(
        result.lockout_type,
        eggsec::auth::lockout::LockoutType::SoftLockout
    );
}
...
async fn test_lockout_detector_no_lockout() {
    ...
    assert_eq!(
        result.lockout_type,
        eggsec::auth::lockout::LockoutType::None
    );
}
...
async fn test_credential_stuffer() {
    ...
    let stuffer = eggsec::auth::CredentialStuffer::new(10, 1, 5).unwrap();
    let credentials = vec![
        eggsec::auth::CredentialPair {
