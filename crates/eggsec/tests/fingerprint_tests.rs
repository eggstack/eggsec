...
fn test_probes_not_empty() {
    ...
    let fp = eggsec::scanner::ServiceFingerprint {
        port: 80,
...
fn test_fingerprint_results_display() {
    let results = eggsec::scanner::FingerprintResults {
...
fn test_spoof_config_default() {
    let config = eggsec::scanner::SpoofConfig::default();
    assert!(!config.enabled);
}

#[test]
fn test_random_ip_from_cidr() {
    let ip = eggsec::scanner::random_ip_from_cidr("10.0.0.0/24");
    assert!(ip.is_ok());
...
fn test_invalid_cidr() {
    let result = eggsec::scanner::random_ip_from_cidr("invalid");
