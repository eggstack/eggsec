...
async fn test_waf_detector_cloudflare_header() {
    ...
    let detector = eggsec::waf::detector::WafDetector::new().unwrap();
    let result = detector
...
fn test_waf_signatures_not_empty() {
    let signatures = eggsec::waf::waf_patterns::get_waf_signatures();
...
fn test_common_response_patterns() {
    let patterns = eggsec::waf::waf_patterns::get_common_waf_response_patterns();
...
fn test_bypass_header_generation() {
    let user_agents = eggsec::waf::bypass::headers::get_user_agents();
...
fn test_xff_ip_generation() {
    let ips = eggsec::waf::bypass::headers::generate_xff_ips();
    assert!(!ips.is_empty(), "Should generate XFF IPs");
}

#[test]
fn test_evasion_case_rotation() {
    let result = eggsec::waf::bypass::evasion::apply_case_rotation("SELECT * FROM users");
...
fn test_evasion_comment_obfuscation() {
    let result = eggsec::waf::bypass::evasion::apply_comment_obfuscation("SELECT");
    assert!(!result.is_empty(), "Should produce obfuscated output");
}

#[test]
fn test_smuggling_payloads() {
    let cl_te = eggsec::waf::bypass::smuggling::generate_cl_te_payloads();
    let te_cl = eggsec::waf::bypass::smuggling::generate_te_cl_payloads();
...
fn test_waf_profiles() {
    let profiles = eggsec::waf::bypass::profiles::get_waf_profiles();
    assert!(!profiles.is_empty(), "Should have WAF profiles");

    let cloudflare = eggsec::waf::bypass::profiles::get_profile_by_name("cloudflare");
    assert!(cloudflare.is_some(), "Should find Cloudflare profile");
}

#[test]
fn test_auto_profile() {
    let profile = eggsec::waf::bypass::profiles::get_auto_profile();
