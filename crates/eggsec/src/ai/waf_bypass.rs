...
impl SmartWafBypass {
    ...
    pub fn with_config(client: AiClient, max_bypasses: usize) -> Self {
        let persist_path = directories::ProjectDirs::from("com", "eggsec", "eggsec")
    ...
    fn evict_knowledge_base_if_needed(&mut self) {
        if self.knowledge_base.len() >= self.max_knowledge_base_size {
            ...
            self.knowledge_base.sort_by(|a, b| {
                b.failed_attempts.cmp(&a.failed_attempts).then(
                    now.saturating_sub(b.last_accessed)
                        .cmp(&now.saturating_sub(a.last_accessed)),
                )
            });
