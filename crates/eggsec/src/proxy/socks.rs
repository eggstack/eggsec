...
impl SocksProxy {
    ...
    pub async fn connect_with_domain(&self, domain: &str, port: u16) -> Result<TcpStream> {
        match self.version {
            ...
            SocksVersion::V4 => Err(EggsecError::Proxy(
    ...
    async fn socks5_handshake(&self, stream: &mut TcpStream) -> Result<()> {
        ...
        if response[0] != 0x05 {
            return Err(EggsecError::Proxy("Invalid SOCKS5 response".to_string()));
        }
    ...
    async fn socks5_auth(&self, stream: &mut TcpStream) -> Result<()> {
        let username = self
            .username
            .as_ref()
            .ok_or_else(|| EggsecError::Proxy("Username required".to_string()))?;
        let password = self
            .password
            .as_ref()
            .ok_or_else(|| EggsecError::Proxy("Password required".to_string()))?;
    ...
    async fn socks5_connect_domain(
        ...
        if domain.len() > 255 {
            return Err(EggsecError::Proxy("Domain name too long".to_string()));
        }
...
fn map_socks5_error(code: u8) -> EggsecError {
    match code {
        0x01 => EggsecError::Proxy("SOCKS5: General failure".to_string()),
        0x02 => EggsecError::Proxy("SOCKS5: Connection not allowed by ruleset".to_string()),
        0x03 => EggsecError::Proxy("SOCKS5: Network unreachable".to_string()),
        0x04 => EggsecError::Proxy("SOCKS5: Host unreachable".to_string()),
        0x05 => EggsecError::Proxy("SOCKS5: Connection refused".to_string()),
        0x06 => EggsecError::Proxy("SOCKS5: TTL expired".to_string()),
        0x07 => EggsecError::Proxy("SOCKS5: Command not supported".to_string()),
        0x08 => EggsecError::Proxy("SOCKS5: Address type not supported".to_string()),
        _ => EggsecError::Proxy(format!("SOCKS5: Unknown error 0x{:02X}", code)),
    }
}
...
pub async fn connect_through(proxy: ProxyEntry, target: SocketAddr) -> Result<ProxiedConnection> {
    let version = match proxy.proxy_type {
        ...
        _ => return Err(EggsecError::Proxy("Not a SOCKS proxy".to_string())),
    };
...
pub async fn chain_connect(proxies: &[ProxyEntry], target: SocketAddr) -> Result<TcpStream> {
    if proxies.is_empty() {
        return Err(EggsecError::Proxy("No proxies in chain".to_string()));
    }
