...
impl ProxyManager {
    ...
    pub async fn create_connection(&self, target: &str) -> Result<ProxiedConnection> {
        let proxy = self
            .get_healthy_proxy()
            .await
            .ok_or_else(|| EggsecError::Proxy("No healthy proxies available".to_string()))?;
    ...
    pub async fn create_chained_connection(
        ...
        let chain = self
            ...
            .ok_or_else(|| EggsecError::Proxy("Failed to select proxy chain".to_string()))?;
...
async fn resolve_target(target: &str) -> Result<SocketAddr> {
    ...
    if target.contains(':') {
        ...
        let mut addrs = tokio::net::lookup_host(&target_addr)
            .await
            .map_err(|e| EggsecError::Proxy(format!("DNS lookup failed: {}", e)))?;
        ...
        if let Some(addr) = addrs.next() {
            ...
            Err(EggsecError::Proxy(format!("Failed to resolve {}", target)))
        }
    } else {
        Err(EggsecError::Proxy(format!(
