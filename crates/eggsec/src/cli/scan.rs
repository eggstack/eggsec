...
Examples:
  ...
  eggsec scan-endpoints https://example.com -w wordlist.txt -c 20
...
Examples:
  eggsec fingerprint example.com
  eggsec fingerprint 192.168.1.1 -p 22,80,443,3306
  eggsec fingerprint example.com --json
  eggsec fingerprint example.com --udp  # Requires root/sudo";
...
Examples:
  ...
  eggsec scan example.com --profile endpoint
  eggsec scan example.com --profile web  # Web-focused assessment
  eggsec scan example.com --profile waf  # WAF evaluation
  eggsec scan https://example.com --stages port,fingerprint,endpoint
  eggsec scan example.com -o report.html --format html
  eggsec scan example.com --json -o results.json";
...
pub struct ScanArgs {
    ...
    #[arg(
        short = 'c',
        long,
        help = "Concurrent requests (default: 10 or config scan.default_concurrency)"
    )]
