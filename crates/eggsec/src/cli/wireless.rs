pub(crate) const WIRELESS_ABOUT: &str = "MODE: Defense Lab (passive) | Lab-only; authorized use only.

Scan wireless networks for security issues (passive reconnaissance only; no injection or handshake capture).
...
--detect-suspicious controls inclusion of full rogue/suspicious details in human output (analysis always runs).
...
Examples:
  sudo eggsec wireless wlan0
  sudo eggsec wireless wlan0 --json
  sudo eggsec wireless wlan0 -o results.json
  sudo eggsec wireless wlan0 --duration 15
  sudo eggsec wireless wlan0 --repeat 5 --duration 10
  sudo eggsec wireless wlan0 --detect-suspicious
  sudo eggsec wireless wlan0 --repeat 3 --known-good ./lab-aps.txt
  sudo eggsec wireless wlan0 --dry-run --json
  eggsec wireless wlan0 -q --repeat 2 --json";
...
pub struct WirelessArgs {
    ...
    pub detect_suspicious: bool, // exposed as --detect-suspicious (clap normalizes)
