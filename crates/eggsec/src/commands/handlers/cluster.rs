    ...
    match &args.command {
        ClusterCommand::Worker(worker_args) => {
            ...
            match worker.start().await {
                ...
                Err(e) => {
                    anyhow::bail!("Failed to connect to coordinator: {}. Make sure the coordinator is running with 'eggsec cluster coordinator --psk <key>'", e);
                }
            }
        ...
        ClusterCommand::Coordinator(coordinator_args) => {
            ...
            println!(
                "Workers can connect using: eggsec cluster worker --coordinator {}:{} --psk <key>",
                bind_addr, coordinator_args.port
            );
...
pub async fn handle_remote(ctx: &CommandContext, args: crate::cli::RemoteArgs) -> Result<()> {
    ...
    match &args.command {
        ...
        RemoteCommand::Cert(_cert_args) => {
            ...
            println!();
            println!("Usage:");
            println!("  eggsec remote start --tls-cert cert.pem --tls-key key.pem");
            println!();
