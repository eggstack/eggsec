...
- **Dynamic (Phase 1 + Phase 2 closed 2026-06-12)**: Pure-Rust ADB TCP framing (emulator-XXXX primary; host:port supported); external `adb` binary only for `list_devices` convenience (plus `--list-devices` convenience per polish). High-signal-only log parser (no full log processing). Phase 2 (proxy foundation + runtime permissions + correlation) closed 2026-06-12 per combined closeout+kickoff plan (executed): new `traffic.rs`; adb helpers `set_global_proxy`/`clear_global_proxy` + `grant_permission`/`revoke_permission`/`list_permissions`; `DynamicMobileReport` extensions (`traffic_summary: Option<TrafficSummary>`, `permission_state: Option<PermissionState>`); `to_scan_report_data_dynamic` emits extra info findings (categories `mobile-dynamic-android-*`); `correlate_findings` populates `static_correlation`. CLI flags: `--proxy`/`--reset-proxy`/`--traffic-capture` + `--grant-permission`/`--revoke-permission`/`--list-permissions`. All under `mobile-dynamic` (M1 decision: no new sub-feature / kept flat; Phase 3 will add gated `mobile-frida`). Policy enforced in handler (DefenseLab mode + explicit `--allow-dynamic-mobile` for non-dry runs; audited). Dry-run always valid (produces complete serializable report, zero device/net side effects). All actions recorded in `actions_performed`; best-effort cleanup (uninstall) always attempted on real install paths. Standalone defense-lab (MCP/agent absent; same pattern as wireless-active + static-mobile + auth-test). Final polish + close-out delivered correlation, parser robustness, report surface, doc hygiene. See `dynamic.rs:127`, `adb.rs`, `runtime.rs`, `traffic.rs`, `commands/handlers/mobile.rs:26`. Feature gating decision (M1): all Phase 1+2 dynamic functionality kept under the single `mobile-dynamic` feature (no split to mobile-dynamic-advanced); decision recorded in Cargo.toml comments, docs/MOBILE.md, AGENTS.override.md, root AGENTS.md. Phase 3 will introduce mobile-frida.
