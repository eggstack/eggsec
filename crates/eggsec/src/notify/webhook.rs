...
impl WebhookNotifier {
    ...
    fn build_slack_payload(&self, payload: &NotificationPayload) -> serde_json::Value {
        ...
        let mut attachment = serde_json::json!({
            "color": color,
            "title": format!("Eggsec - {:?}", payload.event),
            "fields": [
                {
                    "title": "Target",
            ...
            "footer": "Eggsec Security Scanner",
    ...
    fn build_teams_payload(&self, payload: &NotificationPayload) -> serde_json::Value {
        ...
        serde_json::json!({
            ...
            "themeColor": color,
            "summary": format!("Eggsec - {:?}", payload.event),
            "sections": [{
                "activityTitle": format!("Eggsec - {:?}", payload.event),
                "facts": facts,
                "markdown": true
            }],
            "potentialAction": [{
