//! `NetworkAuthority` binding over the canonical scope model.
//!
//! [`ScopeAuthority`] implements [`eggsec_transport::NetworkAuthority`] using
//! the single canonical policy ([`eggsec_policy::Scope`] /
//! [`eggsec_policy::TargetScope`] + `classify_address`).
//! No second target-policy language is introduced: every checkpoint
//! delegates to scope evaluation with DNS facts supplied by the
...
use eggsec_policy::{classify_address, AddressClass, Scope, TargetScope};
...
///
/// Borrows the [`Scope`]; the transport supplies DNS facts per call.
#[derive(Debug)]
