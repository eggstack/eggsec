...
pub fn random_ip_from_cidr(cidr: &str) -> Result<Ipv4Addr> {
    ...
    let base: Ipv4Addr = parts[0]
        .parse()
        .map_err(|_| EggsecError::Parse("Invalid IP address in CIDR".to_string()))?;
    let prefix: u8 = parts[1]
        .parse()
        .map_err(|_| EggsecError::Parse("Invalid prefix length".to_string()))?;
...
pub fn build_tcp_packet(
    ...
    let mut tcp_packet = MutableTcpPacket::new(&mut buffer[20..])
        .ok_or_else(|| EggsecError::Runtime("Failed to create TCP packet".to_string()))?;
...
pub fn get_local_ip(interface: &NetworkInterface) -> Result<Ipv4Addr> {
    interface
        ...
        .ok_or_else(|| EggsecError::Network("No IPv4 address found on interface".to_string()))
}
