...
impl TemplateSigner {
    ...
    pub fn from_keypair(signing_key: &[u8], signer_info: SignerInfo) -> Result<Self> {
        let signing_key = SigningKey::from_bytes(
            signing_key
                .try_into()
                .map_err(|_| EggsecError::Validation("Invalid signing key length".to_string()))?,
        );
        let public_key = signing_key.verifying_key();
    ...
    pub fn sign(&self, template: &VulnerabilityTemplate) -> Result<SignedTemplate> {
        let template_bytes =
            serde_yaml_neo::to_string(template).map_err(|e| EggsecError::Parse(e.to_string()))?;
...
impl TemplateVerifier {
    ...
    pub fn with_public_key(public_key: &str) -> Result<Self> {
        let key_bytes =
            base64::Engine::decode(&base64::engine::general_purpose::STANDARD, public_key)
                .map_err(|e| EggsecError::Parse(e.to_string()))?;

        let key_array: [u8; 32] = key_bytes
            .try_into()
            .map_err(|_| EggsecError::Validation("Invalid public key length".to_string()))?;

        let verifying_key = VerifyingKey::from_bytes(&key_array)
            .map_err(|e| EggsecError::Validation(format!("Invalid public key: {}", e)))?;
    ...
    pub fn verify(&self, signed_template: &SignedTemplate) -> Result<VerificationResult> {
        let verifying_key = self.verifying_key.as_ref().ok_or_else(|| {
            EggsecError::Validation("No public key configured for verification".to_string())
        })?;
        ...
        let signature = Signature::from_bytes(
            &signature_bytes
                .try_into()
                .map_err(|_| EggsecError::Validation("Invalid signature length".to_string()))?,
        );
...
impl SignedTemplate {
    pub fn load(path: &Path) -> Result<Self> {
        ...
        serde_yaml_neo::from_str(&content).map_err(|e| EggsecError::Parse(e.to_string()))
    }

    pub fn save(&self, path: &Path) -> Result<()> {
        let content =
            serde_yaml_neo::to_string(self).map_err(|e| EggsecError::Output(e.to_string()))?;
...
pub fn load_private_key(path: &Path) -> Result<[u8; 32]> {
    ...
    key_bytes
        .try_into()
        .map_err(|_| EggsecError::Validation("Invalid private key length".to_string()))
}
