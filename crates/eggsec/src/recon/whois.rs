...
pub async fn whois_lookup_with_config(domain: &str, config: &WhoisConfig) -> Result<WhoisResult> {
    ...
    Err(last_error.unwrap_or_else(|| EggsecError::Network("WHOIS lookup failed".to_string())))
}
...
async fn lookup_whois(domain: &str, server: &str) -> Result<String> {
    ...
    let socket_addr = addrs
        .next()
        .ok_or_else(|| EggsecError::Network(format!("No address found for {}", server)))?;
    ...
    if response_str.contains("Throttled") || response_str.contains("Rate limit") {
        return Err(EggsecError::RateLimited("WHOIS rate limited".to_string()));
    }

    if response_str.is_empty() {
        return Err(EggsecError::Network("Empty WHOIS response".to_string()));
    }
