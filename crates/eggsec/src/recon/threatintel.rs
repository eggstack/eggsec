...
impl ThreatIntelClient {
    ...
    async fn check_virustotal_ip(&self, ip: &str, api_key: &str) -> Result<IpReputation> {
        ...
        if !response.status().is_success() {
            return Err(EggsecError::Recon("VT API request failed".to_string()));
        }
        ...
        let data = vt_resp
            ...
            .ok_or_else(|| EggsecError::Recon("Invalid VT response".to_string()))?;

        let last_analysis = data
            .get("last_analysis_stats")
            .ok_or_else(|| EggsecError::Recon("No analysis stats".to_string()))?;
    ...
    async fn check_shodan_ip(&self, ip: &str, api_key: &str) -> Result<IpReputation> {
        ...
        if !response.status().is_success() {
            return Err(EggsecError::Recon("Shodan API request failed".to_string()));
        }
