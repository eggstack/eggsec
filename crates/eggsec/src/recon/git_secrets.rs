...
impl GitSecretsScanner {
    ...
    pub fn scan_directory(&self, repo_path: &str) -> Result<GitSecretsReport> {
        ...
        let canonical_path = path.canonicalize().map_err(|e| {
            crate::error::EggsecError::Io(std::io::Error::new(
        ...
        if !canonical_path.exists() {
            return Err(crate::error::EggsecError::Io(std::io::Error::new(
                std::io::ErrorKind::NotFound,
...
mod tests {
    ...
    fn test_scan_current_directory() {
        ...
        assert!(
            result.is_ok(),
            "Git secrets scan failed: {:?}",
            result.err()
        );
        let report = result.unwrap();
        assert!(
            report.commits_scanned <= 100,
            "Expected 0-100 commits, got {}",
