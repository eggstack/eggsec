...
impl ConfigReloader for EggsecConfigReloader {
    fn reload(&self, path: &Path) -> Result<()> {
...
impl EggsecConfigReloader {
    ...
    fn reload_main_config(&self) -> Result<()> {
        if let Some(ref config_path) = self.config_path {
            ...
            match EggsecConfig::load(config_path) {
                Ok(_new_config) => {
...
mod tests {
    ...
    #[test]
    fn test_eggsec_config_reloader_new_signature() {
        let portfolio = TargetPortfolio::new();
        let portfolio_path = PathBuf::from("/tmp/portfolio.json");
        let config_path = PathBuf::from("/tmp/eggsec.toml");

        let reloader = EggsecConfigReloader::new(
            Some(portfolio),
    ...
    fn test_portfolio_file_change_reloads_targets() {
        ...
        let reloader =
            EggsecConfigReloader::new(Some(portfolio.clone()), Some(portfolio_path.clone()), None);
