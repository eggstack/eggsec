//! Autonomous security agent for Eggsec.
//!
...
trait ScanDispatcherTrait: Send + Sync {
    fn dispatch<'a>(
        ...
        Box<
            dyn Future<Output = std::result::Result<ToolResponse, crate::error::EggsecError>>
                + Send
                + 'a,
        >,
    >;
...
impl Default for AgentConfig {
    fn default() -> Self {
        let memory_dir = directories::ProjectDirs::from("com", "eggsec", "eggsec")
            .map(|d| d.config_dir().to_path_buf())
            .unwrap_or_else(|| PathBuf::from("~/.config/eggsec"));
...
impl Agent {
    pub async fn new(config: AgentConfig) -> Result<Self> {
        ...
        // Load alert channels from EggsecConfig
        if let Some(config_path) = EggsecConfig::default_path() {
            if config_path.exists() {
                match crate::config::EggsecConfig::load(&config_path) {
                    Ok(eggsec_config) => {
                        for (name, channel_config) in eggsec_config.alert_channels.channels {
        ...
        let config_paths = std::iter::once(config.portfolio_path.clone())
            .flatten()
            .chain(crate::config::EggsecConfig::default_path())
            .collect::<Vec<_>>();
        let reloader = Arc::new(EggsecConfigReloader::new(
            Some(portfolio.clone()),
            config.portfolio_path.clone(),
            crate::config::EggsecConfig::default_path(),
        ));
...
mod tests {
    ...
    impl ScanDispatcherTrait for MockDispatcher {
        ...
        > {
            ...
            Box::pin(async move {
                response
                    .ok_or_else(|| crate::error::EggsecError::Network("Mock no response".into()))
            })
        }
    }
    ...
    async fn test_scheduled_scan_idempotent() {
        ...
        let base_dir = ProjectDirs::from("com", "eggsec", "eggsec")
            .map(|d| d.config_dir().to_path_buf())
            .unwrap_or_else(|| PathBuf::from("~/.config/eggsec"));
