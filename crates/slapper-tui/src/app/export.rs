...
impl super::App {
    ...
    fn export_csv(&mut self, filename: &str) {
        use slapper::output::csv::{CsvExporter, EndpointCsv, PortCsv};
    ...
    fn export_converted(&mut self, filename: &str) {
        use slapper::output::convert::load_scan_report;
        ...
        let export_dir = self
            ...
            .unwrap_or(slapper_core::constants::DEFAULT_EXPORT_DIR);

        let base_dir = std::path::Path::new(slapper_core::constants::DEFAULT_EXPORT_DIR);
        ...
        match load_scan_report(&json_path) {
            Ok(report) => {
                let converted = match self.export_format {
                    OutputFormat::Html => slapper::output::convert::convert_to_html(&report),
                    OutputFormat::Markdown => {
                        slapper::output::convert::convert_to_markdown(&report)
                            .unwrap_or_else(|e| format!("Error: {}", e))
                    }
                    OutputFormat::Sarif => slapper::output::convert::convert_to_sarif(&report)
                        .unwrap_or_else(|e| format!("Error: {}", e)),
                    OutputFormat::Junit => slapper::output::convert::convert_to_junit(&report)
                        .unwrap_or_else(|e| format!("Error: {}", e)),
                    _ => {
