...
pub async fn run_hunt_task(
    target: String,
    config: slapper::hunt::HuntConfig,
    progress_tx: tokio::sync::mpsc::Sender<(u64, u64)>,
    result_tx: tokio::sync::mpsc::Sender<TaskResult>,
) -> anyhow::Result<()> {
    use slapper::hunt::run_hunt;
...
pub async fn run_browser_task(
    target: String,
    config: slapper::browser::BrowserConfig,
    progress_tx: tokio::sync::mpsc::Sender<(u64, u64)>,
    result_tx: tokio::sync::mpsc::Sender<TaskResult>,
) -> anyhow::Result<()> {
    use slapper::browser::run_browser_scan;
...
pub async fn run_compliance_task(
    target: String,
    framework: slapper::compliance::ComplianceFramework,
    progress_tx: tokio::sync::mpsc::Sender<(u64, u64)>,
    result_tx: tokio::sync::mpsc::Sender<TaskResult>,
) -> anyhow::Result<()> {
    use slapper::compliance::generate_compliance_report;
    ...
    if let Ok(resp) = slapper::utils::get_shared_http_client()
        .get(&target)
...
pub async fn run_storage_task(
    config: slapper::storage::StorageConfig,
    mode: String,
    ...
    use slapper::findings::lifecycle::StoredFinding;
    use slapper::storage::init_storage;
    use slapper::storage::models::{ScanStatus, StoredScan};
    ...
    let result_data = match mode.as_str() {
        ...
        "search_cve" => {
            if let Some(ref cve) = cve_id {
                let finding = slapper::findings::Finding {
                    id: uuid::Uuid::new_v4().to_string(),
                    fingerprint: String::new(),
                    title: format!("CVE search: {}", cve),
                    description: format!("Search results for {}", cve),
                    severity: slapper::types::Severity::Medium,
                    confidence: slapper::findings::Confidence::Informational,
                    finding_type: slapper::findings::FindingType::ScanResult,
                    cwe: None,
                    owasp: None,
                    cve: Some(cve.clone()),
                    affected_asset: slapper::findings::AffectedAsset {
                    ...
                    location: slapper::findings::FindingLocation::default(),
                    evidence: vec![],
                    reproduction: None,
                    remediation: None,
                    discovered_at: chrono::Utc::now(),
                    source: slapper::findings::FindingSource {
...
pub async fn run_integrations_task(
    config: slapper::integrations::IntegrationConfig,
    mode: String,
    title: Option<String>,
    ...
    use slapper::integrations::{Issue, IssueTracker};
    ...
    match tokio::time::timeout(std::time::Duration::from_secs(60), async move {
        ...
        let tracker: Option<Box<dyn IssueTracker>> = if let Some(jira_config) = config.jira {
            Some(Box::new(slapper::integrations::jira::JiraClient::new(
                jira_config,
            )))
        } else if let Some(github_config) = config.github {
            Some(Box::new(slapper::integrations::github::GitHubClient::new(
                github_config,
            )))
        } else if let Some(gitlab_config) = config.gitlab {
            Some(Box::new(slapper::integrations::gitlab::GitLabClient::new(
                gitlab_config,
            )))
        } else {
            None
...
pub async fn run_workflow_task(
    ...
    use slapper::workflow::WorkflowReport;
...
pub async fn run_vuln_task(
    ...
    use slapper::types::Severity;
    use slapper::vuln::asset::assess_asset;
    use slapper::vuln::prioritizer::prioritize_findings;
    use slapper::vuln::triage::triage_finding;
    use slapper::vuln::{AssetCriticality, CvssScore, ExploitInfo, Remediation, VulnAssessment};
...
pub async fn run_wireless_task(
    ...
    let scanner = slapper::wireless::WirelessScanner::new();
