...
pub async fn run_port_scan(
    ...
    use slapper::scanner::ports::scan_ports;

    if let Err(e) = progress_tx.send((0, 100)).await {
        tracing::warn!("Failed to send initial progress: {}", e);
    }

    let port_list = slapper::utils::parsing::parse_ports(&ports)?;
    ...
    let results = match tokio::time::timeout(
        ...
        scan_ports(
            &target,
            slapper::scanner::ports::PortScanConfig {
                ports: port_list,
                concurrency,
...
pub async fn run_fingerprint(
    ...
    use slapper::scanner::fingerprint::fingerprint_services;
