...
pub async fn run_load_test(
    ...
    use slapper::loadtest::runner::LoadTestRunner;
...
pub async fn run_stress_test(
    ...
    use slapper::stress::{StressConfig, StressTest, StressType};
...
pub async fn run_packet_capture(
    ...
    use pnet::datalink;
    use slapper::packet::capture::CaptureBuilder;
...
pub async fn run_packet_traceroute(
    ...
    use slapper::packet::traceroute::{Traceroute, TracerouteConfig};
...
fn parse_target_host_port(target: &str) -> (String, u16) {
    ...
    if target.contains("]:") {
        if let Some((host_part, port_part)) = target.rsplit_once("]:") {
            ...
            let port = port_part.parse().unwrap_or_else(|e| {
                tracing::warn!("Invalid port '{}', defaulting to 80: {}", port_part, e);
                80
            });
            return (host, port);
        }
    ...
    if target.matches(':').count() == 1 {
        if let Some((host, port_part)) = target.split_once(':') {
            return (
                host.to_string(),
                port_part.parse().unwrap_or_else(|e| {
                    tracing::warn!("Invalid port '{}', defaulting to 80: {}", port_part, e);
                    80
                }),
            );
