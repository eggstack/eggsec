...
use slapper::config::ProxyConfigEntry;
use slapper::proxy::{HealthCheckConfig, HealthChecker, ProxyEntry, ProxyType};
    ...
    pub async fn run_health_check(&mut self, test_url: &str) {
        ...
        let proxy_entries: Vec<ProxyEntry> = self
            ...
            .map(|p| ProxyEntry {
                ...
                timeout_ms: slapper_core::constants::DEFAULT_PROXY_TIMEOUT_MS,
                enabled: p.enabled,
                tags: Vec::new(),
            })
            .collect();
