...
use slapper::integrations::IntegrationConfig;
...
impl IntegrationsTab {
    ...
    fn update_config_labels(&mut self) {
        ...
        let (l0, l1, l2, l3) = match tracker {
            "github" => ("Owner", "Repository", "API Token", ""),
            "gitlab" => ("GitLab URL", "Project ID", "API Token", ""),
            _ => ("Jira URL", "Username", "Project Key", "API Token"),
        };
    ...
    pub fn get_config(&self) -> IntegrationConfig {
        use slapper::integrations::{github::GitHubConfig, gitlab::GitLabConfig, jira::JiraConfig};
        use slapper::types::SensitiveString;
...
impl TabRender for IntegrationsTab {
    ...
    fn render(&self, f: &mut Frame, area: Rect, insert_mode: bool) {
        ...
        let Some(input_area) = chunks.get(0).copied() else {
            return;
        };
        let Some(results_area) = chunks.get(1).copied() else {
            return;
        };
