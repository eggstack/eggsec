...
impl SettingsTab {
        ...
        config.http.proxy_auth = self
            ...
            .map(|f| {
                if f.value.is_empty() {
                    ...
                    Some(slapper::types::SensitiveString::new(f.value.clone()))
                }
            })
            .unwrap_or(None);
    ...
    pub fn to_config(&self) -> SlapperConfig {
        let mut config = self.config.clone().unwrap_or_else(|| SlapperConfig {
            http: HttpConfig {
                timeout_secs: slapper_core::constants::http::DEFAULT_TIMEOUT_SECS,
                max_retries: slapper_core::constants::DEFAULT_MAX_RETRIES,
                ...
                retry_delay_ms: slapper_core::constants::DEFAULT_RETRY_DELAY_MS,
            },
            scan: ScanConfig {
            ...
            paths: slapper::config::PathsConfig {
                custom_payloads_dir: None,
                wordlists_dir: None,
                export_dir: None,
            },
            profiles: rustc_hash::FxHashMap::default(),
            recon: slapper::config::ReconConfig::default(),
            schedule: Vec::new(),
            remote: slapper::config::RemoteConfig::default(),
            proxies: Vec::new(),
            ai: None,
            search: None,
            alert_channels: slapper::config::AlertChannelsConfig::default(),
            execution_policy: slapper::config::ExecutionPolicy::default(),
            #[cfg(feature = "external-integrations")]
            integrations: slapper::integrations::IntegrationConfig::default(),
    ...
    pub fn convert_report(&mut self) -> Result<String, String> {
        ...
        let report = slapper::output::convert::load_scan_report(input)
            .map_err(|e| format!("Failed to load report: {}", e))?;

        let converted = match output_format.parse::<slapper::types::OutputFormat>() {
            Ok(slapper::types::OutputFormat::Junit) => {
                slapper::output::convert::convert_to_junit(&report)
                    .unwrap_or_else(|e| format!("Error: {}", e))
            }
            Ok(slapper::types::OutputFormat::Csv) => {
                slapper::output::convert::convert_to_csv(&report)
            }
            Ok(slapper::types::OutputFormat::Html) => {
                slapper::output::convert::convert_to_html(&report)
            }
            Ok(slapper::types::OutputFormat::Sarif) => {
                slapper::output::convert::convert_to_sarif(&report)
                    .unwrap_or_else(|e| format!("Error: {}", e))
            }
            Ok(slapper::types::OutputFormat::Markdown) => {
                slapper::output::convert::convert_to_markdown(&report)
                    .unwrap_or_else(|e| format!("Error: {}", e))
            }
            _ => slapper::output::convert::convert_to_html(&report),
        };
