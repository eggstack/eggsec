...
use slapper::compliance::{ComplianceFramework, ComplianceReport, ComplianceStatus};
    ...
    pub fn set_report(&mut self, report: ComplianceReport) {
        ...
        self.results_view.add_line(Line::from(Span::styled(
            ...
            Style::default().fg(match summary.risk_level {
                slapper::compliance::report::RiskLevel::Critical => tc!(error),
                slapper::compliance::report::RiskLevel::High => tc!(error),
                slapper::compliance::report::RiskLevel::Medium => tc!(warning),
                slapper::compliance::report::RiskLevel::Low => tc!(success),
            }),
        )));
        self.results_view.add_line(Line::from(""));
        let na_count = report
