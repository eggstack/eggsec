//! Autonomous-agent cron scheduling (Phase A ownership cleanup).
//!
//! Owns [`CronExpression`] parsing/matching and [`CronScheduler`] aggregation
//! for the autonomous security agent. This is the narrowest real execution
//! owner: the only durable consumer of cron matching is autonomous-agent
//! scheduling (`eggsec::agent`), which evaluates portfolio `schedule` strings
//! on each tick.
//!
//! Scan-specific DTOs (`ScheduledScan`, `ScanQueue`, `ScanType`, `ScanOptions`,
//! `Priority`) intentionally do **not** live here. The generic
//! [`crate::scheduler::TaskScheduler`] is the canonical queue (priority,
//! delayed execution, retry, leasing, cancellation); scan request types belong
//! in the engine composition layer and translate into generic tasks.
//!
//! Time is acquired via `chrono::Utc` only; no Tokio `test-util` needed.

use std::str::FromStr;

/// Parsed cron expression (5 fields `min hour dom month dow` or 6 fields with
/// leading seconds).
#[derive(Debug, Clone)]
...
/// Minimal aggregation of cron expressions used by the autonomous agent tick.
///
/// Matching itself is stateless (`CronExpression::matches`); this type only
/// owns the expression list so multiple portfolio schedules can be evaluated
/// together.
