...
pub fn convert_to_junit(report: &ScanReportData) -> Result<String, String> {
    ...
    let mut builder = JUnitBuilder::new("Eggsec Security Scan");

    for finding in &report.findings {
        let result = if matches!(
            parse_severity(&finding.severity),
            eggsec_core::types::Severity::Critical | eggsec_core::types::Severity::High
        ) {
...
pub fn convert_to_sarif(report: &ScanReportData) -> Result<String, String> {
    ...
    for finding in &report.findings {
        let level = match parse_severity(&finding.severity) {
            eggsec_core::types::Severity::Critical | eggsec_core::types::Severity::High => "error",
            eggsec_core::types::Severity::Medium => "warning",
            _ => "note",
        };
...
impl From<&ScanReportData> for crate::markdown::ScanSummary {
    fn from(report: &ScanReportData) -> Self {
        let findings_by_severity = |sev: eggsec_core::types::Severity| {
            report
                .findings
                .iter()
        ...
        crate::markdown::ScanSummary {
            ...
            critical_count: findings_by_severity(eggsec_core::types::Severity::Critical),
            high_count: findings_by_severity(eggsec_core::types::Severity::High),
            medium_count: findings_by_severity(eggsec_core::types::Severity::Medium),
            low_count: findings_by_severity(eggsec_core::types::Severity::Low),
            info_count: findings_by_severity(eggsec_core::types::Severity::Info),
        }
    }
}
