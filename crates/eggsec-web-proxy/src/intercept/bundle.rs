...
use crate::intercept::correlation::CorrelationReference;
use crate::intercept::rules::EnhancedRuleSet;
...
pub struct EvidenceBundle {
    ...
    /// Proxy flows.
    pub flows: Vec<crate::intercept::types::ProxyFlow>,
    /// WebSocket sessions.
    pub ws_sessions: Vec<crate::intercept::protocols::WebSocketSession>,
    /// HTTP/2 sessions.
    pub http2_sessions: Vec<crate::intercept::protocols::Http2Session>,
    /// gRPC sessions.
    pub grpc_sessions: Vec<crate::intercept::protocols::GrpcSession>,
    /// Enhanced rule set snapshot.
    pub rules: EnhancedRuleSet,
    /// Manipulation audit trail.
    pub manipulations: Vec<crate::intercept::types::ManipulationRecord>,
...
impl EvidenceBundle {
    ...
    pub fn to_bytes(&self) -> Result<Vec<u8>> {
        let json = serde_json::to_vec_pretty(self).map_err(|e| {
            WebProxyError::Proxy(format!("Failed to serialize evidence bundle: {}", e))
        })?;
        let mut encoder = flate2::write::GzEncoder::new(Vec::new(), flate2::Compression::default());
        encoder.write_all(&json).map_err(|e| {
            WebProxyError::Proxy(format!("Failed to compress evidence bundle: {}", e))
        })?;
        encoder
            .finish()
            .map_err(|e| WebProxyError::Proxy(format!("Failed to finish gzip stream: {}", e)))
    }
    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self> {
        ...
        decoder.read_to_string(&mut json).map_err(|e| {
            WebProxyError::Proxy(format!("Failed to decompress evidence bundle: {}", e))
        })?;
        serde_json::from_str(&json).map_err(|e| {
            WebProxyError::Proxy(format!("Failed to deserialize evidence bundle: {}", e))
        })
    }
    ...
    pub fn sign(&mut self, key: &[u8], key_id: Option<&str>) -> Result<()> {
        ...
        let mut mac = HmacSha256::new_from_slice(key)
            .map_err(|e| WebProxyError::Proxy(format!("HMAC key error: {}", e)))?;
    ...
    pub fn verify(&self, key: &[u8]) -> Result<bool> {
        ...
        let provided_sig = self
            ...
            .signature
            .as_deref()
            .ok_or_else(|| WebProxyError::Proxy("Bundle is unsigned".to_string()))?;

        let sig_bytes = hex::decode(provided_sig)
            .map_err(|e| WebProxyError::Proxy(format!("Invalid signature hex: {}", e)))?;
...
pub fn export_evidence_bundle(
    ...
    let mut file = std::fs::File::create(bundle_path).map_err(|e| {
        WebProxyError::Proxy(format!(
            "Failed to create bundle file {}: {}",
            bundle_path, e
        ))
    })?;
    file.write_all(&bytes)
        .map_err(|e| WebProxyError::Proxy(format!("Failed to write bundle file: {}", e)))?;
...
pub fn import_evidence_bundle(bundle_path: &str) -> Result<EvidenceBundle> {
    let data = std::fs::read(bundle_path).map_err(|e| {
        WebProxyError::Proxy(format!("Failed to read bundle file {}: {}", bundle_path, e))
    })?;
...
mod tests {
    ...
    fn test_evidence_bundle_ws_http2_grpc_counts() {
        ...
        report
            .ws_sessions
            .push(crate::intercept::protocols::WebSocketSession::new(
                "wss://example.com/ws",
                "example.com",
                "/ws",
                true,
            ));
        let mut h2 = crate::intercept::protocols::Http2Session::new("example.com", true);
        h2.add_stream(crate::intercept::protocols::Http2Stream::new(
            1, "GET", "/data",
        ));
        report.http2_sessions.push(h2);
        let mut grpc = crate::intercept::protocols::GrpcSession::new("example.com", true);
        grpc.add_call(crate::intercept::protocols::GrpcCall::new(
            "/pkg.Svc/Method",
