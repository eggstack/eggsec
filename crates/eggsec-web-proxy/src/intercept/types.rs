...
impl WebProxySessionReport {
    ...
    pub fn save_to_file(&self, path: &str) -> Result<(), crate::error::WebProxyError> {
        let json = serde_json::to_string_pretty(self).map_err(|e| {
            crate::error::WebProxyError::Proxy(format!("Failed to serialize session: {}", e))
        })?;
        std::fs::write(path, json).map_err(|e| {
            crate::error::WebProxyError::Proxy(format!("Failed to write session file: {}", e))
        })?;
        Ok(())
    }

    /// Load a session report from a JSON file for resume.
    pub fn load_from_file(path: &str) -> Result<Self, crate::error::WebProxyError> {
        let json = std::fs::read_to_string(path).map_err(|e| {
            crate::error::WebProxyError::Proxy(format!("Failed to read session file: {}", e))
        })?;
        serde_json::from_str(&json).map_err(|e| {
            crate::error::WebProxyError::Proxy(format!("Failed to deserialize session: {}", e))
        })
    }
...
impl InterceptSession {
    ...
    pub fn save_to_file(&self, path: &str) -> error::Result<()> {
        let json = serde_json::to_string_pretty(self).map_err(|e| {
            error::WebProxyError::Proxy(format!("Failed to serialize session: {}", e))
        })?;
        std::fs::write(path, json).map_err(|e| {
            error::WebProxyError::Proxy(format!("Failed to write session file: {}", e))
        })?;
        Ok(())
    }
    ...
    pub fn load_from_file(path: &str) -> error::Result<Self> {
        let json = std::fs::read_to_string(path).map_err(|e| {
            error::WebProxyError::Proxy(format!("Failed to read session file: {}", e))
        })?;
        serde_json::from_str(&json).map_err(|e| {
            error::WebProxyError::Proxy(format!("Failed to deserialize session: {}", e))
        })
    }
