...
impl ProxyServer {
    ...
    pub async fn start(&self) -> Result<()> {
        let listener = TcpListener::bind(self.addr)
            .await
            .map_err(|e| WebProxyError::Network(format!("Failed to bind proxy: {}", e)))?;
...
fn validate_target(host: &str, port: u16) -> Result<()> {
    let ip: IpAddr = host
        .parse()
        .map_err(|_| WebProxyError::Proxy(format!("Invalid host address: {}", host)))?;
...
async fn read_http_headers(stream: &mut (impl tokio::io::AsyncRead + Unpin)) -> Result<Vec<u8>> {
    ...
    loop {
        let n = stream
            .read(&mut buf)
            .await
            .map_err(|e| WebProxyError::Proxy(format!("Failed to read HTTP headers: {}", e)))?;
        if n == 0 {
            break;
        }
...
async fn handle_connect_request(
    ...
    let host_port = request
        ...
        .next()
        .and_then(|line| line.split_whitespace().nth(1))
        .ok_or_else(|| WebProxyError::Proxy("Invalid CONNECT request".to_string()))?;
    ...
    .await
    .map_err(|e| WebProxyError::Network(format!("Connection timeout to upstream: {}", e)))?
    .map_err(|e| WebProxyError::Network(format!("Failed to connect to upstream: {}", e)))?;

    let material = cert_gen
        .generate_for_host(host)
        .map_err(|e| WebProxyError::Proxy(format!("Cert generation failed: {}", e)))?;

    let tls_acceptor = create_tls_acceptor(&material)
        .map_err(|e| WebProxyError::Proxy(format!("TLS config failed: {}", e)))?;
    ...
    upstream.write_all(&request_bytes).await.map_err(|e| {
        WebProxyError::Proxy(format!("Failed to forward request to upstream: {}", e))
    })?;
...
mod tests {
    ...
    fn test_proxy_server_with_enhanced_rules() {
        use crate::intercept::rules::{RuleAction, RuleCondition};
