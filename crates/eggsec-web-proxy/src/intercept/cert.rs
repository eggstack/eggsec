...
impl CertGenerator {
    ...
    fn generate_cert(&self, host: &str) -> Result<CertMaterial> {
        ...
        let mut params = CertificateParams::new(alt_names)
            .map_err(|e| WebProxyError::Proxy(format!("Certificate params failed: {}", e)))?;
        ...
        let key_pair = KeyPair::generate()
            .map_err(|e| WebProxyError::Proxy(format!("Key generation failed: {}", e)))?;

        let cert = params
            .self_signed(&key_pair)
            .map_err(|e| WebProxyError::Proxy(format!("Certificate creation failed: {}", e)))?;
