...
impl SocksProxy {
    ...
    pub async fn connect_with_domain(&self, domain: &str, port: u16) -> Result<TcpStream> {
        match self.version {
            ...
            SocksVersion::V4 => Err(WebProxyError::Proxy(
    ...
    async fn connect_socks4(&self, target: SocketAddr) -> Result<TcpStream> {
        let mut stream = connect_with_nodelay_timeout(&self.proxy_addr, self.timeout)
            .await
            .map_err(|e| WebProxyError::Proxy(format!("Failed to connect to proxy: {}", e)))?;
    ...
    async fn socks5_handshake(&self, stream: &mut TcpStream) -> Result<()> {
        ...
        if response[0] != 0x05 {
            return Err(WebProxyError::Proxy("Invalid SOCKS5 response".to_string()));
        }
    ...
    async fn socks5_auth(&self, stream: &mut TcpStream) -> Result<()> {
        let username = self
            .username
            .as_ref()
            .ok_or_else(|| WebProxyError::Proxy("Username required".to_string()))?;
        let password = self
            .password
            .as_ref()
            .ok_or_else(|| WebProxyError::Proxy("Password required".to_string()))?;
    ...
    async fn socks5_connect_domain(
        ...
        if domain.len() > 255 {
            return Err(WebProxyError::Proxy("Domain name too long".to_string()));
        }
...
fn map_socks5_error(code: u8) -> WebProxyError {
    match code {
        0x01 => WebProxyError::Proxy("SOCKS5: General failure".to_string()),
        0x02 => WebProxyError::Proxy("SOCKS5: Connection not allowed by ruleset".to_string()),
        0x03 => WebProxyError::Proxy("SOCKS5: Network unreachable".to_string()),
        0x04 => WebProxyError::Proxy("SOCKS5: Host unreachable".to_string()),
        0x05 => WebProxyError::Proxy("SOCKS5: Connection refused".to_string()),
        0x06 => WebProxyError::Proxy("SOCKS5: TTL expired".to_string()),
        0x07 => WebProxyError::Proxy("SOCKS5: Command not supported".to_string()),
        0x08 => WebProxyError::Proxy("SOCKS5: Address type not supported".to_string()),
        _ => WebProxyError::Proxy(format!("SOCKS5: Unknown error 0x{:02X}", code)),
    }
}
