...
impl HttpConnectProxy {
    ...
    pub async fn connect(&self, target: SocketAddr) -> Result<TcpStream> {
        let mut stream = timeout(self.timeout, TcpStream::connect(self.proxy_addr))
            .await
            .map_err(|e| WebProxyError::Proxy(format!("Connection timeout: {}", e)))?
    ...
    fn parse_response(&self, response: &str) -> Result<()> {
        let first_line = response
            .lines()
            .next()
            .ok_or_else(|| WebProxyError::Proxy("Empty response from proxy".to_string()))?;
        ...
        let status_code: u16 = parts[1].parse().map_err(|e| {
            WebProxyError::Proxy(format!("Invalid status code: {}: {}", parts[1], e))
        })?;
