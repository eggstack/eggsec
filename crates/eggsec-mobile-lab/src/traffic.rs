...
"#;
        ...
        assert!(s.findings.iter().any(|f| f.category == "traffic-cleartext"
            && f.severity == eggsec_core::types::Severity::Medium
    ...
    fn proxy_simulation_path_via_synthetic_traffic_and_bridge_roundtrips() {
        ...
        let mut r = crate::dynamic::DynamicMobileReport::new("app.apk");
        r.traffic_summary = Some(sum.clone());
        let pretty = crate::dynamic::format_dynamic_report(&r);
        ...
        let data = crate::dynamic::to_scan_report_data_dynamic(&r);
        assert!(data
            .findings
            .iter()
        ...
        let back: crate::dynamic::DynamicMobileReport = serde_json::from_str(&j).unwrap();
