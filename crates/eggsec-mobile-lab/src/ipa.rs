...
fn read_entry_safe<R: Read + Seek>(archive: &mut ZipArchive<R>, name: &str) -> Result<Vec<u8>> {
    ...
    let mut zf = archive.by_name(name).map_err(|e| {
        MobileError::Internal(format!("failed to open zip entry '{}': {}", name, e))
    })?;
    let sz = zf.size();
    if sz > MAX_FILE_READ {
        return Err(MobileError::Validation(format!(
    ...
    zf.read_to_end(&mut buf)
        .map_err(|e| MobileError::Internal(format!("read failed for '{}': {}", name, e)))?;
    Ok(buf)
}
...
pub async fn analyze_ipa(path: &Path) -> Result<MobileScanReport> {
    ...
    let file = std::fs::File::open(path).map_err(|e| {
        MobileError::Internal(format!("failed to open IPA '{}': {}", path.display(), e))
    })?;
    let mut archive = ZipArchive::new(file)
        .map_err(|e| MobileError::Internal(format!("invalid or corrupt IPA ZIP: {}", e)))?;
    ...
    let info: InfoPlist = from_bytes(&plist_bytes).map_err(|e| {
        MobileError::Internal(format!("failed to parse Info.plist as plist: {}", e))
    })?;
