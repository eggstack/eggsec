...
use crate::{MobileFinding, MobilePlatform};
...
/// Phase 4c (partial): adds top-level "bundle_manifest" with version/contents counts for better report integration.
pub fn export_evidence_bundle(
    report: &DynamicMobileReport,
    traffic: Option<&crate::TrafficSummary>,
    bundle_path: &str,
) -> crate::Result<String> {
    use std::io::Write;
    ...
    payload["bundle_manifest"] = serde_json::json!({
        "version": "1",
        "contents": contents,
        "frida_structured_count": frida_structured_count,
        "regression_notes_count": regression_notes_count,
        "findings_count": report.findings.len(),
        "actions_count": report.actions_performed.len(),
    });
    let bytes = serde_json::to_vec_pretty(&payload)?;
    let file = std::fs::File::create(bundle_path).map_err(|e| {
        crate::MobileError::Validation(format!("bundle create {}: {}", bundle_path, e))
    })?;
    let mut enc = flate2::write::GzEncoder::new(file, flate2::Compression::default());
    enc.write_all(&bytes)
        .map_err(|e| crate::MobileError::Validation(format!("bundle write: {}", e)))?;
    enc.finish()
        .map_err(|e| crate::MobileError::Validation(format!("bundle finish: {}", e)))?;
    Ok(bundle_path.to_string())
}

/// Phase 4c (partial, additive): thin pure convenience for baseline → compare → optional correlate.
/// Dry-run safe, no side effects, no device/network. Reads a prior MobileBaseline JSON,
/// runs compare_to_baseline (now with category delta), and if a static baseline report is supplied
/// also runs correlate_reports to produce a CorrelationResult (timeline + scores).
/// Returns (regression_notes_vec, optional CorrelationResult).
/// Exposed for scriptable workflows (users can also call the primitives directly).
pub fn run_baseline_compare_workflow(
    baseline_path: &str,
    current: &DynamicMobileReport,
    static_baseline: Option<&super::MobileScanReport>,
) -> (Vec<String>, Option<CorrelationResult>) {
    let mut notes: Vec<String> = Vec::new();
    let mut corr: Option<CorrelationResult> = None;
    match std::fs::read_to_string(baseline_path) {
        Ok(content) => {
            if let Ok(bl) = serde_json::from_str::<MobileBaseline>(&content) {
                notes = compare_to_baseline(current, &bl);
                if let Some(static_r) = static_baseline {
                    // Use the high-level engine entrypoint for full result (timeline + summary)
                    let engine = CorrelationEngine::new();
                    let res = engine.correlate(static_r, current);
                    corr = Some(res);
                }
            } else {
                notes.push(format!(
...
impl LabManifest {
    ...
    pub fn load(path: &Path) -> Result<Self> {
        ...
        toml::from_str(&content)
            .map_err(|e| MobileError::Validation(format!("invalid lab manifest TOML: {}", e)))
    }
}
...
pub struct DynamicMobileReport {
    ...
    pub traffic_summary: Option<crate::TrafficSummary>,
    ...
    pub frida_instrumentation: Option<crate::FridaInstrumentation>,
}
...
pub async fn run_dynamic_cli(args: DynamicMobileArgs) -> Result<()> {
    ...
    if args.list_devices {
        let devs = crate::adb::AdbClient::list_devices().await?;
        if args.json {
    ...
    let mut traffic_sum_for_report: Option<crate::TrafficSummary> = None;
    let mut perm_state_for_report: Option<String> = None;
    let mut frida_instr_for_report: Option<crate::FridaInstrumentation> = None;
    ...
    if args.dry_run {
        ...
        if let Some(ref tc) = args.traffic_capture {
            ...
            let mut s = crate::TrafficSummary::new();
            s.total_requests = 1;
        ...
        if !all_frida_specs.is_empty() {
            ...
            if all_frida_specs.len() == 1 {
                ...
                frida_instr_for_report = Some(crate::FridaInstrumentation {
                    note: "dry-run simulation of Frida connect + script execution (Phase 3b/3c compat)".to_string(),
                    sessions: vec![crate::FridaSession { device_id: args.device.clone().unwrap_or_else(|| "dry-sim".into()), is_simulation: true }],
                    enabled_builtins: vec!["basic_method_trace (sim)".to_string(), "crypto-keystore (sim)".to_string(), "api-trace (sim)".to_string()],
                    script_results: vec![crate::FridaScriptResult {
                ...
                let mut script_results: Vec<crate::FridaScriptResult> = vec![];
                ...
                for spec in &all_frida_specs {
                    ...
                    script_results.push(crate::FridaScriptResult {
                    ...
                    if spec.contains("crypto") || spec.contains("crypto-keystore") {
                        ...
                        structured_results.push(serde_json::json!({"type":"frida-native-load","method":"spec","lib":"[sim]","ts":0}));
                    }
                }
                frida_instr_for_report = Some(crate::FridaInstrumentation {
        ...
        let _conn = crate::adb::AdbClient::connect(device).await.map_err(|e| {
            MobileError::Validation(format!("adb connect to {} failed: {}", device, e))
        })?;
        ...
        if args.install {
            let data = tokio::fs::read(&target).await.map_err(|e| {
                MobileError::Validation(format!("failed to read apk for install: {}", e))
            })?;
            let mut conn_i = crate::adb::AdbClient::connect(device).await.map_err(|e| {
                MobileError::Validation(format!("adb connect for install failed: {}", e))
            })?;
            let install_out = conn_i
                .install_apk(&data)
                .await
                .map_err(|e| MobileError::Validation(format!("install failed: {}", e)))?;
            actions.push(format!("install: {}", install_out.trim()));
        }

        if let Some(ref activity) = args.launch {
            let mut conn_l = crate::adb::AdbClient::connect(device)
                .await
                .map_err(|e| MobileError::Validation(e.to_string()))?;
            conn_l
                .launch_app(&package, Some(activity))
                .await
                .map_err(|e| MobileError::Validation(format!("launch failed: {}", e)))?;
            actions.push(format!("launched {}", activity));
        }

        if args.capture_logs {
            let dur = std::time::Duration::from_secs(args.duration.unwrap_or(30));
            let mut conn_c = crate::adb::AdbClient::connect(device)
                .await
                .map_err(|e| MobileError::Validation(e.to_string()))?;
            let captured_logs = conn_c
                .capture_logcat(dur, Some(&package))
                .await
                .map_err(|e| MobileError::Validation(format!("logcat capture failed: {}", e)))?;
            actions.push(format!(
                "captured {} bytes of logcat ({}s)",
                captured_logs.len(),
                dur.as_secs()
            ));
            let parsed = crate::runtime::parse_logcat_findings(&captured_logs);
        ...
        if args.list_permissions
            ...
            || !args.revoke_permissions.is_empty()
        {
            let mut conn_p = crate::adb::AdbClient::connect(device)
                .await
                .map_err(|e| MobileError::Validation(e.to_string()))?;
            if args.list_permissions {
        ...
        if let Some(ref cap_path) = args.traffic_capture {
            match tokio::fs::read_to_string(cap_path).await {
                Ok(content) => {
                    let sum = crate::parse_traffic_capture(&content);
                    actions.push(format!(
        ...
        if args.uninstall_after {
            let mut conn_u = crate::adb::AdbClient::connect(device)
                .await
                .map_err(|e| MobileError::Validation(e.to_string()))?;
        ...
        if args.reset_proxy {
            if let Ok(mut conn_rs) = crate::adb::AdbClient::connect(device).await {
        ...
        if !all_specs.is_empty() {
            ...
            match crate::frida::connect(device) {
                Ok(sess) => {
                    ...
                    for spec in &all_specs {
                        ...
                        match crate::frida::run_frida_spec(&sess, spec, &package) {
                            Ok(mut res) => {
                                res.output = crate::frida::redact_frida_evidence(&res.output);
                                ...
                                for fstr in &res.findings {
                                    ...
                                    let ev = Some(crate::frida::redact_frida_evidence(
                    ...
                    let fi = crate::FridaInstrumentation {
                        note: format!(
...
pub fn correlate_findings(
    ...
    for df in dynamic_findings.iter_mut() {
        ...
        if dcat == "frida-native-load" {
            ...
            notes.push(CorrelatedFinding {
                dynamic_category: dcat.to_string(),
                static_category: "native|manifest".to_string(),
                note,
                score: Some(40),
                correlation_type: Some(CorrelationType::CrossLayer),
                enrichment: Some("frida native-load + static surface".into()),
            });
        }
    }
...
"#;
        let fs = crate::runtime::parse_logcat_findings(synthetic);
        assert!(fs
            .iter()
    ...
    async fn dry_run_with_phase2_proxy_reset_grant_list_traffic_populates_actions_and_carriers() {
        ...
        let res = run_dynamic_cli(args).await;
        assert!(
            res.is_ok(),
        ...
        let mut ts = crate::TrafficSummary::new();
        ts.total_requests = 1;
    ...
    fn dry_run_frida_flags_populate_actions_findings_and_carrier() {
        ...
        if let Some(ref fs) = frida_script {
            ...
            let mut fi = crate::FridaInstrumentation::default();
            fi.note =
                "dry-run simulation of Frida connect + script execution (Phase 3b)".to_string();
            fi.sessions.push(crate::FridaSession {
            ...
            fi.script_results.push(crate::FridaScriptResult {
    ...
    fn redaction_applied_to_frida_evidence_in_findings() {
        ...
        if let Some(ref mut e) = f.evidence {
            *e = crate::frida::redact_frida_evidence(e);
        }
    ...
    fn build_recommendations_mentions_correlation_when_frida_present() {
        ...
        r.frida_instrumentation = Some(crate::FridaInstrumentation::default());
        r.traffic_summary = Some(crate::TrafficSummary::new());
        r.findings.push(DynamicMobileFinding {
            category: "frida-api-trace".into(),
            severity: Severity::Low,
            title: "t".into(),
    ...
    // Phase 4c partial delivery tests: native-load builtin surface, workflow helper, bundle manifest, richer regression categories
    #[test]
    fn phase4c_native_load_builtin_and_workflow_and_bundle_manifest_and_regression_categories() {
        // 1. native-load builtin dry path category selection
        let mut args = DynamicMobileArgs::default();
        args.dry_run = true;
        ...
        r.frida_instrumentation = Some(crate::FridaInstrumentation {
            note: "x".into(),
            structured_results: vec![serde_json::json!({})],
            regression_notes: vec!["r".into()],
            ..Default::default()
        });
        let ts = crate::TrafficSummary::new();
        let tmp = std::env::temp_dir().join(format!("eggsec_4c_bm_{}.json.gz", std::process::id()));
        ...
        let mut fi2 = crate::FridaInstrumentation::default();
        fi2.script_results.push(crate::FridaScriptResult {
