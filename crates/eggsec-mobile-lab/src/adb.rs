...
impl AdbStreamIds {
    fn write(self, data: Vec<u8>) -> AdbMessage {
        AdbMessage::new(ADB_WRTE, self.local, self.remote, data)
    }

    fn ack(self) -> AdbMessage {
        AdbMessage::new(ADB_OKAY, self.local, self.remote, vec![])
    }

    fn close(self) -> AdbMessage {
        AdbMessage::new(ADB_CLSE, self.local, self.remote, vec![])
    }
}
...
impl AdbMessage {
    ...
    async fn read_from<R: AsyncReadExt + Unpin>(r: &mut R) -> Result<Self> {
        ...
        r.read_exact(&mut header)
            .await
            .context("failed to read adb header")?;
        ...
        let data_len =
            u32::from_le_bytes(header[12..16].try_into().expect("header slice is 4 bytes"))
                as usize;
        ...
        if data_len > ADB_MAX_PAYLOAD as usize {
            return Err(anyhow!(
                "adb data_len {} exceeds max payload {}",
                data_len,
                ADB_MAX_PAYLOAD
            ));
        }
        ...
        if data_len > 0 {
            r.read_exact(&mut data)
                .await
                .context("failed to read adb payload")?;
        }
        Ok(Self {
            command,
            arg0,
...
fn resolve_device_addr(spec: &str) -> Result<SocketAddr> {
    ...
    let port: u16 = spec.parse().context("invalid port for adb")?;
...
fn parse_sync_status(data: &[u8]) -> SyncStatus {
    if data.starts_with(b"OKAY") {
    ...
    let Some(len_bytes) = data.get(4..8).and_then(|bytes| bytes.try_into().ok()) else {
        return SyncStatus::Fail("unknown".to_string());
    };
    let reason_len = u32::from_le_bytes(len_bytes) as usize;
    let Some(reason) = data.get(8..8 + reason_len) else {
        return SyncStatus::Fail("unknown".to_string());
    };

    SyncStatus::Fail(String::from_utf8_lossy(reason).to_string())
}
...
impl AdbClient {
    ...
    pub async fn list_devices() -> Result<Vec<String>> {
        ...
        for port in (5554u16..=5584).step_by(2) {
            ...
            {
                let cnxn =
                    AdbMessage::new(ADB_CNXN, ADB_VERSION, ADB_MAX_PAYLOAD, b"host::\0".to_vec());
...
impl AdbConnection {
    ...
    async fn open_service(&mut self, name: &str) -> Result<AdbStreamIds> {
        if name.contains('\0') {
            bail!("adb service name contains NUL byte");
        }
    ...
    async fn close_service(&mut self, ids: AdbStreamIds) -> Result<()> {
        if let Err(e) = ids.close().write_to(&mut self.stream).await {
    ...
    pub async fn shell_exec(&mut self, command: &str) -> Result<String> {
        ...
        let ids = self.open_service(&service).await?;
    ...
    pub async fn sync_push(&mut self, data: &[u8], remote_path: &str) -> Result<()> {
        let ids = self.open_service("sync:").await?;
        // SEND
        ...
        ids.write(send).write_to(&mut self.stream).await?;
        ...
        while off < data.len() {
            ...
            ids.write(dbuf).write_to(&mut self.stream).await?;
            off = end;
        }
        ...
        ids.write(done).write_to(&mut self.stream).await?;
    ...
    pub async fn launch_app(&mut self, package: &str, activity: Option<&str>) -> Result<()> {
        let cmd = match activity {
            Some(a) => {
                let component = if a.starts_with('.') {
                    format!("{}/{}", package, a)
                } else {
                    a.to_string()
                };
                format!("am start -n {}", shell_quote(&component))
            }
            None => format!(
    ...
    pub async fn uninstall(&mut self, package: &str, keep_data: bool) -> Result<()> {
        let cmd = if keep_data {
            format!("pm uninstall -k {}", shell_quote(package))
        } else {
            format!("pm uninstall {}", shell_quote(package))
        };
        let out = self.shell_exec(&cmd).await?;
    ...
    pub async fn dumpsys_package(&mut self, package: &str) -> Result<String> {
        self.shell_exec(&format!("dumpsys package {}", shell_quote(package)))
            .await
    }
    ...
    pub async fn list_permissions(&mut self, package: &str) -> Result<String> {
        ...
        let mut lines: Vec<&str> = out
            ...
            .filter(|l| {
                ...
                ll.contains("permission")
                    || ll.contains("grantedpermissions")
                    || ll.contains("requestedpermissions")
            })
            .collect();
        if lines.len() > 50 {
            lines.truncate(50);
...
mod tests {
    ...
    fn resolve_device_addr_mappings() {
        ...
        assert_eq!(resolve_device_addr("127.0.0.1:5555").unwrap().port(), 5555);
        assert_eq!(
    ...
    #[test]
    fn shell_quote_wraps_and_escapes_shell_arguments() {
        assert_eq!(shell_quote("com.example.app"), "'com.example.app'");
        assert_eq!(shell_quote(""), "''");
        assert_eq!(
            shell_quote("com.example'; rm -rf /"),
            "'com.example'\\''; rm -rf /'"
        );
    }

    #[test]
    fn adb_stream_ids_use_client_local_then_peer_remote_order() {
        let ids = AdbStreamIds {
            local: 7,
            remote: 99,
        };

        let wrte = ids.write(b"payload".to_vec());
        assert_eq!(wrte.command, ADB_WRTE);
        assert_eq!(wrte.arg0, 7);
        assert_eq!(wrte.arg1, 99);

        let okay = ids.ack();
        assert_eq!(okay.command, ADB_OKAY);
        assert_eq!(okay.arg0, 7);
        assert_eq!(okay.arg1, 99);

        let close = ids.close();
        assert_eq!(close.command, ADB_CLSE);
        assert_eq!(close.arg0, 7);
        assert_eq!(close.arg1, 99);
    }

    #[test]
    fn parse_sync_status_handles_ok_fail_and_malformed_frames() {
        assert_eq!(parse_sync_status(b"OKAY"), SyncStatus::Ok);
        assert_eq!(parse_sync_status(b"NOPE"), SyncStatus::Unknown);

        let mut fail = b"FAIL".to_vec();
        fail.extend_from_slice(&6u32.to_le_bytes());
        fail.extend_from_slice(b"denied");
        assert_eq!(
            parse_sync_status(&fail),
            SyncStatus::Fail("denied".to_string())
        );

        assert_eq!(
            parse_sync_status(b"FAIL"),
            SyncStatus::Fail("unknown".to_string())
        );

        let mut truncated = b"FAIL".to_vec();
        truncated.extend_from_slice(&16u32.to_le_bytes());
        truncated.extend_from_slice(b"short");
        assert_eq!(
            parse_sync_status(&truncated),
            SyncStatus::Fail("unknown".to_string())
        );
    }
    ...
    async fn sync_push_and_shell_framing_smoke() {
        ...
        ids.write(send).write_to(&mut c).await.unwrap();

        let mut data = b"DATA".to_vec();
        data.extend_from_slice(&5u32.to_le_bytes());
        data.extend_from_slice(b"hello");
        ids.write(data).write_to(&mut c).await.unwrap();

        let mut done = b"DONE".to_vec();
        done.extend_from_slice(&0u32.to_le_bytes());
        ids.write(done).write_to(&mut c).await.unwrap();
    ...
    async fn set_clear_get_global_proxy_framing_and_shell_command_strings() {
        ...
        let open_clear = AdbMessage::new(
            ADB_OPEN,
            101,
            0,
            format!("{}\0", clear_service).into_bytes(),
        );
    ...
    async fn grant_revoke_list_permissions_shell_command_framing() {
        ...
        let open_grant = AdbMessage::new(
            ADB_OPEN,
            200,
            0,
            format!("shell:{}\0", grant_cmd).into_bytes(),
        );
        open_grant.write_to(&mut c).await.unwrap();

        // revoke_permission
        let revoke_cmd = format!("pm revoke {} android.permission.READ_SMS", pkg);
        let open_revoke = AdbMessage::new(
            ADB_OPEN,
            201,
            0,
            format!("shell:{}\0", revoke_cmd).into_bytes(),
        );
