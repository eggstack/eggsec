//! Property-based tests for Slapper.
//!
//! Uses proptest to test invariants across arbitrary inputs.

use proptest::prelude::*;
use slapper::config::ScopeRule;
use slapper::fuzzer::{generate_mutations, ReDosDetector};
...
proptest! {
    ...
    fn test_scope_rule_exact_match(domain in "[a-z]{1,20}\\.(com|org|net)") {
        ...
        let target = slapper::config::TargetScope::parse(&domain);
    ...
    fn test_scope_wildcard_matches_subdomain(
        ...
        let target = slapper::config::TargetScope::parse(&full_domain);
        if let Ok(target) = target {
            prop_assert!(rule.matches(&target));
        }
    }
...
proptest! {
    ...
    fn test_error_is_timeout_detection(
        ...
        let err = slapper::SlapperError::Timeout {
            timeout_ms,
    ...
    fn test_error_http_status_preserves_code(status in 400u16..600) {
        let err = slapper::SlapperError::HttpStatus {
            status,
