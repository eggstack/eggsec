...
/// Create a mock endpoint with rate limiting headers.
pub fn mock_rate_limited(path: &str) -> Mock {
    Mock::given(method("GET"))
        .and(path_matcher(path))
        .respond_with(
...
/// Create a mock that returns HTML with security headers.
pub fn mock_secure_headers(path: &str) -> Mock {
    Mock::given(method("GET"))
        .and(path_matcher(path))
        .respond_with(
            ResponseTemplate::new(200)
...
/// Create a mock that returns a JWT token.
pub fn mock_jwt_response(path: &str) -> Mock {
    Mock::given(method("POST"))
        .and(path_matcher(path))
        .respond_with(
            ResponseTemplate::new(200)
                .set_body_string(r#"{"token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIn0.dozjgNryP4J3jVmNHl0w5N_XgL0n3I9PlFUP0THsR8U"}"#)
                .insert_header("content-type", "application/json"),
        )
}
