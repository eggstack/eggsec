...
fn validate_target(target: &str) -> Result<(), SlapperError> {
    if target.is_empty() {
        return Err(SlapperError::Config("Target cannot be empty".to_string()));
    }
...
fn validate_payload_size(params: &Option<serde_json::Value>) -> Result<(), SlapperError> {
