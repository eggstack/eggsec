...
impl TemplateMarketplace {
    ...
    pub async fn list_templates(
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| SlapperError::Network(format!("Failed to fetch templates: {}", e)))?;
    ...
    fn save_to_cache(&self, template_id: &str, content: &str) -> Result<()> {
        ...
        std::fs::write(&cache_path, content)
            .map_err(|e| SlapperError::Config(format!("Failed to write cache file: {}", e)))?;
    ...
    pub fn get_cached_template(&self, template_id: &str) -> Result<Option<VulnerabilityTemplate>> {
        ...
        let content = std::fs::read_to_string(&cache_path)
            .map_err(|e| SlapperError::Config(format!("Failed to read cache file: {}", e)))?;
    ...
    pub fn clear_cache(&self) -> Result<()> {
        if self.local_cache.exists() {
            std::fs::remove_dir_all(&self.local_cache)
                .map_err(|e| SlapperError::Config(format!("Failed to clear cache: {}", e)))?;
        }
        Ok(())
    }
...
mod tests {
    ...
    fn test_default_marketplace() {
        ...
        assert_eq!(marketplace.base_url, "https://templates.slapper.io");
    }
}
