...
pub async fn scan_drupal(target: &CmsTarget, client: &Client) -> Result<CmsScanResult, crate::error::SlapperError> {
    ...
    if let Some(ref ver) = version {
        for (cve, title, severity, desc, fixed) in DRUPAL_VULNERABILITIES {
        ...
        Ok(resp) => {
            if resp.status().as_u16() == 200 {
                misconfigurations.push(CmsMisconfiguration {
                    id: "DR001".to_string(),
                    title: "Default Admin Login Page Accessible".to_string(),
                    severity: Severity::Low,
                    description: "The Drupal user login page is accessible".to_string(),
                    recommendation: "Consider implementing rate limiting on login pages".to_string(),
                });
            }
        }
        Err(_) => {}
