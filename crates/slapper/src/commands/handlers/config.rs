...
pub async fn handle_config(_ctx: &CommandContext, args: ConfigArgs) -> Result<()> {
    match args.command {
        ...
        ConfigCommand::Show(show_args) => {
            ...
            let config = load_config(config_path).map_err(|e| anyhow::anyhow!("Failed to load configuration: {}", e))?;
            ...
            println!();
            let toml_str = toml::to_string_pretty(&config).map_err(|e| anyhow::anyhow!("Failed to serialize config: {}", e))?;
            println!("{}", toml_str);
            Ok(())
        }
    }
