...
Slapper supports race condition testing using concurrent request attacks. TOCTOU (Time-of-Check-Time-of-Use) vulnerabilities occur when there's a gap between when a system checks a condition and when it uses the result.
...
slapper fuzzer --target http://target.com/api --payload-type race

# With high concurrency
slapper fuzzer --target http://target.com/api --payload-type race --concurrency 50

# Token reuse scenario
slapper fuzzer --target http://target.com/api --payload-type race --payload-file race_tokens.txt
```
