...
XSS testing finds vulnerabilities where attacker-controlled data is reflected in web pages without proper encoding. Slapper tests for reflected, stored, and DOM-based XSS.
...
```bash
slapper fuzz --target https://example.com/search?q=test --type xss
```

### Test Multiple Parameters

```bash
slapper fuzz --target https://example.com/ --type xss --all-params
```

### Stored XSS (Form Fields)

```bash
slapper fuzz --target https://example.com/comment --type xss --method post
```

### DOM XSS Testing

```bash
slapper fuzz --target https://example.com/#test --type xss --dom
```
