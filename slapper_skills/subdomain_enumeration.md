...
```bash
slapper recon subdomains --target example.com --wordlist /path/to/subdomains.txt
```

### Using Certificate Logs

```bash
slapper recon subdomains --target example.com --crt-search
```

### Permutation Mode

```bash
slapper recon subdomains --target example.com --permutate
```

### Find Virtual Hosts

```bash
slapper recon subdomains --target 192.168.1.1 --vhost
```
...
```toml
[recon.subdomains]
wordlist = "~/.config/slapper/wordlists/subdomains.txt"
concurrent = 100
timeout = 5
```
