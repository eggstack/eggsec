...
```bash
# Start OAST listener
slapper oast --server interactsh.com --port 443

# Test for blind SSRF
slapper fuzz --target http://example.com --oast --callback-url http://attacker.com

# Test for blind XSS
slapper fuzz --target http://example.com --oast --blind-xss
```

### API Usage

```rust
use slapper::tool::implementations::oast::{OastTool, OastConfig};
