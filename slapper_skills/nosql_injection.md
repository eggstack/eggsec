...
Slapper supports NoSQL injection testing for MongoDB, Redis, CouchDB, and Elasticsearch databases. NoSQL injection exploits vulnerabilities in applications that build queries using user input without proper sanitization.
...
slapper fuzzer --target http://target.com/api --payload-type nosql

# With custom wordlist
slapper fuzzer --target http://target.com/api --payload-type nosql --wordlist custom_nosql.txt

# Enable detection only (no exploitation)
slapper fuzzer --target http://target.com/api --payload-type nosql --detect-only
```
