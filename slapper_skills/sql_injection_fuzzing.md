...
SQL injection testing discovers vulnerabilities where user input is incorporated into SQL queries without proper sanitization. Slapper tests for error-based, UNION-based, blind boolean, and time-based SQL injection.
...
```bash
slapper fuzz --target https://example.com/api/user?id=1 --type sqli
```

### Test All Parameters

```bash
slapper fuzz --target https://example.com/search --type sqli --all-params
```

### With Custom Payloads

```bash
slapper fuzz --target https://example.com/api --type sqli --payloads /path/to/payloads.txt
```

### Blind Boolean Testing

```bash
slapper fuzz --target https://example.com/profile?id=1 --type sqli --blind
```
...
1. Always detect WAF before testing (use `slapper waf detect`)
