...
Slapper's TUI provides clipboard functionality using the `arboard` crate (pure Rust clipboard access). This enables copy/paste operations in the terminal UI.

## Usage

```rust
use slapper::tui::utils::clipboard::Clipboard;
...
- `crates/slapper/src/tui/utils/clipboard.rs` - Clipboard utility
...
```bash
cargo test --lib -p slapper -- clipboard
```