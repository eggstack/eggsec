...
- Performance work: evidence lives in `architecture/performance.md` (loopback-only harness `scripts/perf-profile.sh` + ignored profile targets; medians/ranges, never single samples; timing is informational, never a merge gate). Keep high-cardinality fan-out bounded by configured concurrency (JoinSet admission, guards Checks 140–142): fuzzer/scanner/subdomain paths must not return to spawn-per-payload retention; `WorkerConfig::max_concurrency` stays a real capacity contract via the shared `CapacityTracker`; the load-test executor dispatches prototype clones compiled once per run (no per-request DTO rebuild, no policy-result caching); the worker multiplexes control traffic over one `CoordinatorSession` per healthy connection lifetime (heartbeat retries once; task acquisition/result errors surface, never speculative replay).
