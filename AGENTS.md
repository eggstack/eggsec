...
- **MCP Enforcement**: MCP tools/call handler evaluates `self.enforcement.evaluate()` BEFORE dispatch. Preferred production constructor: `McpServer::with_enforcement`. Legacy `policy_decision_for_mcp_call` (and direct `evaluate_operation_policy`) deprecated for denial paths; prefer `policy_decision_for_mcp_call_with_enforcement` + `EnforcementContext`.
...
- **MCP Policy Helpers**: `classify_tool_risk()` and `infer_tool_category()` in MCP policy infer tool metadata from tool IDs; `policy_decision_for_mcp_call_with_enforcement` (via `EnforcementContext`) builds a `PolicyDecision` for MCP tool invocations; legacy `policy_decision_for_mcp_call` deprecated.
- **MCP Policy Denials**: The `tools/call` handler in `tool/protocol/mcp/handlers/server.rs` computes full `PolicyDecision` via `policy_decision_for_mcp_call_with_enforcement` (or legacy) and embeds it in the MCP error response `data` field.
...
- **MCP Strict Enforcement**: MCP server now forces `ExecutionProfile::McpStrict` and requires explicit scope manifest for all networked tool calls. Shared `EnforcementContext` is used instead of parallel policy paths (2026-06-10). Production MCP startup uses `McpServer::with_enforcement`.
