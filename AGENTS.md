...
| `proxy/intercept/` | `crates/eggsec/src/proxy/AGENTS.override.md` (web proxy types, bridge, TUI, manipulation audit trail) |
...
- `web-proxy` - Standalone defense-lab interactive web proxy for HTTP/HTTPS traffic interception. Phase 1: MITM server + CA + CLI + dry-run + policy + bridge. Phase 2: Interactive TUI tab (Tab::Intercept) with live flow inspection, header/body editing, forward/drop/replay actions, intercept rules, session save/load, HAR export, and full manipulation audit trail. Requires `--allow-web-proxy` + policy for real interception. No MCP/agent exposure. `web-proxy = []` marker feature.
