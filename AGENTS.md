...
Python bindings (`eggsec-python`): Build with `maturin develop` from `crates/eggsec-python/`. Default wheel includes core binding, scanner, endpoint discovery, service fingerprinting, recon, WAF detection, reporting, and policy/configuration/execution context (Milestone B). Type stubs included. Release 1/2 closure pass completed 2026-07-14 (1977 passed, 89 skipped). Release 4 closure pass completed. Release 1-4 operational correction pass completed 2026-07-15 (2969 passed, 0 failed; NSE surface hardened, daemon DTOs completed, stub parity closed, flaky test fixed). Shared async runtime (`OnceLock<Runtime>`) fixes chained-awaits; all async lifecycle tests pass without skip markers.
...
- **Release 5 Phase E**: Packaging, documentation, and executable examples — corrected package metadata (repository URLs, author, classifiers), machine-readable wheel profile manifest (`wheel-profiles.json`), enhanced `build_info()` with wheel profile detection, compiled features, Python version, and ABI/schema/protocol versions; new `eggsec.wheel_profile()` function; workflow-oriented Python docs landing page; 29 deterministic executable examples covering all stable operations, networking, events, artifacts, reporting, feature detection, and experimental namespaces; sdist build in CI; multi-version (Python 3.9–3.13) test matrix; wheel metadata recording (SHA-256, size, tags); documentation example test harness (`scripts/test_documentation_examples.py`); auto-generated API reference from operation registry and stubs (`scripts/generate_api_reference.py`); tag-driven release workflow with version validation, evidence bundles, and GitHub release notes.
