...
# db-pentest feature (Phase 1: postgres/mysql + manifest + bridge)
...
| `db_pentest/` | `crates/eggsec/src/db_pentest/AGENTS.override.md` (Phase 1 foundation + postgres/mysql + manifest + bridge) |
...
- `db-pentest` - Standalone defense-lab direct Postgres/MySQL security assessment (Phase 1 foundation + postgres/mysql + manifest + bridge; requires `--allow-db-pentest` for non-dry runs; dry-run always safe; local `DbPentestReport`/`DbFinding` + optional `to_scan_report_data_db` bridge; auto-bridged in report convert; no TUI/pipeline/MCP in Phase 1). See `plans/database-pentesting-phase1-foundation-handoff-plan.md` (executed) and `plans/non-web-database-pentesting-loadout-design-plan.md`.
...
- `DbPentestReport` / `DbFinding` / `LabDbManifest` / `DbTarget` / `CheckType` / `to_scan_report_data_db` - Database pentesting types (Phase 1: Postgres + MySQL checks + manifest + bridge; public under `db-pentest` feature; `DbPentestReport` provides `to_scan_report_data_db` bridge to unified reports). See `plans/database-pentesting-phase1-foundation-handoff-plan.md` (executed) and `plans/non-web-database-pentesting-loadout-design-plan.md`.
