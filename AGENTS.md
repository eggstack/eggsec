...
- `make clippy` covers engine lib (empty default + `cli`) + leaf crates (`eggsec-core`, `eggsec-tool-core`, `eggsec-report-model`, `eggsec-output`, `eggsec-runtime`, `eggsec-ui-model`, `eggsec-agent`, `eggsec-transport`, `eggsec-transport-eggfetch`). Domain/platform lint is `make clippy-domain` (deep checks only).
...
19 crates. Engine `eggsec` is lib-only (no binary); binary shell is `eggsec-cli`; CLI handlers live in-engine at `crates/eggsec/src/commands/handlers/` (`crates/eggsec/src/cli/` holds command types only; `crates/eggsec-cli/src/` is just main/daemon-client/logging).
...
- Dependency boundaries (guard-enforced): `eggsec-runtime` stays light (serde/serde_json, thiserror, tokio, tokio-util, tracing, uuid); `eggsec-report-model` is data-only (serde/serde_json, chrono, uuid + `eggsec-core`; no Tokio/filesystem/renderers); `eggsec-output` renders over the model (`eggsec-core` + `eggsec-report-model`, never the reverse); domain DTO consumers (`eggsec-db-lab`, `eggsec-mobile-lab`, `eggsec-web-proxy`, `eggsec-nse`) depend on the model, not the renderer; `eggsec-daemon` default deps are `eggsec-runtime` + `eggsec-daemon-protocol` only (engine behind `full-executor`, transport behind `http-api`, no TUI deps); `eggsec-transport` stays light (`bytes`/`http`/`url`/`thiserror` only, no concrete clients).
