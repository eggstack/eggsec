...
- `architecture/mobile.md` - Mobile app static analysis (APK/IPA; Phase 1 static only, pure-Rust parsers, lab/defense framing; standalone CLI + local reports + to_scan_report_data bridge). Dynamic Phase 1 complete 2026-06-12; Phase 2a (proxy Level-1 + runtime permissions) complete 2026-06-12 per `plans/mobile-dynamic-phase2-implementation-handoff-plan.md` (executed); final polish + close-out per `plans/mobile-dynamic-phase2-final-polish-handoff-plan.md` (executed) + `plans/mobile-dynamic-phase2-close-out-polish-plan.md` (executed). See `plans/dynamic-mobile-testing-loadout-design-plan.md`.
...
- `mobile-dynamic` - Mobile dynamic testing (Phase 1: Android ADB core + runtime log analysis; Phase 2a 2026-06-12: proxy Level-1 device config + traffic summary + runtime permission testing; standalone defense-lab, MCP-absent; `mobile-dynamic = ["mobile"]`). Auto-bridge in `report convert` via `to_scan_report_data_dynamic`. Phase 1 complete 2026-06-12 per `plans/mobile-dynamic-phase1-implementation-handoff-plan.md` (executed). Phase 1 polish (smoke test script `scripts/test-mobile-dynamic.sh`, `--list-devices` convenience, troubleshooting, docs + success criteria) complete 2026-06-12 per `plans/mobile-dynamic-post-phase1-polish-and-phase2-planning.md` (executed). Phase 2a per `plans/mobile-dynamic-phase2-implementation-handoff-plan.md` (executed) (final polish executed 2026-06-12 per plans/mobile-dynamic-phase2-final-polish-handoff-plan.md (executed); `correlate_findings` + `static_correlation` delivered; close-out polish executed 2026-06-12 per `plans/mobile-dynamic-phase2-close-out-polish-plan.md` (executed); final code hygiene + doc consistency for Phase 2 closed).
...
- `DynamicMobileReport` / `DynamicMobileFinding` / `LabManifest` / `run_dynamic_cli` - Dynamic mobile types + entrypoint (under `mobile-dynamic` feature; `mobile-dynamic = ["mobile"]`; Phase 1: Android ADB core + runtime log analysis; Phase 2a: + `traffic_summary`/`permission_state` + proxy/permission actions; standalone defense-lab, MCP-absent; `to_scan_report_data_dynamic` bridge to `ScanReportData` with `mobile-dynamic-*` categories; auto-bridged in `report convert`). Phase 1 complete 2026-06-12 per `plans/mobile-dynamic-phase1-implementation-handoff-plan.md` (executed). Phase 1 polish (smoke script, `--list-devices` convenience, troubleshooting, docs) complete 2026-06-12 per `plans/mobile-dynamic-post-phase1-polish-and-phase2-planning.md` (executed). Phase 2a complete 2026-06-12 (final polish: `static_correlation` populated by `correlate_findings`; close-out polish: final code hygiene + doc consistency for Phase 2 closed).
- `TrafficSummary` / `parse_traffic_capture` - Phase 2a traffic summary type + parser (public under `mobile-dynamic`; summary-only from mitmproxy-style capture; feeds `traffic_summary` in report + bridge info findings).
...
- **Mobile Static Analysis**: Standalone defense-lab CLI (`eggsec mobile <path.{apk,ipa}>`) under `mobile` feature (gated command/module, not in TUI or pipeline profiles). Handler uses `evaluate_and_enforce_operation` with `SafeActive` risk + `required_features: ["mobile"]` (local file target, no scope). Pure-Rust ZIP/AXML/plist parsers only. Produces local `Mobile*` findings + `to_scan_report_data` bridge (like wireless). Phase 1 closed 2026-06-11. Dynamic Phase 1 polish (smoke test script `scripts/test-mobile-dynamic.sh`, `--list-devices` convenience, troubleshooting, docs) complete 2026-06-12 per `plans/mobile-dynamic-post-phase1-polish-and-phase2-planning.md` (executed). Phase 2a (proxy Level-1 + permissions) complete 2026-06-12 per `plans/mobile-dynamic-phase2-implementation-handoff-plan.md` (executed) (final polish executed 2026-06-12 per plans/mobile-dynamic-phase2-final-polish-handoff-plan.md (executed); close-out polish executed 2026-06-12 per plans/mobile-dynamic-phase2-close-out-polish-plan.md (executed)). See `commands/handlers/mobile.rs`, `mobile/mod.rs`, `src/mobile/AGENTS.override.md`.
...
# Mobile-dynamic (Phase 1 + Phase 2a proxy/permissions; no hardware required for unit tests)
cargo check -p eggsec --features mobile-dynamic
cargo test --lib -p eggsec --features mobile-dynamic
