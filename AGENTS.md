...
8. **Canonical dispatch ownership (Phase 1)**: `dispatch::canonical_execution::execute_approved` (ApprovedOperation + CanonicalOperationRequest + ExecutionSink) is the single executor owner; `execute_canonical` owns the match. `dispatch_inner`/`dispatch_task` are legacy manual shims. CLI routing is single-owned via `commands::route::route_for_commands` (exhaustive, aliases resolved). Executor code must not import Clap/Ratatui/daemon-protocol/Python types. Both runtime adapters route cancellation through `eggsec-runtime::race_with_cancel` (no ad-hoc `cancel.cancelled()` select in adapter code).
9. **TUI surface convergence (Phase 2)**: `TabSpec` (`tabs/spec.rs`) is the single production surface owner (`aliases()`/`palette_command()`/`surface_route()`/`canonical_operation()`/`availability()`); the four-action pilot is deleted. Alias lookup is single-owned via `resolve_palette_command()` (static slice, no manual match); palette/keys converge via `app/palette.rs` (`parse_palette_action`/`global_action_for` -> `UiAction`); help/palette discovery derive from the same spec (`tabs/surface.rs` catalog). `copy-cli` uses `cli_argv()` (argv vector, quoting only at the boundary) and round-trips through the real Clap `Cli` plus canonical adapters — only emit flags the real tree accepts (`--json` for Json-mapped tabs, `--format` only where declared; TUI-only state omitted). `reload-scope` is not discoverable (restart-required info only). Fuzz HTTP-session flag is `--http-session` (not `--session`, which is the global daemon attach ID).
10. **Runtime contract closure (Phase 3)**: runtime types answer transport/session questions, engine contracts answer operation questions. `RuntimeSurface` is a wire DTO (serialization compat); the bridge owns the exhaustive bidirectional conversion (`runtime_surface_to_execution_surface` / `execution_surface_to_runtime_surface`, `Unknown` rejected wire → engine). `TaskKind` is a wire adapter: engine `operation_id_for_task_kind`/`target_for_task_kind` delegate to `TaskKind::operation_id()`/`canonical_target()` (no parallel tables); the canonical typed conversion is `CanonicalOperationRequest::from_task_kind`. Result/envelope conversion is single-owned via `dispatch::task_result_envelope` (stable wire kinds; TUI and daemon adapters consume it, never a parallel `TaskResult` match). Closure tests live in `crates/eggsec/tests/runtime_contract_closure.rs`.

### Runtime dispatch flow

```
TUI → TuiTaskDispatcher (shallow: TaskKind → canonical → execute_canonical) → TaskResult + envelope (via dispatch::task_result_envelope)
...
- **Runtime mapping (Phase 0.5, closed Phase 3)**: the single wire-side match is `TaskKind::operation_id()`/`canonical_target()` (29 kinds, exhaustive); engine `operation_id_for_task_kind()`/`target_for_task_kind()` delegate to it (no parallel tables — guard 94); the canonical typed conversion `CanonicalOperationRequest::from_task_kind()` agrees on identity/target/route (closure test). `RuntimeSurface` is a wire DTO; the bridge owns the bidirectional conversion (`Unknown` rejected wire → engine — guard 96).
