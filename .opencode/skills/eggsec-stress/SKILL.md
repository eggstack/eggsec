...
while the `loadtest` module handles HTTP load/performance testing. See the `eggsec-agent/http_load_testing.md` skill for HTTP load testing guidance.
...
```bash
cargo test --lib -p eggsec stress::
```
...
## Resources
- `crates/eggsec/src/stress/AGENTS.override.md` - Detailed stress patterns
