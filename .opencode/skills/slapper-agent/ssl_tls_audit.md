...
Slapper provides TestSSL-like TLS/SSL security auditing capabilities through the `ssl_audit` module. This performs comprehensive security testing of TLS configurations including certificate analysis, protocol version checking, cipher suite evaluation, and vulnerability detection.
...
slapper ssl-audit example.com --port 443

# With specific timeout
slapper ssl-audit example.com --port 443 --timeout 60

# Output in JSON format
slapper ssl-audit example.com --port 443 --format json
```

### API Usage

```rust
use slapper::recon::ssl_audit::{SslAuditor, SslGrade};
