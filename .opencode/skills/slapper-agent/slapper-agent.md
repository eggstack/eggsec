...
Slapper autonomous agent development — operational constraints, testable seams, event handling, scheduled scans, task scheduling, lifecycle management. Triggers: agent, constraint, scan, schedule, trigger_event, dispatch, alert, portfolio, memory, task, lease, lifecycle.
...
- `crates/slapper/src/tool/agents/scheduler.rs` — TaskScheduler with lease-based model
- `crates/slapper/src/tool/agents/lifecycle.rs` — LifecycleManager with health monitoring
- `crates/slapper/src/tool/protocol/agent_routes.rs` — REST API for agents and tasks
