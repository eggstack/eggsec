...
```bash
slapper auth-test --target https://example.com/login --brute-force
slapper auth-test --target https://example.com/login --wordlist /path/to/passwords.txt
```

### Credential Stuffing

```bash
slapper auth-test --target https://example.com/login --credential-file /path/to/breach.txt
```

### MFA Bypass Test

```bash
slapper auth-test --target https://example.com/login --test-mfa
```
