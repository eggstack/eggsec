...
```bash
slapper fuzz --target https://example.com/api/trace --type log-injection --param user
```

### Test User-Agent Logging

```bash
slapper fuzz --target https://example.com/api/log --type log-injection --header User-Agent
```

### Custom Log Payloads

```bash
slapper fuzz --target https://example.com/debug --payloads ./log_injection.txt
```
