...
Slapper supports mass assignment testing. Mass assignment vulnerabilities occur when applications automatically bind user input to internal object properties without proper filtering, allowing attackers to modify fields they shouldn't.
...
slapper fuzzer --target http://target.com/api/user --payload-type mass_assign

# JSON API target
slapper fuzzer --target http://target.com/api/profile --payload-type mass_assign -H "Content-Type: application/json"
```
