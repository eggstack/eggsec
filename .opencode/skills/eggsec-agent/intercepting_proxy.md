...
Eggsec provides an intercepting proxy with dynamic SSL certificate generation for security testing. This enables HTTP/HTTPS traffic interception, request/response modification, and monitoring capabilities.
...
eggsec proxy --listen 127.0.0.1:8080

# Start proxy in intercept mode
eggsec proxy --listen 127.0.0.1:8080 --mode intercept

# With custom rules
eggsec proxy --listen 127.0.0.1:8080 --rules rules.yaml
```

### API Usage

```rust
use eggsec::proxy::intercept::{ProxyServer, InterceptMode, InterceptRule, RuleAction};
...
```bash
# Export CA cert
eggsec proxy export-ca --file ca.crt

# Import to system trust store (requires root)
cp ca.crt /usr/local/share/ca-certificates/eggsec-proxy.crt
