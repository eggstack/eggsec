...
Eggsec's NSE support includes sandboxing for Lua scripts via `SandboxConfig`. The sandbox restricts filesystem access, command execution, and environment modification.
...
- `crates/eggsec-nse/src/libraries/io.rs` - io library implementation
- `crates/eggsec-nse/src/libraries/lfs.rs` - lfs library implementation
- `crates/eggsec-nse/src/libraries/socket.rs` - socket library (not sandboxed)
- `crates/eggsec-nse/src/lib.rs` - SandboxConfig definition
