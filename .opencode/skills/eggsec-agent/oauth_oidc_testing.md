...
```bash
eggsec oauth --target https://auth.example.com --test-flow authorization_code
```

### JWT Security Testing

```bash
eggsec oauth --target https://api.example.com --test-jwt
```

### PKCE Bypass Test

```bash
eggsec oauth --target https://auth.example.com --test-pkce
```
