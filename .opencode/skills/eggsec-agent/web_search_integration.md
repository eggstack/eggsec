...
```bash
eggsec search --query "example.com vulnerability 2024"
eggsec search --query "apache struts exploit" --source searxng
```

### CVE Lookup via OSV.dev

```bash
eggsec search --query "CVE-2024-1234" --source osv
eggsec search --query "RUST: GHSA-xxxx-xxxx" --source osv
```

### NVD Vulnerability Search

```bash
eggsec search --query "buffer overflow" --source nvd
eggsec search --query "CVE-2024" --source nvd
```

### Multi-Source Search

```bash
eggsec search --query "log4j" --source all
```
