...
description: "Code quality patterns, error handling, testing best practices, and documentation standards for Eggsec"
triggers:
  - code quality
  - error handling
  - From impl
...
This skill documents code quality patterns, error handling best practices, and testing conventions for working on the Eggsec security toolkit codebase.
    ...
    fn from(e: crate::ai::AiError) -> Self {
        match e {
            crate::ai::AiError::Timeout => {
                EggsecError::Timeout { timeout_ms: 0, operation: "ai-request".to_string() }
            }
            // ... other variants
        }
    }
...
```rust
impl From<ThirdPartyError> for EggsecError {
    fn from(e: ThirdPartyError) -> Self {
        match e {
            ThirdPartyError::Timeout => EggsecError::Timeout { /* ... */ },
            ThirdPartyError::AuthFailed => EggsecError::Config("Auth failed".to_string()),
            ThirdPartyError::RateLimited => EggsecError::RateLimited("rate limit".to_string()),
        }
    }
}
```
...
These helpers convert serde errors to `EggsecError::Parse` for consistent error handling.
...
///
/// ```rust
/// use eggsec::tool::response::ToolResponse;
///
...
2. **RateLimited variant**: Takes a `String` argument: `EggsecError::RateLimited("reason".to_string())`
