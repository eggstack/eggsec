...
description: Guide for migrating Rust web framework dependencies in Eggsec
triggers:
  - axum migration
  - tonic migration
  - update axum
...
This skill guides the migration of major Rust web framework dependencies in the Eggsec security toolkit. It documents the breaking changes and migration paths for Axum and Tonic.
...
# Test rebuild
cargo check --lib -p eggsec --features rest-api,grpc-api,ai-integration
...
- `crates/eggsec/src/tool/protocol/rest.rs` - `/api/v1/tools/:tool_id` → `/api/v1/tools/{tool_id}`
- `crates/eggsec/src/tool/protocol/mcp/routes.rs` - `/mcp/stream/:request_id` → `/mcp/stream/{request_id}`
...
# Clippy
cargo clippy --lib -p eggsec --features rest-api,ai-integration
```
