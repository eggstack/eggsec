...
Eggsec's TUI includes session persistence that automatically saves and restores UI state using stable string IDs for tabs and bookmarks. This enables crash recovery and preserves user preferences across sessions, including the active theme.
...
```rust
use eggsec::tui::session::{SessionManager, SessionState};
use eggsec::tui::tabs::Tab;
...
- `crates/eggsec/src/tui/session.rs` - SessionManager and SessionState
- `crates/eggsec/src/tui/app/mod.rs` - `App::bookmarks` is now `HashSet<String>`
...
```bash
cargo test --lib -p eggsec -- session
# or
...
```rust
// Test cases in crates/eggsec/src/tui/session.rs
#[test]
...
```bash
cargo test --lib -p eggsec -- test_restore_session
```
