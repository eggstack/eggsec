...
```bash
eggsec loadtest --target https://example.com --rate 1000 --duration 60
```

### Timing Profile

```bash
eggsec loadtest --target https://example.com --profile --requests 1000
```

### Rate Limit Detection

```bash
eggsec loadtest --target https://api.example.com --rate-limit-test
```
