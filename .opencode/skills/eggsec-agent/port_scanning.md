...
Port scanning reveals exposed network services and their versions, providing critical information for attack surface assessment. Eggsec supports various scan types including SYN, TCP connect, and spoofed scans.
...
```bash
eggsec scan ports --target 192.168.1.1 --ports 22,80,443,8080,8443
```

### Fast Scan (Top Ports)

```bash
eggsec scan ports --target 192.168.1.1 --top-ports 100
```
