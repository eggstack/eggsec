...
Standalone static security analysis of Android APKs and iOS IPAs (Phase 1 static complete) + dynamic Android runtime testing via ADB + logcat (Phase 1 dynamic complete 2026-06-12) + Phase 2a proxy foundation + runtime permission testing (2026-06-12) for authorized lab / defense-validation use. Pure-Rust static (ZIP + bounded AXML / plist). Dynamic: pure-Rust ADB TCP (emulator primary) + high-signal log parser; Level-1 proxy (device global http_proxy via --proxy + user-managed mitmproxy + --traffic-capture summary parser in new traffic.rs); runtime permission grant/revoke/list via adb helpers; no Frida/instrumentation in P1/2a. Produces local `MobileScanReport` / `MobileFinding` (static) and `DynamicMobileReport` / `DynamicMobileFinding` (dynamic; extended with optional traffic_summary/permission_state), both with optional bridges to `ScanReportData` (extra info findings for summary/state under mobile-dynamic-* categories). Standalone defense-lab surface (MCP/agent absent; same pattern as wireless-active + static-mobile + auth-test). Phase 1 polish (smoke test script `scripts/test-mobile-dynamic.sh`, `--list-devices` convenience, troubleshooting, docs) complete 2026-06-12 per `plans/mobile-dynamic-post-phase1-polish-and-phase2-planning.md` (executed). Phase 2a complete 2026-06-12 per `plans/mobile-dynamic-phase2-implementation-handoff-plan.md` (executed; Level 1 proxy integration + permissions; no new sub-feature, all under mobile-dynamic). Final polish (F2 correlation helper, F3 parser robustness, F5 report surface, F6 docs) executed 2026-06-12 per `plans/mobile-dynamic-phase2-final-polish-handoff-plan.md` (executed). Close-out polish (final code hygiene + doc consistency) executed 2026-06-12 per `plans/mobile-dynamic-phase2-close-out-polish-plan.md` (executed). See plans for 2b/Frida/etc.
...
| `DynamicMobileReport` | `mobile/dynamic.rs` | Full dynamic report (target, scan_type="mobile-dynamic", platform=Android, device_serial, app_id, findings, actions_performed, dry_run, duration; Phase 2a: + optional traffic_summary: Option<TrafficSummary>, permission_state: Option<PermissionState>) |
...
| `mobile/dynamic.rs` | Dynamic types (`DynamicMobileReport`/`Finding`, `LabManifest`, `DynamicMobileArgs` internal skeleton), `run_dynamic_cli`, format/bridge (`to_scan_report_data_dynamic`; Phase 2a extensions + extra info findings); `correlate_findings` (public re-export) + `CorrelatedFinding` live here |
| `mobile/adb.rs` | Pure-Rust ADB TCP framing + `AdbClient`/`AdbConnection` (list_devices, connect, shell, install, launch, uninstall, capture_logcat, set_global_proxy/clear_global_proxy, grant/revoke/list_permissions); external `adb` only for discovery convenience; Phase 2a permission/proxy helpers |
...
| `mobile/traffic.rs` | Phase 2a: `TrafficSummary` + `parse_traffic_capture` (summary-only parser for mitmproxy logs/HAR-like; domains, counts, cleartext/suspicious hints; produces high-signal findings); public under mobile-dynamic |
| `cli/mobile.rs` | `MobileArgs` + `MOBILE_ABOUT`; `MobileSubcommand` (Static/Dynamic), `MobileStaticArgs`, `DynamicMobileArgs` (CLI) |
| `commands/handlers/mobile.rs` | `handle_mobile` (subcommand dispatch; static uses StandardAssessment/SafeActive + "mobile"; dynamic uses DefenseLab/SafeActive + "mobile-dynamic" + explicit `--allow-dynamic-mobile` gate; policy + notify + map to internal DynamicMobileArgs; Phase 2a proxy/permission flags passed through) |

## Status

Phase 1 static complete (pure-Rust, SafeActive, standalone CLI + optional report bridge; closed 2026-06-11). Phase 1 dynamic (Android ADB core + high-signal runtime logcat analysis) complete 2026-06-12 per `plans/mobile-dynamic-phase1-implementation-handoff-plan.md` (executed) + parent `plans/dynamic-mobile-testing-loadout-design-plan.md`. Phase 1 polish (smoke test script, `--list-devices` convenience, troubleshooting section, updated success criteria) complete 2026-06-12 per `plans/mobile-dynamic-post-phase1-polish-and-phase2-planning.md` (executed). Phase 2a (Level 1 proxy integration + runtime permission testing) complete 2026-06-12 per `plans/mobile-dynamic-phase2-implementation-handoff-plan.md` (executed): new `traffic.rs`, adb proxy/permission helpers, `DynamicMobileReport` + `to_scan_report_data_dynamic` extensions (traffic_summary/permission_state + bridge info findings), CLI/handler mapping; still under mobile-dynamic (no new sub-flag); standalone defense-lab (MCP/agent absent). Final polish (F2 correlation helper, F3 parser robustness, F5 report surface, F6 docs) executed 2026-06-12 per plans/mobile-dynamic-phase2-final-polish-handoff-plan.md (executed). Close-out polish (final code hygiene + doc consistency) executed 2026-06-12 per `plans/mobile-dynamic-phase2-close-out-polish-plan.md` (executed).
...
Categories in bridged output are `mobile-{android,ios}-<native-category>` (static) or `mobile-dynamic-android-<category>` (dynamic) (e.g. `mobile-android-manifest`, `mobile-dynamic-android-runtime-permission`, `mobile-dynamic-android-crash-log`; Phase 2a: `mobile-dynamic-android-traffic-summary`, `mobile-dynamic-android-permission-state`, etc. via extra info findings). Evidence carries through; empty findings are valid (0 findings in bridge). Dynamic bridge mirrors static + active wireless pattern.

**Design decision (Phase 1 static close 2026-06-11; dynamic P1 2026-06-12; Phase 2a 2026-06-12)**: Standalone CLI-only (no TUI, no pipeline stages/profiles); optional bridge provides reporting unification without forcing `ScanProfile` integration (`mobile-static`/`mobile-dynamic`/`mobile-regression` remain aspirational per `architecture/defense_lab.md` Future). Use native types for lab-specific flows; use bridge (or `report convert` on native JSON) for unified report consumers. Integration is lightweight and opt-in. Dynamic P1 complete (ADB + logcat); Phase 2a (proxy Level-1 + permissions) complete; future phases per the design plan.
...
See `crates/eggsec/src/mobile/`, `crates/eggsec/src/commands/handlers/mobile.rs`, `crates/eggsec/src/mobile/traffic.rs` (Phase 2a), and `crates/eggsec/Cargo.toml:310,318`.

## Future

Phase 1 dynamic complete (ADB core + logcat; 2026-06-12). Phase 2a (Level 1 proxy integration + permission testing) complete 2026-06-12 (new traffic.rs + adb helpers; report fields + bridge; CLI/handler; tests/smoke; under mobile-dynamic). Initial correlation delivered in Phase 2 final polish (F2); deeper work in 2b+. Close-out polish (final code hygiene + doc consistency) complete 2026-06-12. See `plans/dynamic-mobile-testing-loadout-design-plan.md` for Phase 2b/Frida/etc. (proxy correlation, deeper instrumentation, iOS notes, pipeline profiles, etc.). `mobile-static` / `mobile-dynamic` / `mobile-regression` profiles remain aspirational. TUI tab and MCP exposure not in scope for current round (standalone defense-lab pattern). Keep future notes for 2b/Frida/etc.
