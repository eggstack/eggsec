...
The compatibility checker (`scripts/compatibility_check.py`) compares the
...
signatures, and schema versions for a given commit. Generated by
`scripts/build_compatibility_baseline.py`.
...
`tests/test_resource_budgets.py` enforces hard limits on:
...
The release evidence bundle (`scripts/build_python_release_evidence.py`)
now includes Phase F artifacts:

| Artifact | Source | Contents |
|----------|--------|----------|
| `compatibility-baseline.json` | `build_compatibility_baseline.py` | API surface, type signatures, schema versions |
| `compatibility-diff.json` | `compatibility_check.py` | Added/removed/changed symbols, severity |
| `resource-budgets.json` | `test_resource_budgets.py` | Budget name, limit, actual, pass/fail |
| `redaction-coverage.json` | `test_redaction.py` | Paths tested, pass/fail, coverage percentage |
