...
| `libraries/unpwdb.rs` | `unpwdb.*` | in-memory only | CredentialMaterial | medium | `manual_allowed`, `agent_deny`, `ci_allow_local_only` | none | none | n/a | In-memory username/password wordlists only (no file/network I/O); no sandbox check required |
...
| `libraries/http.rs` (all) | `http.get()`, `http.post()`, `http.put()`, `http.delete()`, `http.head()`, `http.options()`, `http.request()` | `network_tcp` | NetworkAccess | high | `manual_allowed`, `agent_allow_if_scoped`, `ci_deny` | `network_operations`, `network_bytes_read`, `network_bytes_written` | **migrated** | `net_http` | **Migrated (Phase 05)** — wrapped via `check_network_tcp()`; denied requests never reach reqwest |
| `libraries/http.rs` (all) | `http.post_host()`, `http.put_data()` | `network_tcp` | NetworkAccess | high | `manual_allowed`, `agent_allow_if_scoped`, `ci_deny` | `network_operations`, `network_bytes_read`, `network_bytes_written` | **migrated** | `net_http` | **Migrated (Phase 05)** — wrapped via `check_network_tcp()` |
| `libraries/http.rs` (all) | `http.async_get()`, `http.async_post()`, `http.async_request()` | `network_tcp` | NetworkAccess | high | `manual_allowed`, `agent_allow_if_scoped`, `ci_deny` | `network_operations`, `network_bytes_read`, `network_bytes_written` | **migrated** | `net_http` | **Migrated (Phase 05)** — async HTTP, wrapped via `check_network_tcp()` |
...
| File | Function | Capability | Side Effect | Blocking Risk | Profile Policy | Accounting | Cancellation | Report Event | Notes |
|------|----------|------------|-------------|---------------|----------------|------------|--------------|--------------|-------|
| `libraries/tls.rs` + `sslcert.rs` (there is no `ssl.rs`) | SSL/TLS connection setup, certificate handling | `tls_crypto` | NetworkAccess | medium | `manual_allowed`, `agent_allow_if_scoped`, `ci_deny` | `network_operations`, `crypto_operations` | **migrated** | `tls_handshake` | **Migrated (Phase 05)** — wrapped via `check_crypto()`/`check_network_tcp()` |
| `libraries/openssl.rs` (all) | OpenSSL crypto operations | `tls_crypto` | NetworkAccess | medium | `manual_allowed`, `agent_allow_if_scoped`, `ci_allow_local_only` | `crypto_operations` | **migrated** | `crypto_op` | **Migrated (Phase 05)** — wrapped via `check_crypto()`/`check_randomness()` |
