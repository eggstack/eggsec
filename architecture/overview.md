...
| `eggsec-transport-eggfetch` | Pinned HTTP backend | Yes | `HttpTransport` over published `eggfetch-core 0.1.5` (approved-IP pinning, manual authorized redirects, H1/H2 via ALPN, pinned proxy peers/targets where enforceable). Production direct load-test backend. |
...
| Transport/Eggfetch | `crates/eggsec-transport-eggfetch/` | `HttpTransport` over published `eggfetch-core 0.1.5` (approved-IP pinning, manual authorized redirects, H1/H2 via ALPN, pinned proxy peers/targets; production direct load-test backend) | [transport_eggfetch.md](transport_eggfetch.md) |
...
eggsec-core (leaf — no workspace deps)
    ...
    ├── eggsec-transport-eggfetch (pinned HttpTransport over eggfetch-core 0.1.5; production direct load-test backend)
