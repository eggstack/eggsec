...
- **`eggsec-daemon`**: long-running daemon host for persistent sessions (`Runtime`), transport abstraction (Unix socket default; HTTP/SSE feature-gated via `http-api`), and client library. Provides `DaemonHost` (Runtime bridge with client registry and session access control), `ClientRegistry` (tracks connected clients with `ClientKind`/`ClientRole`), `DaemonClient` (Unix socket client with `declare_client()` handshake), and `ClientCommand`/`ServerMessage` JSON-line protocol. Transport types (`TransportKind`, `TransportCapability`, `DaemonCapabilities`) declare available transports to clients. `DaemonRequestContext` carries per-request metadata (client ID, peer address, transport kind). Supports multi-session/multi-frontend semantics: multiple clients can observe the same session, session owners control access, and permission checks enforce role-based command authorization. Persistence via `DaemonStore` trait with SQLite backend (WAL mode), snapshot hydration on startup, and audit event recording. Guardrails: no TUI dependencies, no engine dependencies (`eggsec`), transport crates are feature-gated optional dependencies. See [daemon.md](daemon.md).
- **`eggsec-ui-model`**: frontend-neutral view DTOs and renderer registry for session, task, result, artifact, permission, and event views. Provides `From` conversions from runtime types (`SessionSnapshot`, `TaskOutcome`, `RuntimeEvent`, etc.) to serializable view models. The `ResultRendererRegistry` maps result kinds to rendering metadata (kind label, artifact kinds, TUI/JSON support flags). Depends only on `eggsec-runtime` — no TUI, CLI, or transport dependencies. Consumed by `eggsec-tui` and `eggsec-cli` for normalized daemon-facing rendering.
...
| [`eggsec-nse/`](../crates/eggsec-nse/) | Nmap Scripting Engine support (Lua 5.4), selective practical NSE compatibility; 166 library implementations, 43 registry descriptors | [nse_integration.md](nse_integration.md) |
...
| [`eggsec-python`](../crates/eggsec-python/) | Python bindings via PyO3/maturin. The scoped pre-1.0 stable core provides sync/async clients, 22 canonical operation dispatch, mandatory policy/audit decisions, typed payloads, versioned structured errors, and governed event backpressure. Pipeline supports step dependencies, parallel execution groups, retry policy, and configurable failure policy. All promoted operations (git-secrets, SBOM, consolidated-recon, GraphQL, OAuth, authentication, database, NSE, container, mobile-static) go through the same policy gate and event lifecycle as the original ten. Daemon, proxy, and other domains remain provisional or experimental until they meet the common-engine graduation checklist. Release 4 adds stateful session types for mobile and headless-browser subsystems (`BrowserSession`, `BrowserCapabilities`, `MobileSession`, `MobileDeviceDescriptor`), session repository abstractions (`SessionRepository` backed by SQLite or in-memory), content-addressed artifact storage (`ArtifactStore`), incremental streaming reports (`StreamingReporter`), and report diffing (`ReportDiff`). All Release 4 session types are provisional — scope-checked and policy-gated but not yet part of the stable-core operation registry. See [docs/python/BROWSER_SESSION_ARCHITECTURE.md](../docs/python/BROWSER_SESSION_ARCHITECTURE.md) and [docs/python/DAEMON_PARITY_PROTOCOL.md](../docs/python/DAEMON_PARITY_PROTOCOL.md) for subsystem details. | [docs/python/](../docs/python/) |
...
Selective practical Nmap Scripting Engine compatibility:

| Component | Description |
|-----------|-------------|
| **Lua VM** | Lua 5.4 via mlua crate |
| **Libraries** | 166 library implementations; 43 registry descriptors (source of truth for compatibility) |
...
|--------|----------|---------|
| Daemon | `ClientKind` | Client type enum (Cli, Tui, Mcp, Rest, Agent, etc.) |
| Daemon | `ClientRole` | Session role enum (Owner, Controller, Observer, Approver) |
| Daemon | `ClientInfo` | Connected client metadata (id, kind, surface, label) |
...
| Daemon | `TransportKind` | Transport type enum (`UnixSocket`, `LoopbackHttp`) — WebSocket and gRPC deferred |
| Daemon | `DaemonRequestContext` | Per-request metadata (client ID, peer address, transport kind) |
| Daemon | `TransportCapability` | Describes a single available transport (kind, address, features) |
| Daemon | `DaemonCapabilities` | Runtime capabilities plus available transports list |
| Daemon | `HttpConfig` | HTTP/SSE server configuration (bind address, loopback enforcement) |
