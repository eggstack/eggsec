...
| Egress Decision | — | Phase E WS1 1.0.6 rejection (measured graphs, no edge) superseded 2026-09-22: narrow Eggress `eggsec-web-proxy` proxy-engine edge (`eggress-outbound`/`eggress-uri`, exact-pinned, no embed/routing/advanced), moved 2026-09-25 to exact 1.0.10 with first-hop socket-metadata closure; transport/eggfetch/policy/core stay Eggress-free | [egress_reuse_decision.md](egress_reuse_decision.md) |
...
*Last updated: 2026-09-22 — Re-verified counts against source (workspace 20 crates; CLI 52 variants; TUI 33 tabs; TaskKind 29; PayloadType 40; EggsecError 23; ScanProfile 18; endpoints 347; daemon-protocol 15/14/11; themes 50; findings 19/9/5/13; handlers 32 modules + mod). Corrections: operations 34 canonical + 42 aliases (was 43; `crates/eggsec-policy/src/catalog.rs:291`, alias table at `:875` after Phase C extraction, facade at `config/policy_catalog.rs`); indexed previously unlinked [performance.md](performance.md) in Deep-Dive Index. Same-day systematic deep-dive review: 11 subagent batches across all 71 deep-dives; applied verified HIGH/MED fixes (c2 wiring/mode, daemon orphan-registry note, TUI TAB_SPECS/attach cites, mobile adb lines, domain test count, enforcement kernel paths, TaskResult 34, recon modules/toggles/secrets, packet gates, fuzzer pattern taxonomy, browser dead-stub note, audit counts, storage/web-proxy/db-lab/supply-chain counts, defense-lab c2/postex rows, python feature inventory, output/engine file counts, NSE Phase-05 rows, agent-reqwest removal, MSRV pins). Code-level LOW nits (FxHashMap blotches, `let _ =` sites, missing timeout wrappers, live-Chrome ignores) recorded for follow-up, not fixed in this docs pass.*
