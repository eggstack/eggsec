...
| NSE | [eggstack/eggsec-nse](https://github.com/eggstack/eggsec-nse) | Standalone Nmap Scripting Engine compatibility runtime: Lua 5.4 VM (mlua), curated libraries, `ScriptResolver`, sandbox, execution profiles, and CVE integration; Eggsec feature-gated: `nse` | [nse_integration.md](nse_integration.md), [nse_capability_inventory.md](nse_capability_inventory.md) |
...
*Last updated: 2026-09-25 — Same-day systematic deep-dive review: 8 subagent batches across all 68 deep-dives; applied verified HIGH/MED fixes (agent `Agent::new` Result + reqwest/rustls removal, compile-time-baseline supersession note, runtime-bridge `ApprovedExecution` bundle, daemon orphan-registry removal, NSE ManualStrict/CiSafe limits, api-schema stub-claim removal, CLI 52-variant/handler-arm re-cites, TUI ~983 tests, c2 57-test breakdown, db-lab 17 mappings/23 correlations, web-proxy Eggress inventory, error From-count + ProbeError 4-variant correction, transport/eggfetch/loadtest re-verified no-change).*
