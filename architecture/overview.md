...
| [`eggsec-python`](../crates/eggsec-python/) | Python bindings via PyO3/maturin. Host-language binding over the Rust engine providing sync/async clients, scope enforcement, scanning, recon, WAF detection, reporting, typed findings schema, artifact storage, workflow management, baseline comparison, compliance mapping, external integrations, and schema migration. Milestone G adds: domain registry and operation introspection (DomainDescriptor, DomainRegistry), versioned event protocol (EventEnvelope, EventStream), callback/sink contracts (AuditSink, FindingSink, AsyncCallback, CallbackScheduler, BackpressureChannel), Python-native ergonomics (pathlib, datetime, hash/eq, context managers, pickle), binary buffer protocol (BinaryBuffer, LazyArtifact, PaginatedResults), API surface introspection and feature matrix, performance benchmarks and regression gates, and 1.0 readiness checklist with stability classifications. **Stability**: Core scanning/recon/WAF operations are `stable`. Milestone C/D/E domains are `provisional`. Wireless, evasion, postex, C2, dynamic mobile, headless browser, advanced hunting, and AI integration are `experimental`. | [docs/python/](../docs/python/) |
