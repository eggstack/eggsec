...
- `wireless` (handler: `commands/handlers/wireless.rs`): Standalone-complete passive WiFi recon (CLI + TUI tab). Uses local `WirelessScanResult`/`WirelessNetwork` (defined in `wireless/mod.rs`). Policy via `SafeActive` + `wireless` feature. Optional `to_scan_report_data` bridge (populates findings + full `wireless_networks`); native `--json` (or `--repeat` wrapped form) is auto-bridged by the report handler for `report convert`. Not integrated with `ScanProfile` pipelines or dedicated profiles. See `architecture/wireless.md`, `crates/eggsec/src/wireless/mod.rs`, and docs/WIRELESS.md.
