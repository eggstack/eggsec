...
- **db-pentest** (`eggsec db pentest ...`, feature `db-pentest`): Direct Postgres/MySQL/MSSQL lab checks (Phase 1+2+3). TUI tab `Tab::DbPentest` + pipeline `ScanProfile::DbRegression` (Phase 3). Advanced gated checks behind `--allow-db-pentest-advanced`. Risk tier `DbPentest` (real) / `SafeActive` (dry-run). Requires `--allow-db-pentest` for non-dry runs. Native `DbPentestReport` + `to_scan_report_data_db` bridge (auto in `report convert`). See `architecture/database_pentest.md`, `plans/database-pentesting-phase1-foundation-handoff-plan.md`, and `plans/database-pentesting-phase3-advanced-and-integration-handoff-plan.md`.
...
| `db-regression` | Defense-lab family profile for db-pentest regression. Maps to defense-lab stages initially. | PortScan → Fingerprint → EndpointScan → Waf → Fuzz | `db-pentest` feature. Full native Stage::DbPentest deferred. |
