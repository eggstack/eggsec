...
- **Mobile static/regression profiles**: `mobile-static` and `mobile-regression` pipeline profiles (aspirational; Phase 1 is standalone CLI `eggsec mobile` under `SafeActive` only, suitable for defense-lab use on lab-provided APKs/IPAs). See `architecture/mobile.md`, `architecture/proposed-wireless-mobile-stages.md`, and plans/mobile-first-handoff-plan.md + integration-work-plan.md. Dynamic loadout per `plans/dynamic-mobile-testing-loadout-design-plan.md`. Phase 2a (proxy foundation + runtime permissions; 2026-06-12) remains standalone defense-lab (no pipeline/TUI/MCP).
