...
**Phase 4 regression coverage**: 96 tests across `config::policy_decision::tests` (48) and `commands::handlers::tests` (48) lock all manual-mode invariants. **Phase 8 enforcement matrix**: 169 tests in `tests/enforcement_matrix.rs` provide systematic cross-surface coverage for the dual-mode contract. See `docs/ENFORCEMENT_MODES.md` Phase 4 and Phase 8 sections for the full invariant-to-test mapping.
