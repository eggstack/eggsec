...
pub use crate::fuzzer::payloads::graphql::{GraphQLFuzzer, GraphQLTestResult, GraphQLVulnerability};
...
pub use crate::fuzzer::payloads::websocket::{WebSocketFuzzer, WebSocketTestResult, WebSocketVulnerability};
...
impl AdvancedFuzzer for GraphQLFuzzer {
    async fn fuzz(&mut self, client: &reqwest::Client) -> Vec<FuzzResult> {
        ...
        let injection_results = self.generate_injection_queries(self.enable_depth_bypass, self.enable_alias_overload);
...
impl FuzzerResultConverter<GraphQLTestResult> for GraphQLTestResult {
    fn into_fuzz_result(self) -> FuzzResult {
        ...
        FuzzResult {
            ...
            leaks_found: if self.success { vec![self.description] } else { vec![] },
            error: None,
...
impl FuzzerResultConverter<JwtTestResult> for JwtTestResult {
    fn into_fuzz_result(self) -> FuzzResult {
        ...
        FuzzResult {
            ...
            leaks_found: if self.success { vec![self.description] } else { vec![] },
            error: None,
            owasp_category: Some(crate::waf::types::OwaspCategory::A02_2023_BrokenAuthentication.to_string()),
            detected_severity: self.severity,
        }
    }
}
...
impl AdvancedFuzzer for OAuthFuzzer {
    async fn fuzz(&mut self, client: &reqwest::Client) -> Vec<FuzzResult> {
        ...
        let mut fuzzer_with_client = OAuthFuzzer::new(
        ...
        if self.enable_redirect {
            let issuer = self.issuer_url.clone().unwrap_or_else(|| "https://example.com".to_string());
...
impl AdvancedFuzzer for IdorFuzzer {
    async fn fuzz(&mut self, client: &reqwest::Client) -> Vec<FuzzResult> {
        ...
        let mut fuzzer_with_client = IdorFuzzer::new(self.base_url.clone())
            .with_client(client.clone());
...
impl FuzzerResultConverter<IdorTestResult> for IdorTestResult {
    fn into_fuzz_result(self) -> FuzzResult {
        ...
        FuzzResult {
            ...
            leaks_found: if self.success { vec![self.description] } else { vec![] },
            error: None,
            owasp_category: Some(crate::waf::types::OwaspCategory::A01_2023_BrokenObjectLevelAuthorization.to_string()),
            detected_severity: self.severity,
        }
    }
}
...
impl AdvancedFuzzer for SstiFuzzer {
    async fn fuzz(&mut self, client: &reqwest::Client) -> Vec<FuzzResult> {
        ...
        let mut fuzzer_with_client = SstiFuzzer::new()
            .with_client(client.clone());
