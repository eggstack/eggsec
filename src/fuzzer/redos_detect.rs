...
impl RegexExecutor {
    ...
    pub fn check_pattern(&self, pattern: &str) -> ReDosResult {
        ...
        for test_string in &self.test_strings {
            ...
            let result = self.execute_with_limit(&regex, test_string, iterations_clone, timeout_clone);
        ...
        let is_vulnerable = timeout_flag.load(Ordering::SeqCst) == 1 || iter_count > self.max_iterations / 10;
        ...
        ReDosResult {
            ...
            sample_match: if matched_any { Some("matched".to_string()) } else { None },
        }
    }
    ...
    pub async fn check_pattern_async(&self, pattern: &str) -> ReDosResult {
        ...
        let result = task::spawn_blocking(move || {
            executor.check_pattern(&pattern_owned)
        })
        .await;
...
impl ReDosDetector {
    ...
    pub fn detect(&self, pattern: &str) -> ReDosResult {
        if self.known_vulnerable_patterns.iter().any(|p| pattern.contains(p)) {
            return ReDosResult {
                pattern: pattern.to_string(),
                is_vulnerable: true,
                is_match: false,
...
impl PayloadReDosChecker {
    ...
    pub fn add_vulnerable_payload(&mut self, payload: &str, patterns: Vec<String>) {
        self.vulnerable_payloads.insert(payload.to_string(), patterns);
    }
}
