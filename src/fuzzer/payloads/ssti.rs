...
impl SstiFuzzer {
    ...
    pub async fn test_ssti_on_server(&mut self) -> Vec<SstiTestResult> {
        ...
        let param = self.param_name.clone().unwrap_or_else(|| "name".to_string());
                ...
                Ok(resp) => {
                    ...
                    if is_vulnerable {
                        ...
                        results.push(SstiTestResult {
                            ...
                            description: format!("Server error (500) - possible {} injection", desc),
                        });
                    }
                }
                Err(_) => continue,
        ...
        let time_based_payloads = vec![
            ("{{range(\"\"|int|list,1,500000000)}}a{{end}}", "Jinja2", TemplateEngine::Jinja2),
        ...
        for (payload, desc, engine) in time_based_payloads {
            ...
            if elapsed > 4000 {
                results.push(SstiTestResult {
                    ...
                    severity: Severity::Critical,
                    description: format!("Time-based SSTI detected! {} - {}ms delay", desc, elapsed),
                });
            }
        }
