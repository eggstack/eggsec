...
impl GraphQLFuzzer {
    ...
    pub async fn run_introspection(&mut self, client: &reqwest::Client) -> Result<bool, Box<dyn std::error::Error + Send + Sync>> {
        let query = r#"
        ...
        if text.contains("\"data\"") && !text.contains("\"errors\"") {
            if let Ok(json) = serde_json::from_str::<serde_json::Value>(&text) {
                if let Some(data) = json.get("data") {
                    if let Ok(schema) = serde_json::from_value::<GraphQLSchema>(data.get("__schema").unwrap().clone()) {
    ...
    pub fn generate_injection_queries(&self, include_depth_bypass: bool, include_alias_overload: bool) -> Vec<GraphQLTestResult> {
        let mut results = Vec::new();
                ...
                if field.r#type.contains("String") || field.r#type.contains("Int") {
                    ...
                    for payload in injection_payloads {
                        let query = format!(
                            r#"{{ {}("{}": "{}") }}"#,
                            field.name,
                            field.args.first().map(|a| a.name.as_str()).unwrap_or("input"),
                            payload
                        );
...
pub fn get_payloads() -> Vec<Payload> {
    ...
    payloads.push(Payload {
        ...
        severity: Severity::Critical,
        tags: vec!["graphql".to_string(), "log4j".to_string(), "rce".to_string()],
    });
