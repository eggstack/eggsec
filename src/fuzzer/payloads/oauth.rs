            ...
            Ok(resp) => {
                if resp.status().as_u16() == 200 {
                    results.push(OAuthTestResult {
                        ...
                        description: format!("OpenID Configuration endpoint accessible: {}", discovery_url),
                    });
                    
                    if let Ok(config) = resp.json::<serde_json::Value>().await {
                        if let Some(auth_endpoint) = config.get("authorization_endpoint").and_then(|v| v.as_str()) {
                            let auth_results = self.test_authorization_endpoint(auth_endpoint).await;
                            results.extend(auth_results);
                        }
                        
                        if let Some(token_endpoint) = config.get("token_endpoint").and_then(|v| v.as_str()) {
                ...
                Ok(resp) => {
                    ...
                    if status == 200 {
                        results.push(OAuthTestResult {
                            ...
                            severity: Severity::High,
                            description: format!("Potentially dangerous grant type allowed: {}", desc),
                        });
                    } else if status == 400 {
