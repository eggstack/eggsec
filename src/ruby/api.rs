...
use magnus::{Error, Ruby, prelude::*, value::ReprValue, module::Module, class::Class};
...
fn http_get(url: String) -> Result<magnus::RHash, Error> {
    ...
    let body = rt.block_on(async { response.text().await })
        .map_err(|e| Error::runtime(e.to_string()))?;
    hash.aset("body", body)?;
...
fn http_post(url: String, body: String) -> Result<magnus::RHash, Error> {
    ...
    let hash = Ruby::get().unwrap().hash_new();
    hash.aset("status", response.status().as_u16())?;
    let response_body = rt.block_on(async { response.text().await })
        .map_err(|e| Error::runtime(e.to_string()))?;
...
#[cfg(feature = "ruby-plugins")]
fn fuzz_param(url: String, param: String, payloads: Vec<String>, options: Vec<String>) -> Result<Vec<magnus::RHash>, Error> {
    ...
    for payload in payloads {
        ...
        let response = rt.block_on(async {
            client.get(&url_with_param).send().await
        });
...
#[cfg(feature = "ruby-plugins")]
fn fuzz_header(url: String, header: String, payloads: Vec<String>, _options: Vec<String>) -> Result<Vec<magnus::RHash>, Error> {
    let mut results = Vec::new();
    let ruby = Ruby::get().unwrap();
...
#[cfg(feature = "ruby-plugins")]
fn fuzz_cookie(url: String, cookie_name: String, payloads: Vec<String>, _options: Vec<String>) -> Result<Vec<magnus::RHash>, Error> {
    let mut results = Vec::new();
    let ruby = Ruby::get().unwrap();
...
fn detect_vulnerability(resp: &reqwest::Response, body: &str) -> bool {
    ...
    if status == 500 || status == 200 && body.contains("SQL") || body.contains("mysql") || body.contains("syntax") {
        return true;
    }
...
#[cfg(feature = "ruby-plugins")]
fn report_finding(severity: String, finding_type: String, description: String, location: String) -> Result<(), Error> {
    tracing::info!(
        severity = %severity,
        type = %finding_type,
        location = %location,
...
#[cfg(feature = "ruby-plugins")]
fn report_vulnerability(severity: String, vuln_type: String, description: String, location: String, _cve: String) -> Result<(), Error> {
    tracing::warn!(
        severity = %severity,
        type = %vuln_type,
        location = %location,
...
fn register_metasploit_api(ruby: &Ruby, slapper: &magnus::RModule) -> Result<(), Error> {
    ...
    msf.define_module_function("connect_with_token", magnus::function!(msf_connect_with_token, 2))?;
    ...
    msf.define_module_function("generate_payload", magnus::function!(msf_generate_payload, 2))?;
    ...
    msf.define_module_function("session_shell_write", magnus::function!(msf_session_write, 2))?;
...
fn register_encoder_api(ruby: &Ruby, slapper: &magnus::RModule) -> Result<(), Error> {
    ...
    encoder.define_module_function("compatible_payloads", magnus::function!(encoder_compatible_payloads, 1))?;
...
static MSF_CLIENT: std::sync::OnceLock<tokio::sync::Mutex<Option<MsfClientState>>> = std::sync::OnceLock::new();
...
#[cfg(feature = "ruby-plugins")]
fn msf_execute_module(module_type: String, module_name: String, options: Vec<String>) -> Result<magnus::RHash, Error> {
    let msf_type = match module_type.to_lowercase().as_str() {
        "exploit" => crate::msf::ModuleType::Exploit,
        "auxiliary" => crate::msf::ModuleType::Auxiliary,
        "post" => crate::msf::ModuleType::Post,
    ...
    rt.block_on(async {
        let guard = get_msf_client().lock().await;
        if let Some(ref state) = *guard {
            match state.client.execute_module(msf_type, &module_name, &opts).await {
                Ok(result) => {
...
fn msf_session_write(session_id: String, command: String) -> Result<String, Error> {
    ...
    rt.block_on(async {
        let guard = get_msf_client().lock().await;
        if let Some(ref state) = *guard {
            match state.client.execute_session_command(&session_id, &command).await {
                Ok(output) => Ok(output),
                Err(e) => Err(Error::runtime(e.to_string()))
            }
        } else {
...
#[cfg(feature = "ruby-plugins")]
fn encoder_encode(payload: String, encoder_name: String, options: Vec<String>) -> Result<String, Error> {
    let opts: std::collections::HashMap<String, String> = options
        .iter()
        .filter_map(|s| {
            let parts: Vec<&str> = s.splitn(2, '=').collect();
