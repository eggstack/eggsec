    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        if self.results.is_empty() {
            ...
            for fp in &self.results {
                let response = fp.response.as_deref().unwrap_or("-").chars().take(50).collect::<String>();
                writeln!(
                    f,
                    "{:>6} {:<15} {:<50}",
                    fp.port,
...
async fn fingerprint_udp_port(host: &str, port: u16, timeout_duration: Duration) -> Option<UdpServiceFingerprint> {
                ...
                if expected_response.starts_with("\\x") {
                    if hex_contains(response, expected_response) {
                        return Some(UdpServiceFingerprint {
                            ...
                            banner: Some(response_str.lines().next().unwrap_or("").chars().take(50).collect()),
                            confidence: 80,
                        });
                    }
                } else if response_str.to_lowercase().contains(&expected_response.to_lowercase()) {
                    return Some(UdpServiceFingerprint {
                        port,
                        service: service.to_string(),
                        response: Some(response_str.chars().take(100).collect()),
