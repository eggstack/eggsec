...
impl std::fmt::Display for FingerprintResults {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        if self.results.is_empty() {
            ...
            writeln!(f, "{:>6} {:<15} {:<20} {:<30}", "PORT", "SERVICE", "PRODUCT/VERSION", "BANNER")?;
            for fp in &self.results {
                ...
                let banner = fp.banner.as_deref().unwrap_or("-").lines().next().unwrap_or("-");
                writeln!(
                    f,
                    "{:>6} {:<15} {:<20} {:<30}",
                    fp.port,
...
pub async fn run_cli(args: FingerprintArgs, config: &SlapperConfig) -> Result<()> {
    ...
    if args.udp {
        ...
        let results = fingerprint_udp_services(&args.host, ports, Duration::from_secs(timeout_secs)).await?;
        ...
        let results = fingerprint_services(&args.host, ports, Duration::from_secs(timeout_secs), false).await?;
...
async fn fingerprint_port(host: &str, port: u16, timeout_duration: Duration) -> Option<ServiceFingerprint> {
    ...
    let probes_to_try: Vec<(&str, &[u8], &str)> = match port {
        ...
        5900 | 5901 | 5902 | 5903 => vec![("VNC", b"", "RFB")],
        23 => vec![("Telnet", b"", "login:|telnet")],
        5672 | 5671 => vec![("RabbitMQ", b"AMQP\x00\x00\x09\x01", "AMQP")],
        2181 | 2182 | 2183 => vec![("Zookeeper", b"ruok", "imok")],
        9200 | 9300 | 9243 => vec![("Elasticsearch", b"GET / HTTP/1.0\r\n\r\n", "\"name\"|\"cluster_name\"|lucene")],
        9092 | 9093 | 9094 => vec![("Kafka", b"\x00\x00\x00\x1c\x00\x01\x00\x00\x00\x00\x00\x03api\x00\x00\x00\x01\x00", "\\x00\\x00\\x00")],
        ...
        4150 | 4151 | 4152 => vec![("NSQ", b"  V2", "\\x20\\x56\\x32")],
    ...
    for (probe_name, probe_data, match_pattern) in probes_to_try {
        ...
        if let Some(fp) = probe_service(&mut stream, probe_name, probe_data, match_pattern, port, timeout_duration).await {
            return Some(fp);
        }
    }
        ...
        Ok(Ok(n)) if n > 0 => {
            ...
            let matches = match_pattern.split('|').any(|pattern| {
                if pattern.starts_with("\\x") {
                    ...
                    response_str.to_lowercase().contains(&pattern.to_lowercase())
                }
            });
            ...
            if matches {
                ...
                return Some(ServiceFingerprint {
                    ...
                    banner: if !banner.is_empty() { Some(banner) } else { None },
                    version,
                    product,
                    extra: None,
                    confidence: 90,
...
fn hex_match(pattern: &str, data: &[u8]) -> bool {
    ...
    data.windows(hex_bytes.len()).any(|window| window == hex_bytes.as_slice())
}
                ...
                if version_line.starts_with("SSH-") {
                    ...
                    if parts.len() >= 2 {
                        ...
                        if version_parts.len() >= 3 {
                            return (Some(parts[1].to_string()), Some(version_parts[2].to_string()));
                        }
                    }
                }
            }
