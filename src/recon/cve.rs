...
impl CveMapper {
    ...
    pub async fn map_cves(&mut self, tech_stack: &TechStack) -> Result<CveMapping> {
        ...
        all_vulns.sort_by(|a, b| b.cvss_score.partial_cmp(&a.cvss_score).unwrap());

        let total_critical = all_vulns.iter().filter(|v| v.severity == "CRITICAL").count();
        ...
        map.insert(
            ...
            vec![
                VulnerabilityInfo {
                    ...
                    description: "Path traversal and remote code execution in Apache HTTP Server 2.4.49".to_string(),
                ...
                VulnerabilityInfo {
                    ...
                    description: "Path traversal and RCE in Apache HTTP Server 2.4.49/50".to_string(),
                    severity: "CRITICAL".to_string(),
                    cvss_score: 9.8,
                    affected_product: "Apache HTTP Server".to_string(),
        ...
        map.insert(
            ...
            vec![
                VulnerabilityInfo {
                    ...
                    description: "Redis heap overflow vulnerability leading to remote code execution".to_string(),
                    severity: "CRITICAL".to_string(),
                    cvss_score: 9.8,
            ...
            for vuln in vulnerabilities.iter().take(10) {
                if let Some(cve) = vuln.get("cve") {
                    ...
                    let (severity, cvss_score) = if let Some(metrics) = cve.get("metrics") {
                        if let Some(cvss) = metrics.get("cvssMetricV31").and_then(|v| v.as_array()).and_then(|arr| arr.first()) {
                            let score = cvss.get("cvssData").and_then(|c| c.get("baseScore")).and_then(|s| s.as_f64()).unwrap_or(0.0) as f32;
                            let base_severity = cvss.get("cvssData").and_then(|c| c.get("baseSeverity")).and_then(|s| s.as_str()).unwrap_or("UNKNOWN");
