...
impl ThreatIntelClient {
    ...
    async fn check_virustotal_ip(&self, ip: &str, api_key: &str) -> Result<IpReputation> {
        ...
        let last_analysis = data.get("last_analysis_stats").ok_or_else(|| anyhow::anyhow!("No analysis stats"))?;

        let malicious = last_analysis.get("malicious").and_then(|v| v.as_i64()).unwrap_or(0);
        let suspicious = last_analysis.get("suspicious").and_then(|v| v.as_i64()).unwrap_or(0);
        ...
        Ok(IpReputation {
            ip: ip.to_string(),
            score,
            category: if score > 50 { "malicious" } else if score > 20 { "suspicious" } else { "clean" }.to_string(),
            threats,
            asn: data.get("asn").and_then(|v| v.as_i64()).map(|n| format!("AS{}", n)),
            isp: data.get("isp").and_then(|v| v.as_str()).map(String::from),
            country: data.get("country").and_then(|v| v.as_str()).map(String::from),
            source: "VirusTotal".to_string(),
        })
    }

    async fn check_virustotal_domain(&self, domain: &str, api_key: &str) -> Result<DomainReputation> {
        let url = format!(
        ...
        Ok(DomainReputation {
            ...
            category: if score > 50 { "malicious" } else if score > 20 { "suspicious" } else { "clean" }.to_string(),
            threats,
            registrar: data.get("registrar").and_then(|v| v.as_str()).map(String::from),
            created_date: data.get("creation_date").and_then(|v| v.as_str()).map(String::from),
            nameservers: data.get("name_servers").and_then(|ns| ns.as_array()).map(|arr| {
    ...
    async fn check_shodan_ip(&self, ip: &str, api_key: &str) -> Result<IpReputation> {
        ...
        Ok(IpReputation {
            ...
            threats,
            asn: shodan_resp.get("asn").and_then(|v| v.as_str()).map(String::from),
            isp: shodan_resp.get("isp").and_then(|v| v.as_str()).map(String::from),
            country: shodan_resp.get("country_name").and_then(|v| v.as_str()).map(String::from),
            source: "Shodan".to_string(),
        })
    }

    async fn check_alienvault_pdns(&self, ip: &str, api_key: &str) -> Result<Vec<PassiveDnsRecord>> {
        let url = format!(
            ...
            for record in pdns.iter().take(20) {
                if let Some(hostname) = record.get("hostname").and_then(|h| h.as_str()) {
                    records.push(PassiveDnsRecord {
                        ...
                        first_seen: record.get("first_seen").and_then(|v| v.as_str()).map(String::from),
                        last_seen: record.get("last_seen").and_then(|v| v.as_str()).map(String::from),
    ...
    async fn check_alienvault_domain_pdns(&self, domain: &str, api_key: &str) -> Result<Vec<PassiveDnsRecord>> {
        let url = format!(
