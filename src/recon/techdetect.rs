...
impl TechDetector {
    ...
    fn detect_servers(&self, headers: &HashMap<String, String>, stack: &mut TechStack) {
        ...
        if let Some(s) = server {
            ...
            if s.contains("microsoft-iis") || s.contains("iis") {
                if !stack.servers.contains(&"IIS".to_string()) {
            ...
            if s.contains("lite-speed") || s.contains("litespeed") {
                if !stack.servers.contains(&"LiteSpeed".to_string()) {
                    stack.servers.push("LiteSpeed".to_string());
                }
            }
            if s.contains("openresty") {
                if !stack.servers.contains(&"OpenResty".to_string()) {
                    stack.servers.push("OpenResty".to_string());
                }
            }
            if s.contains("caddy") {
                if !stack.servers.contains(&"Caddy".to_string()) {
    ...
    fn detect_frameworks(&self, headers: &HashMap<String, String>, body: &str, stack: &mut TechStack) {
        ...
        if let Some(pb) = powered_by {
            ...
            if pb.contains("rails") || pb.contains("ruby on rails") {
                if !stack.frameworks.contains(&"Ruby on Rails".to_string()) {
            ...
            if pb.contains("next.js") || pb.contains("nextjs") {
                if !stack.frameworks.contains(&"Next.js".to_string()) {
    ...
    fn detect_databases(&self, headers: &HashMap<String, String>, _body: &str, stack: &mut TechStack) {
        let server = headers.get("server").map(|s| s.to_lowercase());
        ...
        if let Some(s) = server {
            ...
            if s.contains("postgresql") || s.contains("postgres") {
                if !stack.databases.contains(&"PostgreSQL".to_string()) {
                    stack.databases.push("PostgreSQL".to_string());
                }
            }
            if s.contains("mongodb") || s.contains("mongo") {
                if !stack.databases.contains(&"MongoDB".to_string()) {
            ...
            if s.contains("elasticsearch") && !stack.databases.contains(&"Elasticsearch".to_string()) {
    ...
    fn detect_javascript(&self, body: &str, stack: &mut TechStack) {
        ...
        if body.contains("node_modules/angular") && !stack.javascript.contains(&"Angular".to_string()) {
            stack.javascript.push("Angular".to_string());
        }
    ...
    fn detect_languages(&self, headers: &HashMap<String, String>, _body: &str, stack: &mut TechStack) {
        let powered_by = headers.get("x-powered-by").map(|s| s.to_lowercase());
