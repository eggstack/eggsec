...
static LOCAL_IP_DATA: LazyLock<HashMap<String, (String, String, String)>> = LazyLock::new(|| {
    ...
    m.insert("10.0.0.0/8".to_string(), ("Private Network".to_string(), "XX".to_string(), "Local".to_string()));
    m.insert("172.16.0.0/12".to_string(), ("Private Network".to_string(), "XX".to_string(), "Local".to_string()));
    m.insert("192.168.0.0/16".to_string(), ("Private Network".to_string(), "XX".to_string(), "Local".to_string()));
    m.insert("127.0.0.0/8".to_string(), ("Loopback".to_string(), "XX".to_string(), "Localhost".to_string()));
    m.insert("224.0.0.0/4".to_string(), ("Multicast".to_string(), "XX".to_string(), "Multicast".to_string()));
    m
});
...
impl GeoLocator {
    ...
    pub async fn init_geoip(&mut self) -> Result<()> {
        ...
        if !ip66_path.exists() && !maxmind_path.exists() {
            if settings.as_ref().map(|s| s.account_id.is_some() && s.license_key.is_some()).unwrap_or(false) {
    ...
    async fn download_maxmind_db(&self, settings: &MaxMindSettings) -> Result<()> {
        let account_id = settings.account_id.ok_or_else(|| anyhow!("MaxMind account_id required"))?;
        let license_key = settings.license_key.as_ref().ok_or_else(|| anyhow!("MaxMind license_key required"))?;
        ...
        if !response.status().is_success() {
            return Err(anyhow!("Failed to download MaxMind DB: {}", response.status()));
        }
    ...
    async fn lookup_maxmind(&self, ip: &str) -> Option<GeoLocation> {
        ...
        let region = city.subdivisions.first().and_then(|s| s.names.english).map(String::from);
        let city_name = city.city.names.english.map(|s| s.to_string());
        
        let isp = city.traits.is_anycast.map(|a| if a { "Anycast".to_string() } else { "No".to_string() });
    ...
    async fn lookup_ip2c(&self, ip: &str) -> Result<GeoLocation> {
        ...
        if parts.get(0) == Some(&"1") && parts.len() >= 4 {
