...
fn get_whois_server(domain: &str) -> &'static str {
    ...
    if domain_lower.ends_with(".com") || domain_lower.ends_with(".net") || 
       domain_lower.ends_with(".org") || domain_lower.ends_with(".info") || 
       domain_lower.ends_with(".biz") || domain_lower.ends_with(".io") ||
       domain_lower.ends_with(".co") || domain_lower.ends_with(".me") || 
       domain_lower.ends_with(".tv") || domain_lower.ends_with(".us") || 
       domain_lower.ends_with(".cc") || domain_lower.ends_with(".app") ||
       domain_lower.ends_with(".dev") || domain_lower.ends_with(".cloud") ||
       domain_lower.ends_with(".xyz") || domain_lower.ends_with(".online") {
        "whois.internic.net"
...
async fn lookup_whois(domain: &str, server: &str) -> Result<String> {
    ...
    let socket_addr = addrs.next().ok_or_else(|| anyhow!("No address found for {}", server))?;
...
fn parse_whois_response(domain: &str, response: &str) -> WhoisResult {
    ...
    for line in response.lines() {
        ...
        else if line.to_lowercase().starts_with("creation date:") || line.to_lowercase().starts_with("created date:") {
            result.created_date = Some(extract_value(line));
        }
        else if line.to_lowercase().starts_with("expiry date:") || line.to_lowercase().starts_with("expiration date:") {
            result.expires_date = Some(extract_value(line));
        }
        else if line.to_lowercase().starts_with("updated date:") || line.to_lowercase().starts_with("modified date:") {
            result.updated_date = Some(extract_value(line));
        }
        else if line.to_lowercase().starts_with("name server:") || line.to_lowercase().starts_with("nserver:") {
            let ns = extract_value(line);
            if !ns.is_empty() && !result.nameservers.contains(&ns) {
                result.nameservers.push(ns);
            }
        }
        else if line.to_lowercase().starts_with("domain status:") || line.to_lowercase().starts_with("status:") {
