...
fn extract_asn_from_hostname(hostname: &str) -> (Option<String>, Option<String>) {
    ...
    for (pattern, name) in asn_patterns {
        if hostname_lower.contains(pattern) || hostname_lower.contains(&name.to_lowercase().replace(" ", "-")) {
            return (Some(format!("AS{}", pattern.trim_start_matches("as"))), Some(name.to_string()));
        }
    }
