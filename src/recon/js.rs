...
impl JsAnalyzer {
    ...
    fn extract_secrets(&self, content: &str) -> Vec<Secret> {
        ...
        let patterns = [
            (r#"(?i)(api[_-]?key|apikey|secret[_-]?key)["']?\s*[:=]\s*["']([^"']{8,})["']"#, "API_KEY"),
            (r#"(?i)(aws[_-]?access[_-]?key|aws[_-]?secret)["']?\s*[:=]\s*["']([^"']{10,})["']"#, "AWS_KEY"),
            (r#"(?i)(private[_-]?key|password|passwd|pwd)["']?\s*[:=]\s*["']([^"']{6,})["']"#, "PASSWORD"),
            (r#"(?i)bearer\s+[a-zA-Z0-9\-_\.]+"#, "BEARER_TOKEN"),
            (r#"(?i)basic\s+[a-zA-Z0-9+/=]+"#, "BASIC_AUTH"),
            (r#"(?i)(jwt|token)["']?\s*[:=]\s*["'](eyJ[a-zA-Z0-9\-_\.]+)["']"#, "JWT"),
        ];
