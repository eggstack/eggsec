...
fn build_dhcp_packet(msg_type: u8, xid: u32, mac: &[u8; 6], requested_ip: Option<&str>, server_ip: Option<&str>, options: &[u8]) -> Vec<u8> {
...
fn parse_dhcp_response(packet: &[u8]) -> Result<(String, String, String, String, u32), String> {
    ...
    while i < packet.len() - 2 {
        ...
        match opt_code {
            OPT_SUBNET_MASK => {
                if opt_len >= 4 {
                    subnet_mask = Ipv4Addr::new(packet[i + 2], packet[i + 3], packet[i + 4], packet[i + 5]).to_string();
                }
            }
            OPT_ROUTER => {
                if opt_len >= 4 {
                    router = Ipv4Addr::new(packet[i + 2], packet[i + 3], packet[i + 4], packet[i + 5]).to_string();
                }
            }
            OPT_DNS => {
                if opt_len >= 4 {
                    dns = Ipv4Addr::new(packet[i + 2], packet[i + 3], packet[i + 4], packet[i + 5]).to_string();
                }
            }
            OPT_LEASE_TIME => {
                if opt_len >= 4 {
...
pub fn register_dhcp_library(lua: &Lua) -> LuaResult<()> {
    ...
    let discover_fn = lua.create_function(|lua, (host, mac): (String, String)| {
        ...
        match UdpSocket::bind("0.0.0.0:68") {
            Ok(socket) => {
                ...
                let target = if host.is_empty() { "255.255.255.255" } else { &host };
                        ...
                        match socket.recv_from(&mut buf) {
                            Ok((n, _)) => {
                                if n > 0 {
                                    ...
                                    if let Ok((your_ip, subnet, router, dns, lease)) = parse_dhcp_response(&buf[..n]) {
    ...
    let request_fn =
        lua.create_function(|lua, (host, mac, requested_ip, server_ip): (String, String, String, Option<String>)| {
            let mac_bytes = mac_to_bytes(&mac).map_err(|e| mlua::Error::RuntimeError(e))?;
            let xid = rand::random::<u32>();
            
            let packet = build_dhcp_packet(DHCP_REQUEST, xid, &mac_bytes, Some(&requested_ip), server_ip.as_deref(), &[]);
    ...
    let async_discover_fn = lua.create_function(|lua, (host, mac): (String, String)| {
        ...
        let xid = rand::random::<u32>();
        let target = if host.is_empty() { "255.255.255.255".to_string() } else { host };
        ...
        runtime.block_on(async {
            ...
            match UdpSocket::bind("0.0.0.0:68").await {
                Ok(socket) => {
                    ...
                    match socket.send_to(&packet, format!("{}:{}", target, DHCP_SERVER_PORT)).await {
                        Ok(_) => {
                            let mut buf = [0u8; 1024];
    ...
    let async_request_fn =
        lua.create_function(|lua, (host, mac, requested_ip): (String, String, String)| {
            ...
            let packet = build_dhcp_packet(DHCP_REQUEST, xid, &mac_bytes, Some(&requested_ip), None, &[]);
