...
pub fn register_nmap_library(lua: &Lua) -> LuaResult<()> {
    ...
    nmap.set(
        ...
        lua.create_function(|lua, _: ()| {
            ...
            result.set("description", "Slapper NSE")?;
            Ok(result)
        })?,
    )?;
    ...
    nmap.set(
        ...
        lua.create_async_function(|lua, (socket_table, host, port): (Table, String, u16)| {
            async move {
                let result = lua.create_table()?;
    ...
    nmap.set(
        "async_socket_send",
        lua.create_async_function(|lua, (socket_table, data): (Table, String)| {
            async move {
                let result = lua.create_table()?;
    ...
    nmap.set(
        ...
        lua.create_async_function(|lua, (socket_table, size): (Table, Option<usize>)| {
            async move {
                let result = lua.create_table()?;
                let size = size.unwrap_or(1024);
            ...
            #[cfg(unix)]
            {
                ...
                if let Ok(output) = Command::new("ip").arg("addr").output() {
                    ...
                    for line in output_str.lines() {
                        if line.starts_with(|c: char| c.is_ascii_digit()) || line.starts_with("inet ") {
    ...
    nmap.set(
        ...
        lua.create_function(|lua, ()| {
            let globals = lua.globals();
            let nmap_tbl: Table = globals.get("nmap").unwrap_or_else(|_| lua.create_table().unwrap());
    ...
    nmap.set(
        "save_state",
        lua.create_function(|_lua, ()| Ok(0i32))?,
    )?;
    ...
    nmap.set(
        ...
        lua.create_function(|_lua, ()| {
            ...
            let nmap_tbl: Table = globals.get("nmap").unwrap_or_else(|_| _lua.create_table().unwrap());
