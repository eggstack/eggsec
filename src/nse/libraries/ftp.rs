...
fn ftp_retr_file(host: &str, port: u16, filename: &str) -> std::io::Result<String> {
    ...
    let mut stream = TcpStream::connect_timeout(
        &addr.parse::<std::net::SocketAddr>().map_err(|e| std::io::Error::new(std::io::ErrorKind::InvalidInput, e.to_string()))?,
    ...
    let mut data_stream = TcpStream::connect_timeout(
        &data_addr.parse::<std::net::SocketAddr>().map_err(|e| std::io::Error::new(std::io::ErrorKind::InvalidInput, e.to_string()))?,
        Duration::from_secs(10),
    )?;
    data_stream.set_read_timeout(Some(Duration::from_secs(30))).ok();
...
fn ftp_list_directory(host: &str, port: u16, path: &str) -> std::io::Result<Vec<(String, String, String)>> {
    let addr = format!("{}:{}", host, port);
    let mut stream = TcpStream::connect_timeout(
        &addr.parse::<std::net::SocketAddr>().map_err(|e| std::io::Error::new(std::io::ErrorKind::InvalidInput, e.to_string()))?,
        Duration::from_secs(10),
    ...
    if n > 0 {
        ...
        for line in content.lines() {
            ...
            if parts.len() >= 9 {
                ...
                let file_type = if perms.starts_with('d') { "DIR" } else { "FILE" }.to_string();
...
pub fn register_ftp_library(lua: &Lua) -> LuaResult<()> {
    ...
    let connect_fn = lua.create_function(|lua, (host, port): (String, u16)| {
        ...
        let n = stream.read(&mut buffer).map_err(|e| mlua::Error::RuntimeError(e.to_string()))?;
    ...
    let login_fn = lua.create_function(
        |lua, (host, port, user, pass): (String, u16, String, String)| {
            ...
            let result = lua.create_table()?;
            result.set("success", response[..3].starts_with(b"230") || response[..3].starts_with(b"202"))?;
    ...
    let list_fn = lua.create_function(|lua, (host, port, path): (String, u16, Option<String>)| {
    ...
    let nlst_fn = lua.create_function(|lua, (host, port, path): (String, u16, Option<String>)| {
        let path = path.unwrap_or_else(|| ".".to_string());
        match ftp_list_directory(&host, port, &path) {
            Ok(files) => {
                let result = lua.create_table()?;
    ...
    let retr_fn = lua.create_function(|lua, (host, port, filename): (String, u16, String)| {
    ...
    let stor_fn = lua.create_function(
        |lua, (host, port, filename, data): (String, u16, String, String)| {
    ...
    let rnto_fn = lua.create_function(
        |lua, (host, port, to_name): (String, u16, String)| {
    ...
    let mdtm_fn = lua.create_function(|lua, (host, port, filename): (String, u16, String)| {
        ...
        stream.write_all(format!("MDTM {}\r\n", filename).as_bytes()).ok();
        let mut response = vec![0u8; 4096];
        stream.read(&mut response).ok();
    ...
    let stat_fn = lua.create_function(|lua, (host, port, path): (String, u16, Option<String>)| {
        let addr = format!("{}:{}", host, port);
        let mut stream = match TcpStream::connect_timeout(
            &addr.parse::<std::net::SocketAddr>()
                .map_err(|e| mlua::Error::RuntimeError(e.to_string()))?,
        ...
        let result = lua.create_table()?;
        result.set("response", String::from_utf8_lossy(&response).trim().to_string())?;
    ...
    let type_fn = lua.create_function(|lua, (host, port, type_char): (String, u16, String)| {
        ...
        stream.write_all(format!("TYPE {}\r\n", type_char).as_bytes()).ok();
        let mut response = vec![0u8; 4096];
        stream.read(&mut response).ok();
