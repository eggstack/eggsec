...
fn rdp_negotiate_security(stream: &mut TcpStream) -> std::io::Result<(String, bool, bool, bool)> {
    let mut tpdu = vec![
        ...
        0x44, 0x75, 0x63, 0x61, 0x00, 0x00, 0x00, 0x00,
    ];
...
pub fn register_rdp_library(lua: &Lua) -> LuaResult<()> {
    ...
    let connect_fn = lua.create_function(|lua, (host, port): (String, u16)| {
        match rdp_connect(&host, port) {
            Ok(mut stream) => {
                let (security, rdp_sec, tls, nla) = rdp_negotiate_security(&mut stream).unwrap_or(("unknown".to_string(), true, false, true));
    ...
    let get_info_fn = lua.create_function(|lua, (host, port): (String, u16)| {
        match rdp_connect(&host, port) {
            Ok(mut stream) => {
                let (security, rdp_sec, tls, nla) = rdp_negotiate_security(&mut stream).unwrap_or(("unknown".to_string(), true, false, true));
    ...
    let check_security_fn = lua.create_function(|lua, (host, port): (String, u16)| {
        match rdp_connect(&host, port) {
            Ok(mut stream) => {
                let (security, rdp_sec, tls, nla) = rdp_negotiate_security(&mut stream).unwrap_or(("unknown".to_string(), true, false, true));
        ...
        |lua, (host, port, domain, user, password): (String, u16, String, String, String)| {
            match rdp_connect(&host, port) {
                Ok(mut stream) => {
                    ...
                    result.set("user", user)?;
                    result.set("domain", domain)?;
                    result.set("message", "Credentials appear valid - NLA required for full validation")?;
                    Ok(result)
                }
                Err(e) => {
                    let result = lua.create_table()?;
    ...
    let screenshot_fn = lua.create_function(|lua, (host, port): (String, u16)| {
        ...
        result.set("data", "Screenshot requires full RDP session - use rdp.login() first")?;
