...
pub fn register_dns_library(lua: &Lua) -> LuaResult<()> {
    ...
    dns.set(
        ...
        lua.create_function(|lua, (hostname, query_type): (String, Option<String>)| {
            ...
            runtime.block_on(async {
                ...
                match get_resolver().lookup(hostname_clone.clone(), record_type).await {
                    Ok(lookup) => {
                        let result = lua.create_table()?;
    ...
    dns.set(
        ...
        lua.create_function(|lua, (hostname, _server): (String, Option<String>)| {
            ...
            runtime.block_on(async {
                match get_resolver().lookup(hostname_clone.clone(), RecordType::A).await {
                    Ok(lookup) => {
                        result.set("status", "ok")?;
        ...
        lua.create_function(|lua, ip: String| {
            ...
            if let Ok(ipv4) = ip.parse::<Ipv4Addr>() {
                ...
                return runtime.block_on(async {
                    ...
                    match get_resolver().lookup(reversed_clone.clone(), RecordType::PTR).await {
                        Ok(lookup) => {
                            result.set("status", "ok")?;
