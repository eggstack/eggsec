...
macro_rules! set_fn {
    // No arguments
    ($table:expr, $name:expr, $lua:expr, |$lua_arg:ident| $body:expr) => {{
        let func = $lua.create_function(|$lua_arg| $body)?;
        $table.set($name, func)?;
    }};
    // Single argument
    ($table:expr, $name:expr, $lua:expr, |$lua_arg:ident, $arg1:ident| $body:expr) => {{
        let func = $lua.create_function(|$lua_arg, $arg1| $body)?;
        $table.set($name, func)?;
    }};
    // Multiple arguments
    ($table:expr, $name:expr, $lua:expr, |$lua_arg:ident, $($arg:ident),*| $body:expr) => {{
