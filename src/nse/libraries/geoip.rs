...
pub fn register_geoip_library(lua: &Lua) -> LuaResult<()> {
    ...
    geoip.set(
        ...
        lua.create_function(|lua, _ip: String| {
            ...
            // Try to use maxminddb if available
            #[cfg(feature = "maxmind")]
            {
                ...
                for path in &db_paths {
                    if let Ok(data) = std::fs::read(path) {
                        if let Ok(reader) = Reader::from_mmdb(&data) {
                            if let Ok(mut lookup) = reader.lookup(ip.parse().unwrap()) {
                                if let Some(country) =
                                    lookup.country().map(|c| c.code().to_string())
                                {
                                    result.set("country_code", country).ok();
                                }
                                if let Some(country) =
                                    lookup.country().map(|c| c.name().to_string())
                                {
                                    result.set("country_name", country).ok();
                                }
                                result.set("status", "ok").ok();
                                return Ok(result);
                            }
                        }
                    }
            ...
            result.set("error", "GeoIP database not available")?;
            Ok(result)
        })?,
    )?;
