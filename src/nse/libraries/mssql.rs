...
pub fn register_mssql_library(lua: &Lua) -> LuaResult<()> {
                    ...
                    match stream.read(&mut response).await {
                        Ok(n) => {
                            if n == 0 {
                                ...
                                result.set("success", true)?;
                                result.set("output", String::from_utf8_lossy(&response[..n]).to_string())?;
                            }
                        }
                        Err(e) => {
                            result.set("success", false)?;
    ...
    let get_table_names_fn = lua.create_function(|_lua, (host, port, database): (String, u16, String)| {
        let lua = mlua::Lua::default();
        let result = lua.create_table()?;
    ...
    let sp_columns_fn = lua.create_function(|_lua, (host, port, table): (String, u16, String)| {
        let lua = mlua::Lua::default();
        let result = lua.create_table()?;
    ...
    let get_type_info_fn = lua.create_function(|_lua, (host, port, data_type): (String, u16, Option<i32>)| {
        let lua = mlua::Lua::default();
        let result = lua.create_table()?;
