        ...
        lua.create_function(|lua, (host, duid): (Option<String>, Option<String>)| {
            ...
            runtime.block_on(async {
                ...
                match AsyncUdpSocket::bind("[::]:546").await {
                    Ok(socket) => {
                        match socket.send_to(&packet, format!("[{}]:{}", target, DHCPV6_SERVER_PORT)).await {
                            Ok(_) => {
                                let mut buf = [0u8; 1024];
                                match tokio::time::timeout(Duration::from_secs(5), socket.recv_from(&mut buf)).await {
                                    Ok(Ok((n, _))) => {
                                        if n > 0 && buf[0] == DHCPV6_ADVERTISE {
                                            result.set("success", true)?;
                                            result.set("transaction_id", format!("{:02x}{:02x}{:02x}", transaction_id[0], transaction_id[1], transaction_id[2]))?;
                                            result.set("type", "advertise")?;
                                        } else {
                                            result.set("success", true)?;
                                            result.set("type", "solicit_sent")?;
