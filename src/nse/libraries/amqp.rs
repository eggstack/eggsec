...
pub fn register_amqp_library(lua: &Lua) -> LuaResult<()> {
    ...
    let connect_fn = lua.create_function(|lua, (host, port, user, _password, vhost): (String, u16, String, String, Option<String>)| {
        let result = lua.create_table()?;
        
        let addr = format!("{}:{}", host, port);
        
        match TcpStream::connect_timeout(
            &addr.parse().unwrap_or_else(|_| "127.0.0.1:5672".parse().unwrap()),
    ...
    let list_queues_fn =
        lua.create_function(|lua, (_host, _port, _vhost): (String, u16, Option<String>)| {
            let result = lua.create_table()?;
    ...
    let async_connect_fn = lua.create_function(|lua, (host, port, user, _password, vhost): (String, u16, String, String, Option<String>)| {
