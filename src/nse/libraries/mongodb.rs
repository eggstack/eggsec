...
pub fn register_mongodb_library(lua: &Lua) -> LuaResult<()> {
    ...
    let find_fn = lua.create_function(|_lua, (host, port, _db, collection, _query): (String, u16, String, String, Option<String>)| {
        let addr = format!("{}:{}", host, port);
        let _stream = TcpStream::connect_timeout(
            &addr.parse::<std::net::SocketAddr>().map_err(|e: std::net::AddrParseError| mlua::Error::RuntimeError(e.to_string()))?,
            Duration::from_secs(10),
        ).map_err(|e| mlua::Error::RuntimeError(e.to_string()))?;
    ...
    let count_fn = lua.create_function(|_lua, (_host, _port, _db, _collection, _query): (String, u16, String, String, Option<String>)| {
        Ok(0)
    })?;
    ...
    let async_insert_fn = lua.create_function(|lua, (host, port, _database, collection, document): (String, u16, String, String, String)| {
        ...
        runtime.block_on(async {
            ...
            match AsyncTcpStream::connect(&addr).await {
                Ok(mut stream) => {
                    let doc = format!("{{\"insert\":\"{}\",\"documents\":[{}]}}", collection, document);
    ...
    let async_find_fn = lua.create_function(|lua, (host, port, _database, collection, query): (String, u16, String, String, String)| {
        let runtime = tokio::runtime::Handle::current();
        let host_clone = host.clone();
    ...
    let update_fn = lua.create_function(
        |lua, (host, port, _db, collection, selector, update): (String, u16, String, String, String, String)| {
            let addr = format!("{}:{}", host, port);
            let mut stream = match TcpStream::connect_timeout(
                &addr.parse::<std::net::SocketAddr>().map_err(|e| mlua::Error::RuntimeError(e.to_string()))?,
                Duration::from_secs(10),
            ) {
                Ok(s) => s,
                Err(e) => {
    ...
    let distinct_fn = lua.create_function(
        |lua, (host, port, _db, collection, field, query): (String, u16, String, String, String, Option<String>)| {
            let addr = format!("{}:{}", host, port);
            let mut stream = match TcpStream::connect_timeout(
                &addr.parse::<std::net::SocketAddr>().map_err(|e| mlua::Error::RuntimeError(e.to_string()))?,
                Duration::from_secs(10),
    ...
    let count_fn = lua.create_function(
        |lua, (host, port, _db, collection, query): (String, u16, String, String, Option<String>)| {
            let addr = format!("{}:{}", host, port);
            let mut stream = match TcpStream::connect_timeout(
                &addr.parse::<std::net::SocketAddr>().map_err(|e| mlua::Error::RuntimeError(e.to_string()))?,
                Duration::from_secs(10),
