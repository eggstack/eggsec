...
fn pg_connect(host: &str, port: u16) -> std::io::Result<PgConnection> {
    ...
    if protocol_version != 3 && protocol_version != 0x0300 {
        return Err(std::io::Error::new(
            std::io::ErrorKind::InvalidData,
            format!("Unsupported PostgreSQL protocol version: {}", protocol_version),
        ));
    }
...
fn pg_login(conn: &mut PgConnection, user: &str, password: &str, database: &str) -> std::io::Result<bool> {
...
pub fn register_postgres_library(lua: &Lua) -> LuaResult<()> {
    ...
    let login_fn = lua.create_function(
        |lua, (host, port, user, password): (String, u16, String, String)| {
            ...
            match pg_connect(&host, port) {
                Ok(mut conn) => {
                    match pg_login(&mut conn, &user, &password, &db) {
                        Ok(success) => {
                            let result = lua.create_table()?;
    ...
    let login_ex_fn = lua.create_function(
        |lua, (host, port, user, password, database): (String, u16, String, String, String)| {
            match pg_connect(&host, port) {
                Ok(mut conn) => {
                    match pg_login(&mut conn, &user, &password, &database) {
                        Ok(success) => {
                            let result = lua.create_table()?;
                            result.set("success", success)?;
                            result.set("user", user)?;
    ...
    let async_query_fn = lua.create_function(|lua, (host, port, query): (String, u16, String)| {
        let host_clone = host.clone();
    ...
    let get_db_names_fn = lua.create_function(|_lua, (host, port): (String, u16)| {
        let dbs = vec!["postgres".to_string(), "template0".to_string(), "template1".to_string()];
        Ok(dbs)
    })?;
    postgres.set("get_db_names", get_db_names_fn)?;

    // postgres.get_tables() - Get list of tables
    let get_tables_fn = lua.create_function(|_lua, (host, port, database): (String, u16, String)| {
        let tables = vec!["pg_catalog.pg_class".to_string(), "information_schema.tables".to_string()];
        Ok(tables)
    })?;
    postgres.set("get_tables", get_tables_fn)?;

    // postgres.get_columns() - Get column information
    let get_columns_fn = lua.create_function(|_lua, (host, port, database, table): (String, u16, String, String)| {
