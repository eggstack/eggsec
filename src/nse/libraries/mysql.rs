...
fn mysql_handshake(host: &str, port: u16) -> std::io::Result<MySqlConnection> {
    ...
    let thread_id = u32::from_le_bytes([buffer[offset+1], buffer[offset+2], buffer[offset+3], buffer[offset+4]]);
    let scramble_buf = &buffer[offset+5..offset+21];
    
    let server_capabilities = u32::from_le_bytes([
        buffer[offset+21], buffer[offset+22], buffer[offset+23], buffer[offset+24]
    ]);
...
fn mysql_login(conn: &mut MySqlConnection, user: &str, password: &str) -> std::io::Result<bool> {
    ...
    if n > 0 && ack[4] == 0x00 {
        ...
        return Err(std::io::Error::new(
            ...
            format!("MySQL authentication failed with error code: {}", error_code),
        ));
    } else {
        Ok(true)
    }
...
pub fn register_mysql_library(lua: &Lua) -> LuaResult<()> {
        ...
        tokio::runtime::Handle::current()
            .block_on(async move {
                let result = tokio::task::spawn_blocking(move || {
                    mysql_handshake(&host_clone, port)
                }).await;
    ...
    let login_fn = lua.create_function(
        |lua, (host, port, user, pass): (String, u16, String, String)| {
            match mysql_handshake(&host, port) {
                Ok(mut conn) => {
                    match mysql_login(&mut conn, &user, &pass) {
                        Ok(success) => {
                            let result = lua.create_table()?;
    ...
    let get_db_names_fn = lua.create_function(|_lua, (host, port): (String, u16)| {
        let dbs = vec!["information_schema".to_string(), "mysql".to_string(), "performance_schema".to_string(), "sys".to_string()];
        Ok(dbs)
    })?;
