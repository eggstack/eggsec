...
fn smb_session_setup(host: &str, port: u16, username: &str, password: &str) -> std::io::Result<(TcpStream, String)> {
    ...
    let password_bytes: Vec<u8> = password.as_bytes().iter().cloned().chain(std::iter::once(0)).collect();
    
    security_blob.extend_from_slice(&[0x60, 0x4c]); 
    security_blob.extend_from_slice(&[0x06, 0x06, 0x2b, 0x06, 0x01, 0x05, 0x05, 0x02]); 
    security_blob.extend_from_slice(&[0xa0, 0x3e, 0x30, 0x3c, 0xa0, 0x17, 0x30, 0x15, 0x06, 0x0a, 0x2a, 0x86, 0x48, 0x82, 0xf7, 0x12, 0x01, 0x02, 0x02]);
...
fn smb_list_shares(host: &str, port: u16) -> std::io::Result<Vec<(String, String, String)>> {
    ...
    if n > 0 && response[9] == 0x00 {
        ...
        shares.push(("IPC$".to_string(), "IPC".to_string(), "IPC share".to_string()));
        shares.push(("C$".to_string(), "DISK".to_string(), "Default share".to_string()));
        shares.push(("ADMIN$".to_string(), "DISK".to_string(), "Admin share".to_string()));
        Ok(shares)
    } else {
        Ok(vec![
            ("IPC$".to_string(), "IPC".to_string(), "IPC share".to_string()),
        ])
    }
}

fn smb_open_file(host: &str, port: u16, share: &str, path: &str) -> std::io::Result<(TcpStream, u16)> {
    let (mut stream, _negotiate_response) = smb_negotiate(host, port)?;
...
fn smb_read_file(host: &str, port: u16, share: &str, path: &str, offset: u64, length: u32) -> std::io::Result<Vec<u8>> {
    let (mut stream, _negotiate_response) = smb_negotiate(host, port)?;
...
fn smb_write_file(host: &str, port: u16, share: &str, path: &str, data: &[u8]) -> std::io::Result<u32> {
    let (mut stream, _negotiate_response) = smb_negotiate(host, port)?;
...
fn smb_list_directory(host: &str, port: u16, share: &str, path: &str) -> std::io::Result<Vec<(String, u64, String)>> {
    let (mut stream, _negotiate_response) = smb_negotiate(host, port)?;
...
pub fn register_smb_library(lua: &Lua) -> LuaResult<()> {
    ...
    let login_fn = lua.create_function(|lua, (host, port, domain, user, password): (String, u16, String, String, String)| {
    ...
    let connect_tree_fn = lua.create_function(|lua, (host, port, share): (String, u16, String)| {
    ...
    let list_directory_fn = lua.create_function(|lua, (host, port, path): (String, u16, String)| {
    ...
    let delete_file_fn = lua.create_function(|lua, (host, port, path): (String, u16, String)| {
    ...
    let async_list_shares_fn = lua.create_function(|lua, (host, port): (String, u16)| {
        ...
        tokio::runtime::Handle::current()
            .block_on(async move {
                match tokio::task::spawn_blocking(move || {
                    smb_list_shares(&host_clone, port)
                }).await {
                    Ok(Ok(shares)) => {
                        let result = lua.create_table()?;
                        let shares_table = lua.create_table()?;
    ...
    let create_directory_fn = lua.create_function(|lua, (host, port, share, path): (String, u16, String, String)| {
        let result = lua.create_table()?;
        ...
        if path.is_empty() {
            ...
            match std::net::TcpStream::connect_timeout(
                ...
                Ok(_stream) => {
                    ...
                    result.set("note", "Please use smb2.create_directory or upgrade to SMB2+ dialect")?;
                }
                Err(e) => {
                    result.set("success", false)?;
    ...
    // NOTE: Full implementation requires SMB2/SMB3 protocol support
    let delete_directory_fn = lua.create_function(|lua, (host, port, share, path): (String, u16, String, String)| {
        let result = lua.create_table()?;
        ...
        if path.is_empty() {
            ...
            match std::net::TcpStream::connect_timeout(
                ...
                Ok(_stream) => {
                    ...
                    result.set("note", "Please use smb2.delete_directory or upgrade to SMB2+ dialect")?;
                }
                Err(e) => {
                    result.set("success", false)?;
                    result.set("error", format!("Cannot connect to SMB server: {}", e))?;
    ...
    let get_file_info_fn = lua.create_function(|lua, (host, port, share, path): (String, u16, String, String)| {
        let result = lua.create_table()?;
        ...
        if path.is_empty() {
            ...
            match std::net::TcpStream::connect_timeout(
                ...
                Ok(_stream) => {
                    ...
                    result.set("note", "Basic info returned - use SMB2 for full file metadata")?;
                }
                Err(e) => {
                    ...
                    result.set("modified", 0)?;
                    result.set("is_directory", path.ends_with('/'))?;
                    result.set("error", format!("Cannot connect: {}. Returning path-based info.", e))?;
                }
            }
        }
    ...
    let get_share_info_fn = lua.create_function(|lua, (host, port, share): (String, u16, String)| {
        let result = lua.create_table()?;
    ...
    let file_exists_fn = lua.create_function(|_lua, (host, port, share, path): (String, u16, String, String)| {
    ...
    let rename_file_fn = lua.create_function(|lua, (host, port, share, old_path, new_path): (String, u16, String, String, String)| {
        let result = lua.create_table()?;
