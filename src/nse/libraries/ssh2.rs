...
impl SshSession {
    fn connect(host: &str, port: u16) -> std::io::Result<Self> {
        ...
        session.handshake().map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e))?;
...
impl UserData for SshSession {
    fn add_methods<M: UserDataMethods<Self>>(methods: &mut M) {
        ...
        methods.add_method_mut("userauth_password", |_lua, this, (username, password): (String, String)| {
            Ok(this.authenticate_password(&username, &password))
        });

        methods.add_method_mut("userauth_publickey", |_lua, this, (username, _public_key, password): (String, String, Option<String>)| {
        ...
        methods.add_method("channel_write", |_lua, _this, _data: String| {
            Ok(0i32)
        });

        methods.add_method("channel_close", |_lua, _this, _: ()| {
            Ok(true)
        });

        methods.add_method("get_channel_exit_status", |_lua, _this, _: ()| {
            Ok(0i32)
        });

        methods.add_method("channel_request_pty", |_lua, _this, _: ()| {
            Ok(true)
        });

        methods.add_method("channel_request_shell", |_lua, _this, _: ()| {
            Ok(true)
        });

        methods.add_method("get.banner", |_lua, this, _: ()| {
        ...
        methods.add_method("session_flag", |_lua, _this, (_flag, _value): (String, bool)| {
            Ok(true)
        });
        ...
        methods.add_method("channel_forward_login", |lua, this, (_host, _user): (String, String)| {
            let chan = lua.create_table()?;
            chan.set("session", this.host.clone())?;
            chan.set("local_channel", 0i32)?;
        ...
        methods.add_method("channel_forward_listen", |lua, this, (_host, _port): (Option<String>, Option<u32>)| {
            let chan = lua.create_table()?;
            chan.set("session", this.host.clone())?;
            chan.set("port", 0i32)?;
        ...
        methods.add_method("channel_accept", |lua, _this, (_channel, _timeout): (Value, i32)| {
            let chan = lua.create_table()?;
        ...
        tokio::runtime::Handle::current()
            .block_on(async {
                let result = tokio::task::spawn_blocking(move || {
                    SshSession::connect(&host_clone, port)
                }).await;
...
pub fn register_ssh2_library(lua: &Lua) -> LuaResult<()> {
    ...
    ssh2.set(
        ...
        lua.create_function(|lua, (host, port): (String, Option<u16>)| {
            ...
            match TcpStream::connect_timeout(
                &addr.parse().map_err(|e| std::io::Error::new(std::io::ErrorKind::InvalidInput, e.to_string()))?,
                Duration::from_secs(10),
            ) {
                Ok(mut stream) => {
    ...
    let channel_fn = lua.create_function(|lua, _: mlua::Value| {
        let channel = lua.create_table()?;
        channel.set("error", "SSH2 support not compiled. Enable 'ssh2' feature for full support.")?;
