...
use crate::proxy::{ProxyManager, ProxyEntry, ProxyType};
...
pub async fn run_http_flood(config: &StressConfig, metrics: &StressMetrics) -> Result<StressStats> {
    ...
    progress.set_style(
        ProgressStyle::default_bar()
            .template("{spinner:.red} [{elapsed_precise}] [{bar:40.red/yellow}] {pos}/{len}s - {msg}")
            .unwrap()
            .progress_chars("=>-"),
    );
...
async fn build_client(config: &StressConfig, proxy_manager: Option<&ProxyManager>) -> Result<reqwest::Client> {
