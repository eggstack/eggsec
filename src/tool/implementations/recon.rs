...
impl SecurityTool for ReconTool {
    ...
    async fn execute(&self, request: ToolRequest) -> ToolResult<ToolResponse> {
        ...
        let no_tech = params.get("no_tech").and_then(|v| v.as_bool()).unwrap_or(false);
        let no_dns = params.get("no_dns").and_then(|v| v.as_bool()).unwrap_or(false);
        let no_geo = params.get("no_geo").and_then(|v| v.as_bool()).unwrap_or(false);
        let no_whois = params.get("no_whois").and_then(|v| v.as_bool()).unwrap_or(false);
        let no_subdomains = params.get("no_subdomains").and_then(|v| v.as_bool()).unwrap_or(false);
        let no_ssl = params.get("no_ssl").and_then(|v| v.as_bool()).unwrap_or(false);
        let no_cve = params.get("no_cve").and_then(|v| v.as_bool()).unwrap_or(false);
        ...
        let args = crate::cli::ReconArgs {
            ...
            no_subdomains,
            no_ssl,
            no_dns_records: params.get("no_dns_records").and_then(|v| v.as_bool()).unwrap_or(false),
            no_js: params.get("no_js").and_then(|v| v.as_bool()).unwrap_or(false),
            no_content: params.get("no_content").and_then(|v| v.as_bool()).unwrap_or(false),
            no_cloud: params.get("no_cloud").and_then(|v| v.as_bool()).unwrap_or(false),
            no_wayback: params.get("no_wayback").and_then(|v| v.as_bool()).unwrap_or(false),
            no_cors: params.get("no_cors").and_then(|v| v.as_bool()).unwrap_or(false),
            no_threat: params.get("no_threat").and_then(|v| v.as_bool()).unwrap_or(false),
            no_cve,
            no_email: params.get("no_email").and_then(|v| v.as_bool()).unwrap_or(false),
