...
impl SecurityTool for FuzzerTool {
    ...
    async fn execute(&self, request: ToolRequest) -> ToolResult<ToolResponse> {
        ...
        let mutation_count = params.get("mutation_count").and_then(|v| v.as_u64()).unwrap_or(3) as usize;
        let session = params.get("session").and_then(|v| v.as_bool()).unwrap_or(false);
        let waf_fingerprint = params.get("waf_fingerprint").and_then(|v| v.as_bool()).unwrap_or(false);
        ...
        let param = params.get("param").and_then(|v| v.as_str()).map(String::from);
        ...
        let args = crate::cli::FuzzArgs {
            ...
            mutate,
            mutation_count,
            grammar_fuzz: params.get("grammar_fuzz").and_then(|v| v.as_bool()).unwrap_or(false),
            grammar_type: params.get("grammar_type").and_then(|v| v.as_str()).map(String::from),
            adaptive_rate: params.get("adaptive_rate").and_then(|v| v.as_bool()).unwrap_or(false),
            session,
            diffing: params.get("diffing").and_then(|v| v.as_bool()).unwrap_or(false),
            capture_baseline: params.get("capture_baseline").and_then(|v| v.as_bool()).unwrap_or(false),
            enhanced_redos: params.get("enhanced_redos").and_then(|v| v.as_bool()).unwrap_or(false),
            waf_fingerprint,
            chaining: params.get("chaining").and_then(|v| v.as_bool()).unwrap_or(false),
            chain_file: params.get("chain_file").and_then(|v| v.as_str()).map(String::from),
            method,
            param,
            concurrency,
