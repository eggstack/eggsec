...
use crate::tool::{ToolRegistry, ToolDispatcher, ToolRequest, ToolResponse, Target, TargetType, RequestOptions};
...
impl GrpcService {
    ...
    pub async fn list_tools(&self, category: Option<String>) -> Result<ListToolsResponse, String> {
        ...
        let tool_infos: Vec<GrpcToolInfo> = tools.into_iter().map(|t| GrpcToolInfo {
            id: t.id,
            name: t.name,
            category: t.category.to_string(),
            description: t.description,
            protocols: t.protocols,
            capabilities: t.capabilities.into_iter().map(|c| GrpcToolCapability {
                name: c.name,
                description: c.description,
                parameters: c.parameters.into_iter().map(|p| GrpcParameterDef {
                    name: p.name,
        ...
        let categories: Vec<String> = self.registry.categories()
            .iter()
            .map(|c| c.to_string())
            .collect();
    ...
    pub async fn execute_tool(&self, req: ExecuteToolRequest) -> Result<ExecuteToolResponse, String> {
        ...
        let options = req.options.map(|o| RequestOptions {
        ...
        Ok(ExecuteToolResponse {
            ...
            errors: response.errors.into_iter().map(|e| GrpcToolError {
                code: e.code,
                message: e.message,
                details: e.details,
                target: e.target,
            }).collect(),
            findings: response.findings.into_iter().map(|f| GrpcFinding {
                id: f.id,
    ...
    pub async fn get_capabilities(&self, tool_id: Option<String>) -> Result<CapabilitiesResponse, String> {
        if let Some(id) = tool_id {
            ...
            let tools = self.registry.list();
            let caps: Vec<GrpcToolCapability> = tools.into_iter()
                .flat_map(|t| t.capabilities.into_iter().map(|c| GrpcToolCapability {
                    name: c.name,
                    description: c.description,
                    parameters: c.parameters.into_iter().map(|p| GrpcParameterDef {
                        name: p.name,
