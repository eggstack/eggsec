use anyhow::anyhow;
use crate::cli::{
    PacketCaptureArgs, PacketDumpArgs, PacketSendArgs, PacketTracerouteArgs,
};
...
use crate::packet::{PacketInfo, PacketCapture, CaptureConfig};
use std::fs::File;
...
pub async fn handle_packet_capture(args: PacketCaptureArgs, json: bool) -> Result<(), anyhow::Error> {
    ...
    #[cfg(all(feature = "packet-inspection", unix))]
    {
        ...
        if args.interface.is_none() {
            ...
            for iface in list_interfaces() {
                println!("  - {} ({}): {:?}", iface.name, iface.ips.join(", "), iface.mac);
            }
            return Ok(());
        }

        let config = CaptureConfig {
            interface: args.interface.unwrap(),
            filter: args.filter,
...
#[cfg(all(feature = "packet-inspection", unix))]
fn create_datalink_channel(interface: &NetworkInterface) -> Result<
    (Box<dyn datalink::DataLinkSender>, Box<dyn datalink::DataLinkReceiver>),
    anyhow::Error
> {
...
fn dump_pcap(reader: &mut BufReader<File>, args: PacketDumpArgs, json: bool) -> Result<(), anyhow::Error> {
    use std::io::Seek;
    ...
    loop {
        ...
        let ts_sec = u32::from_le_bytes([pkt_header[0], pkt_header[1], pkt_header[2], pkt_header[3]]);
        let incl_len = u32::from_le_bytes([pkt_header[12], pkt_header[13], pkt_header[14], pkt_header[15]]);
            ...
            if !args.hex_only {
                if let Some(info) = parse_and_print_packet(&payload) {
                    if args.headers_only {
                        ...
                        println!("{}\n", info);
                        println!("{}", hexdump::hexdump_with_offset(&payload, 0, bytes_per_line));
                    }
                } else {
                    println!("{}", hexdump::hexdump_with_offset(&payload, 0, bytes_per_line));
                }
    ...
    if let Some(idx) = index {
        if !packet_data.is_empty() {
            if !args.hex_only {
                ...
                println!("{}", hexdump::hexdump_with_offset(&packet_data, 0, bytes_per_line));
            }
        }
    }
...
fn dump_raw_packet(data: &[u8], args: PacketDumpArgs, json: bool) -> Result<(), anyhow::Error> {
    ...
    if let Some(idx) = args.index {
        ...
        println!("{}", hexdump::hexdump_with_offset(chunk, start, bytes_per_line));
    } else {
            ...
            if !args.hex_only {
                if let Some(info) = parse_and_print_packet(chunk) {
                    if args.headers_only {
                        ...
                        println!("{}\n", info);
                        println!("{}", hexdump::hexdump_with_offset(chunk, offset, bytes_per_line));
                    }
                } else {
                    println!("{}", hexdump::hexdump_with_offset(chunk, offset, bytes_per_line));
                }
...
pub async fn handle_packet_traceroute(args: PacketTracerouteArgs, json: bool) -> Result<(), anyhow::Error> {
    use crate::utils::is_root;
    ...
    if use_icmp {
        ...
        {
            #[cfg(any(all(feature = "packet-inspection", unix), all(feature = "stress-testing", unix)))]
            {
                if !is_root() {
            ...
            #[cfg(not(any(all(feature = "packet-inspection", unix), all(feature = "stress-testing", unix))))]
            {
...
fn parse_and_print_packet(data: &[u8]) -> Option<String> {
    ...
    if let Some(ref ip) = parsed.ip {
        output.push_str(&format!("{}: {} → {}\n", ip.protocol_name, ip.src_ip(), ip.dst_ip()));
    ...
    if let Some(ref trans) = parsed.transport {
        match trans {
            ...
            crate::packet::TransportProtocol::Icmp(icmp) => {
                output.push_str(&format!("ICMP: type={}, code={}\n", icmp.icmp_type, icmp.icmp_code));
            }
...
fn print_traceroute_result(result: &TracerouteResult, json: bool) {
    ...
    println!("traceroute to {} ({}), {} hops max\n", 
        result.target, result.resolved_address, result.hops.len());
