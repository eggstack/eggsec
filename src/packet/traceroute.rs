...
impl Traceroute {
    ...
    async fn probe_hop_icmp_parallel(&self, target: IpAddr, ttl: u8) -> TracerouteHop {
        ...
        for _ in 0..self.config.max_retries {
            ...
            let handle: tokio::task::JoinHandle<(Option<IpAddr>, Option<Duration>)> = tokio::spawn(async move {
    ...
    async fn probe_icmp(&self, target: IpAddr, _ttl: u8) -> Result<IpAddr, ProbeError> {
        #[cfg(all(feature = "stress-testing", unix))]
        {
            use std::net::ToSocketAddrs;
            ...
            let result = tokio::time::timeout(
                timeout,
                async {
                    surge_ping::ping(target, &payload).await
                }
            ).await;
