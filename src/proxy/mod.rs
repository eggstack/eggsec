...
pub use config::{ProxyConfig, ProxyType, ProxyEntry, HealthCheckConfig};
...
impl ProxyManager {
    ...
    pub async fn get_next_proxy(&self) -> Option<ProxyEntry> {
        ...
        self.rotator.select_with_stats(&proxies, &|key| pool_ref.get_stats(key))
    }
    ...
    pub async fn get_healthy_proxy(&self) -> Option<ProxyEntry> {
        ...
        self.rotator.select_with_stats(&healthy, &|key| pool_ref.get_stats(key))
    }
    ...
    pub async fn get_highest_priority_proxy(&self, min_priority: u8) -> Option<ProxyEntry> {
        ...
        let pool_ref = &pool;
        self.rotator.select_with_stats(&by_priority, &|key| pool_ref.get_stats(key))
    }
    ...
    pub async fn create_connection(&self, target: &str) -> Result<ProxiedConnection> {
        let proxy = self.get_healthy_proxy().await
        ...
        match proxy.proxy_type {
            ...
            ProxyType::Tor => {
                socks::connect_through_tor(proxy, target_addr).await
            }
        }
    }

    pub async fn create_connection_to_domain(&self, domain: &str, port: u16) -> Result<ProxiedConnection> {
        let proxy = self.get_healthy_proxy().await
            .ok_or_else(|| anyhow::anyhow!("No healthy proxies available"))?;
        
        connect_through_with_domain(&proxy, domain, port).await.map(|stream| {
            ProxiedConnection {
                proxy_chain: vec![proxy],
                local_addr: stream.local_addr().unwrap_or_else(|_| "0.0.0.0:0".parse().unwrap()),
                target_addr: format!("{}:{}", domain, port).parse().unwrap_or_else(|_| "0.0.0.0:0".parse().unwrap()),
            }
        })
    }

    pub async fn create_chained_connection(&self, target: &str, chain_length: usize) -> Result<ProxiedConnection> {
        let pool = self.pool.read().await;
        let proxies = pool.get_healthy();
        
        if proxies.len() < chain_length {
            anyhow::bail!("Not enough healthy proxies for chaining (have {}, need {})", proxies.len(), chain_length);
        }
        
        let chain = self.rotator.select_chain(&proxies, chain_length)
    ...
    pub async fn start_background_health_check(
        &self,
        interval_secs: u64,
    ) -> JoinHandle<()> {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                tracing::debug!("Running background health check for {} proxies", proxies.len());
...
mod tests {
    ...
    fn test_proxy_type_parsing() {
        assert!(matches!(ProxyType::from_str("socks5"), Ok(ProxyType::Socks5)));
        assert!(matches!(ProxyType::from_str("socks4"), Ok(ProxyType::Socks4)));
