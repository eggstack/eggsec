...
pub use profiles::{WafProfile, ProfileBypass, get_waf_profiles, get_profile_by_name, get_auto_profile};
...
impl BypassEngine {
    ...
    pub async fn run_bypasses(&self, detection: &WafDetectionResult) -> Result<Vec<BypassResult>> {
        ...
        if self.args.header_bypass || self.args.bypass {
            ...
            results.extend(header_bypass.run(&self.client, &self.args.url, detection, self.test_type).await?);
        }

        if self.args.evasion || self.args.bypass {
            let evasion_bypass = EvasionBypass::new(profile.cloned());
            results.extend(evasion_bypass.run(&self.client, &self.args.url, detection, self.test_type).await?);
        }

        if self.args.smuggling || self.args.bypass {
            let smuggling_bypass = SmugglingBypass::new(profile.cloned());
            results.extend(smuggling_bypass.run(&self.client, &self.args.url, detection).await?);
        }
