...
use super::{BypassResult, BypassTechnique, WafProfile, ProfileBypass, TestType};
...
impl HeaderBypass {
    ...
    pub async fn run(&self, client: &Client, url: &str, detection: &WafDetectionResult, _test_type: TestType) -> Result<Vec<BypassResult>> {
        let mut results = Vec::new();
        
        if let Some(ref profile) = self.profile {
            for bypass in &profile.bypasses {
                if matches!(bypass.technique, BypassTechnique::HeaderManipulation | BypassTechnique::UserAgentRotation | BypassTechnique::XForwardedForSpoof | BypassTechnique::ContentTypeBypass) {
                    let result = self.test_profile_bypass(client, url, bypass, detection).await?;
                    results.push(result);
                }
            }
        } else {
            for header_set in self.generate_header_bypasses() {
                let result = self.test_header_set(client, url, &header_set, detection).await?;
                results.push(result);
            }
        }
    ...
    async fn test_profile_bypass(&self, client: &Client, url: &str, bypass: &ProfileBypass, _detection: &WafDetectionResult) -> Result<BypassResult> {
    ...
    async fn test_header_set(&self, client: &Client, url: &str, header_set: &HeaderSet, detection: &WafDetectionResult) -> Result<BypassResult> {
        let mut request = client.get(url);
...
pub fn get_random_ua() -> &'static str {
    get_user_agents().choose(&mut rand::thread_rng()).unwrap()
}
