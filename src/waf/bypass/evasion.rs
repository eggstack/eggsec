...
use super::{BypassResult, BypassTechnique, WafProfile, TestType};
use crate::waf::detector::WafDetectionResult;
use crate::waf::payloads::encoding::{
    get_sqli_payloads, get_xss_payloads, get_ssrf_payloads, 
    get_traversal_payloads, get_command_injection_payloads,
    get_waf_test_payloads, BypassType
};
...
impl EvasionBypass {
    ...
    pub async fn run(&self, client: &Client, url: &str, detection: &WafDetectionResult, test_type: TestType) -> Result<Vec<BypassResult>> {
        ...
        if let Some(ref profile) = self.profile {
            for bypass in &profile.bypasses {
                ...
                if matches!(technique, 
                    BypassTechnique::EncodingBypass | BypassTechnique::Homoglyph | 
                    BypassTechnique::ZeroWidthInjection | BypassTechnique::CaseRotation |
                    BypassTechnique::UnicodeEncoding | BypassTechnique::CommentObfuscation |
                    BypassTechnique::WhitespaceVariation | BypassTechnique::DoubleEncoding) 
                {
                    for payload in &bypass.payloads {
                        let result = self.test_payload(client, url, payload, technique, bypass.description.clone(), detection).await?;
                        results.push(result);
                    }
                }
            }
            ...
            for (technique, payload, description) in test_payloads {
                let result = self.test_payload(client, url, &payload, technique, description, detection).await?;
                results.push(result);
            }
    ...
    fn generate_evasion_payloads(&self, test_type: TestType) -> Vec<(BypassTechnique, String, String)> {
        ...
        if test_type == TestType::All || test_type == TestType::Traversal {
            for traversal in get_traversal_payloads().iter().take(5) {
                payloads.push((BypassTechnique::EncodingBypass,
                    apply_double_encoding(traversal),
                    format!("Traversal Double Encoding: {}", &traversal[..30.min(traversal.len())])));
            }
        }
    ...
    fn generate_structured_payloads(&self, test_type: TestType) -> Vec<(BypassTechnique, String, String)> {
        let mut payloads = Vec::new();
        ...
        for waf_payload in get_waf_test_payloads() {
            let should_include = match test_type {
                ...
                TestType::Cmd => waf_payload.bypass_types.contains(&BypassType::CommandInjection),
                TestType::Traversal => waf_payload.bypass_types.contains(&BypassType::PathTraversal),
            };
    ...
    async fn test_payload(&self, client: &Client, url: &str, payload: &str, technique: BypassTechnique, description: String, detection: &WafDetectionResult) -> Result<BypassResult> {
    ...
    fn is_bypass_successful(&self, status: u16, _detection: &WafDetectionResult) -> bool {
        ...
        !blocked_codes.contains(&status) && status < 400
    }
}
