...
use tokio::sync::mpsc;
use crate::distributed::{Task, TaskResult, TaskType, Heartbeat, WorkerStatus, WorkerRegistration};
...
async fn process_task(task: Task) -> Result<TaskResult> {
    ...
    let output = result.as_ref().ok().map(|o| serde_json::to_string(o).unwrap_or_default()).unwrap_or_default();
...
async fn process_port_scan(task: Task) -> Result<serde_json::Value> {
    ...
    let ports = task.payload.get("ports")
        .and_then(|v| v.as_str())
        .unwrap_or("1-1000");
    let concurrency: usize = task.payload.get("concurrency")
        .and_then(|v| v.as_u64())
        .unwrap_or(100) as usize;
    let timeout: u64 = task.payload.get("timeout")
        .and_then(|v| v.as_u64())
        .unwrap_or(5);
...
async fn process_fingerprint(task: Task) -> Result<serde_json::Value> {
    let target = &task.target;
    let ports: Vec<u16> = task.payload.get("ports")
        ...
        .unwrap_or_else(|| {
            vec![21, 22, 23, 25, 53, 80, 110, 143, 443, 445, 993, 995, 1433, 1521, 3306, 3389, 5432, 5900, 6379, 8080, 8443, 27017]
        });
    let timeout: u64 = task.payload.get("timeout")
        .and_then(|v| v.as_u64())
        .unwrap_or(5);
...
async fn process_endpoints(task: Task) -> Result<serde_json::Value> {
    let target = &task.target;
    let wordlist = task.payload.get("wordlist")
        .and_then(|v| v.as_str())
        .map(|w| {
...
async fn process_fuzz(task: Task) -> Result<serde_json::Value> {
    let target = &task.target;
    let payload_type = task.payload.get("payload_type")
        .and_then(|v| v.as_str())
        .unwrap_or("all");
    let concurrency: usize = task.payload.get("concurrency")
        .and_then(|v| v.as_u64())
    ...
    let findings: Vec<_> = session.results.iter().take(50).map(|f| {
        serde_json::json!({
            "payload": f.payload,
...
async fn process_waf(task: Task) -> Result<serde_json::Value> {
    let target = &task.target;
    let detect_only = task.payload.get("detect_only")
...
async fn process_load_test(task: Task) -> Result<serde_json::Value> {
    let target = &task.target;
    let requests: u64 = task.payload.get("requests")
        .and_then(|v| v.as_u64())
        .unwrap_or(100);
    let concurrency: usize = task.payload.get("concurrency")
        .and_then(|v| v.as_u64())
        .unwrap_or(10) as usize;
    let method = task.payload.get("method")
        .and_then(|v| v.as_str())
        .unwrap_or("GET");
...
async fn process_recon(task: Task) -> Result<serde_json::Value> {
    let target = &task.target;
    let no_tech = task.payload.get("no_tech")
        .and_then(|v| v.as_bool())
        .unwrap_or(false);
    let no_dns = task.payload.get("no_dns")
        .and_then(|v| v.as_bool())
        .unwrap_or(false);
