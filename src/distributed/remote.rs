...
use crate::distributed::io::{StreamWrapper, LineWriter, TlsServer, TlsClient};
...
impl RemoteListener {
    ...
    pub fn with_tls(psk: String, tls_config: TlsConfig) -> Self {
        let tls_server = match TlsServer::from_pkcs12(&tls_config.pkcs12_path, &tls_config.password) {
            ...
            Err(e) => {
                tracing::warn!("Failed to initialize TLS: {}. Falling back to plaintext.", e);
                None
            }
        };
    ...
    pub async fn start(&self, port: u16) -> anyhow::Result<()> {
        ...
        tracing::info!("Max connections: {}, Rate limit: {}/min", self.max_connections, self.rate_limit);
    ...
    async fn handle_connection(
        ...
        let stream = match tls_acceptor {
            Some(acceptor) => {
                match StreamWrapper::accept_tls(&acceptor, stream).await {
                    Ok(s) => s,
                    Err(e) => {
        ...
        let auth: AuthMessage = serde_json::from_str(&auth_line.ok_or_else(|| anyhow::anyhow!("No auth"))?)?;
        ...
        if !bool::from(auth.psk.as_bytes().ct_eq(psk.as_bytes())) {
            ...
            line_writer.write_line(&serde_json::to_string(&error)?).await?;
        ...
        line_writer.write_line(&serde_json::to_string(&welcome)?).await?;
        ...
        loop {
            ...
            let request: CommandMessage = match serde_json::from_str(&line) {
                Ok(req) => req,
                Err(e) => {
                    let error = ResponseMessage::error("unknown".to_string(), format!("Invalid request: {}", e), None);
                    line_writer.write_line(&serde_json::to_string(&error)?).await?;
                    continue;
                }
            };
            ...
            match request {
                CommandMessage::Execute { id, command, timeout, env } => {
                    ...
                    match CommandExecutor::execute(command, timeout, env).await {
                        Ok((output, duration_ms)) => {
                            ...
                            line_writer.write_line(&serde_json::to_string(&response)?).await?;
                        }
                        Err(e) => {
                ...
                CommandMessage::Register { id, hostname, capabilities } => {
...
impl RemoteClient {
    ...
    pub fn with_tls(psk: String, domain: &str) -> anyhow::Result<Self> {
        let tls = match TlsClient::new(domain) {
            Ok(c) => Some(c),
            Err(e) => {
                tracing::warn!("Failed to initialize TLS client: {}. Falling back to plaintext.", e);
                None
            }
        };
        Ok(Self {
    ...
    pub async fn execute(
        ...
        line_writer.write_line(&serde_json::to_string(&auth)?).await?;

        // Auth response timeout
        let auth_response: ResponseMessage = tokio::time::timeout(
            std::time::Duration::from_secs(10),
            async {
                let line = line_writer.read_line().await?.ok_or_else(|| anyhow::anyhow!("No response"))?;
        ...
        if !auth_response.success {
            return Err(anyhow::anyhow!("Authentication failed: {:?}", auth_response.error));
        }
        ...
        line_writer.write_line(&serde_json::to_string(&cmd)?).await?;
        ...
        let response: ResponseMessage = tokio::time::timeout(
            response_timeout,
            async {
                let response_line = line_writer.read_line().await?.ok_or_else(|| anyhow::anyhow!("No response"))?;
                Ok::<_, anyhow::Error>(serde_json::from_str::<ResponseMessage>(&response_line)?)
            }
        )
        .await
        .map_err(|_| anyhow::anyhow!("Response timed out after {} seconds", response_timeout.as_secs()))??;
