...
use super::types::{MsfError, MsfResponse, ModuleType};
use super::module::{ModuleInfo, ModuleExecutionResult};
...
impl MsfClient {
    ...
    pub async fn connect(&mut self) -> Result<()> {
        ...
        let result: MsfResponse = response.json().await
    ...
    pub async fn list_modules(&self, module_type: ModuleType) -> Result<Vec<String>> {
        ...
        let response: ModulesResponse = self.request(
            "module.exploits",
            serde_json::json!({ "module_type": type_str }),
        ).await?;
    ...
    pub async fn list_sessions(&self) -> Result<HashMap<String, Session>> {
        let response: SessionsResponse = self.request(
            "session.list",
            serde_json::json!({}),
        ).await?;
    ...
    pub async fn execute_session_command(
        ...
        let response: CommandResponse = self.request(
            "session.shell_write",
            serde_json::json!({
    ...
    pub async fn generate_payload(
        ...
        let response: PayloadResponse = self.request(
            "module.execute",
            serde_json::json!({
                "module_type": "payload",
    ...
    pub async fn stop_job(&self, job_id: &str) -> Result<()> {
        self.request("job.stop", serde_json::json!({ "job_id": job_id })).await
    }
}
