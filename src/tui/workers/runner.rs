        ...
        match operation().await {
            ...
            Err(e) => {
                ...
                if is_retryable_error(&error_str) && attempt < max_retries {
                    ...
                    tracing::warn!("Attempt {} failed, retrying in {} seconds...", attempt, delay);
...
impl TaskRunner {
    ...
    pub async fn run(self) -> anyhow::Result<()> {
        ...
        let result = match self.config {
            TaskConfig::LoadTest { target, requests, concurrency, timeout } => {
                Self::run_load_test(target, requests, concurrency, timeout, progress_tx, result_tx).await
            }
            TaskConfig::StressTest { target, stress_type, rate, duration, concurrency } => {
                Self::run_stress_test(target, stress_type, rate, duration, concurrency, progress_tx, result_tx).await
            }
            TaskConfig::PortScan { target, ports, concurrency, timeout } => {
                Self::run_port_scan(target, ports, concurrency, timeout, progress_tx, result_tx).await
            }
            TaskConfig::EndpointScan { target, concurrency, timeout, wordlist } => {
                Self::run_endpoint_scan(target, concurrency, timeout, wordlist, progress_tx, result_tx).await
            }
            TaskConfig::Fingerprint { target, ports, timeout } => {
                Self::run_fingerprint(target, ports, timeout, progress_tx, result_tx).await
            }
            TaskConfig::Fuzz { target, payload_type, mode, mutations, mutation_count, method, param, concurrency, timeout, graphql_introspection, graphql_depth_bypass, graphql_alias_overload, oauth_redirect_test, oauth_scope_test, oauth_state_test, oauth_grant_test } => {
                Self::run_fuzz(target, payload_type, mode, mutations, mutation_count, method, param, concurrency, timeout, graphql_introspection, graphql_depth_bypass, graphql_alias_overload, oauth_redirect_test, oauth_scope_test, oauth_state_test, oauth_grant_test, progress_tx, result_tx).await
            }
            TaskConfig::Waf { target, bypass_mode, techniques } => {
                Self::run_waf(target, bypass_mode, techniques, progress_tx, result_tx).await
            }
            TaskConfig::Pipeline { target, profile, output_file, output_format } => {
                Self::run_pipeline(target, profile, output_file, output_format, progress_tx, result_tx).await
            }
            TaskConfig::Recon { target, concurrency, options } => {
                Self::run_recon(target, concurrency, options, progress_tx, result_tx).await
            }
            TaskConfig::PacketCapture { interface, filter, max_packets, output_file } => {
                Self::run_packet_capture(interface, filter, max_packets, output_file, progress_tx, result_tx).await
            }
            TaskConfig::PacketTraceroute { target, max_hops } => {
                Self::run_packet_traceroute(target, max_hops, progress_tx, result_tx).await
            }
            TaskConfig::PacketSend { target, port, count, packet_size } => {
                Self::run_packet_send(target, port, count, packet_size, progress_tx, result_tx).await
            }
        };
        result
    }
    ...
    async fn run_stress_test(
        ...
        let _ = result_tx.send(TaskResult::StressTest { target: target.clone(), stats: stats.clone() }).await;
    ...
    async fn run_port_scan(
        ...
        let results = scan_ports(&target, port_list, concurrency, timeout, true, SpoofConfig::default()).await?;
    ...
    async fn run_endpoint_scan(
        ...
        let results = scan_endpoints(&target, endpoints, concurrency, timeout, false, true, SpoofConfig::default()).await?;
    ...
    async fn run_waf(
        ...
        if bypass_mode {
            ...
            use crate::waf::{BypassEngine, get_auto_profile, TestType};
            ...
            let _ = result_tx.send(TaskResult::WafBypass { detection, bypasses }).await;
        } else {
    ...
    async fn run_pipeline(
        ...
        let args = ScanArgs {
            ...
            concurrency: 10,
            json: false,
            output: if output_file.is_empty() { None } else { Some(output_file) },
    ...
    async fn run_recon(
            ...
            match run_full_recon(&args, &config, stage, false).await {
                ...
                Err(e) => {
                    ...
                    if is_retryable && attempt < max_retries {
                        let delay = base_delay_secs * 2u64.pow(attempt - 1);
                        tracing::warn!("Recon attempt {} failed, retrying in {} seconds...", attempt, delay);
                        let _ = progress_tx.send(((attempt as u64) * 20, 100)).await;
                        tokio::time::sleep(tokio::time::Duration::from_secs(delay)).await;
                    } else {
                        break;
        ...
        tracing::error!("Recon failed after {} attempts: {:?}", max_retries, last_error);
        Err(last_error.unwrap_or_else(|| anyhow::anyhow!("Unknown error")))
    }
    ...
    async fn run_packet_capture(
        ...
        use crate::packet::capture::{CaptureBuilder, CaptureConfig};
        ...
        let capture = CaptureBuilder::new(config)
            .map_err(|e| anyhow::anyhow!("Failed to create capture: {}", e))?;
        ...
        for packet in capture {
            captured += 1;
            let _ = progress_tx.send((captured as u64, max_packets as u64)).await;
            if captured >= max_packets {
                break;
            }
        }
        
        let _ = result_tx.send(TaskResult::PacketCapture {
    ...
    async fn run_packet_traceroute(
        ...
        let _socket_addr = addr.to_socket_addrs()
        ...
        let hops: Vec<TracerouteHopResult> = result.hops.iter().map(|h| {
            TracerouteHopResult {
                hop: h.hop,
    ...
    async fn run_packet_send(
        ...
        use crate::stress::syn::SynFlooder;
        use std::net::ToSocketAddrs;
        ...
        let _ = result_tx.send(TaskResult::PacketSend {
            packets_sent: sent,
            bytes_sent: bytes,
        }).await;
