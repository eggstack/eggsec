...
impl PacketTab {
    pub fn new() -> Self {
        ...
        let is_root = crate::utils::privilege::is_root();
    ...
    pub fn run_traceroute(&mut self) {
        ...
        self.results_view.add_line(Line::from(format!(
            "Use CLI for traceroute: slapper packet traceroute {}",
            target
        )));
    ...
    pub fn run_send(&mut self) {
        ...
        self.results_view.add_line(Line::from(""));
        self.results_view.add_line(Line::from(format!(
            "Use CLI for packet sending: slapper packet send {} --dst-port {}",
            target,
            self.filter()
        )));
                ...
                Ok(mut file) => {
                    ...
                    if file.read_to_end(&mut buffer).is_ok() {
                        ...
                        for line in dump.lines().take(100) {
                            self.results_view.add_line(Line::from(line));
                        }
                    } else {
                        self.results_view.add_line(Line::from(vec![
                            Span::styled("Error: ", Style::default().fg(Color::Red)),
    ...
    fn handle_enter(&mut self) {
        if self.view_selector.is_focused() {
            ...
            self.current_view = match self.view_selector.selected {
                ...
                1 => PacketView::Send,
                2 => PacketView::Dump,
                3 => PacketView::Traceroute,
                4 => PacketView::Interfaces,
