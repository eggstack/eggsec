...
impl ProxyTab {
    ...
    pub fn load_proxies_from_file(&mut self, path: &str) -> Result<usize, String> {
        ...
        self.proxies = entries.iter().map(|p| ProxyConfigEntry {
    ...
    pub async fn run_health_check(&mut self, test_url: &str) {
        self.state = AppState::Running;
        self.results_view.clear();
        self.results_view.add_line(Line::from("Running health checks..."));
        
        let proxy_entries: Vec<ProxyEntry> = self.proxies.iter().map(|p| {
            ProxyEntry {
                name: None,
        ...
        match checker.check_all(&proxy_entries).await {
            Ok(results) => {
                self.health_results = results.results.iter().map(|r| ProxyHealthResult {
    ...
    pub async fn run_test(&mut self, proxy_url: &str, test_url: &str) {
        self.state = AppState::Running;
        self.results_view.clear();
        self.results_view.add_line(Line::from(format!("Testing proxy: {}", proxy_url)));
...
fn parse_proxy_url(proxy_url: &str) -> Result<(String, u16, Option<String>, Option<String>), String> {
