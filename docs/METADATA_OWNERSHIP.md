...
- **Phase B (crate-boundary consolidation, 2026-09-16)**: the normalized envelope model moved verbatim to `eggsec-report-model` (canonical data-contract owner); `eggsec-output::envelope` is now a compatibility re-export, and domain bridges depend on the model directly.
