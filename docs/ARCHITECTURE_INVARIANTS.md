...
14. **Operation metadata consistency**: All operation IDs, risk levels, capabilities, and surface exposure flags must be defined in the static `ALL_OPERATION_METADATA` registry. Individual surfaces must not hardcode divergent metadata.
...
28. **`with_execution_profile`**: Transitional. Only used in tests. New code must use `with_execution_surface()` or construct `EnforcementContext` directly.

29. **`ensure_scope` / `ensure_scope_url`**: Unused legacy helpers. Must not be called by new handlers. Scope checks are centralized in `EnforcementContext::evaluate()`.
