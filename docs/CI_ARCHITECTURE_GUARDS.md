...
### Network-Dependency Baseline + Transport-Contract Invariants (Phases A–C + Phase D increment 1)
...
- Retained baseline: `architecture/network_dependency_baseline.md` (per-artifact deps, leakage inventory, parity matrix, policy state + §7 increment-1 addendum). Scoped contract: `architecture/transport.md`. Adapter: `architecture/transport_eggfetch.md`. Executable invariants: `crates/eggsec/tests/network_policy_invariants.rs` (12 behaviors, local fixtures only) + `crates/eggsec/tests/transport_contract.rs` (11 closure tests through the fake).
