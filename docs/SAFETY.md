...
| (db pentest) | Direct Postgres/MySQL security assessment (lab-only, non-web). Phase 1: checks + manifest + bridge (postgres primary, mysql secondary). | Blocked by default; requires `db-pentest` feature + `--allow-db-pentest` for non-dry real runs; dry-run always safe; standalone defense-lab. |
