...
| `eggsec wireless <iface> deauth` | defense-lab (active, requires wireless-advanced) | Active WiFi deauth/disassoc frame injection for authorized lab testing. Policy: `OperationRisk::Intrusive` + `wireless-advanced` feature + `--allow-active-wireless` flag. Same standalone defense-lab rule: MCP/agent tool exposure intentionally absent. See docs/WIRELESS.md and architecture/wireless.md. |
| `eggsec mobile <path.{apk,ipa}>` (or `eggsec mobile static ...`) | defense-lab (static) | Standalone static analysis of Android APKs and iOS IPAs (manifest, permissions, transport config, secrets, debug/backup/exported components, signing/provisioning). Pure-Rust offline on user-supplied lab binaries only. Feature-gated `mobile`. Policy via SafeActive + required_features:["mobile"]; local MobileScanReport/MobileFinding + optional to_scan_report_data bridge. See docs/MOBILE.md and architecture/mobile.md. `eggsec mobile dynamic ...` requires `--features mobile-dynamic` (Android ADB + Frida + behavioral correlation; standalone defense-lab, MCP-absent; see docs/MOBILE.md). |
| `eggsec db pentest ...` | defense-lab (Phase 1+) | Direct (non-web) Postgres/MySQL/MSSQL/MongoDB/Redis security assessment for authorized lab/defense use. Phase 1–4 (complete 2026-06-12): Postgres + MySQL + MSSQL tiberius checks, lab manifest, budgets, dry-run, `--allow-db-pentest` gate, local `DbPentestReport`/`DbFinding`, optional `to_scan_report_data_db` bridge (auto-bridged via `report convert`). Phase 3 added: TUI tab `Tab::DbPentest` + pipeline `ScanProfile::DbRegression` + advanced gated checks behind `--allow-db-pentest-advanced` + correlation/evidence stubs. Phase 4: real advanced checks + correlation engine + native pipeline stage + evidence bundles. Phase 5 (complete 2026-06-12): MongoDB (`db-pentest-mongodb`) + Redis (`db-pentest-redis`) marker features, compliance mapping (OWASP/PCI/HIPAA/SOC2), MCP opt-in (`db-pentest-mcp`). Standalone defense-lab (see docs/DATABASE_PENTEST.md, architecture/defense_lab.md). |
| `eggsec proxy intercept ...` | defense-lab (Phase 5) | Interactive HTTP/HTTPS MITM proxy for authorized lab traffic inspection. Phase 1–5 complete: MITM server + CA + CLI + dry-run + policy + bridge (`WebProxySessionReport`/`ProxyFlow`/`BudgetUsage` + `to_scan_report_data_proxy`). Phase 2: Interactive TUI tab (`Tab::Intercept`) with live flow inspection, header/body editing, forward/drop/replay, intercept rules, session save/load, HAR export, manipulation audit trail. Phase 3: WebSocket/HTTP2/gRPC protocol support, enhanced rule engine (AND/OR/NOT conditions, persistence, new actions), cross-loadout correlation hooks, TUI protocol detail panes, extended bridge findings. Phase 4: Pipeline profile (`ScanProfile::WebProxy`, `Stage::WebProxy`), MCP proxy surface (12 tools via `web-proxy-mcp`), evidence bundle v2, performance optimizations (`FlowBuffer`, `ProxyMetrics`). Phase 5: Attack narrative generation, bundle comparison, TUI timeline view, TUI search/filter, security audit, 33 red-team adversarial tests, protocol handler plugin system (`ProtocolHandler` trait + `PluginRegistry`), temporal + behavioral correlation engine (`CorrelationEngine`), gRPC streaming frame tracking (`GrpcStreamFrame`/`GrpcStreamingState`/`GrpcSecurityFinding`), criterion benchmarks, FlowBuffer O(1) eviction, ProxyMetrics methods. Requires `--allow-web-proxy` + policy for real interception. `web-proxy = []` marker feature; `web-proxy-mcp` optional MCP exposure marker. See `docs/WEB_PROXY.md`, `architecture/web_proxy.md`. |
