...
Dynamic phases (Frida etc.) designed in `plans/dynamic-mobile-testing-loadout-design-plan.md` (gated `mobile-dynamic`; standalone; MCP-absent).
...
| `mobile` | Mobile app static analysis (APK/IPA manifest & config checks for authorized lab/defense use only; static-only Phase 1) (dynamic design in plan). |
