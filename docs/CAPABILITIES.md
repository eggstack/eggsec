...
| `eggsec proxy intercept ...` | defense-lab (Phase 1) | Interactive HTTP/HTTPS MITM proxy for authorized lab traffic inspection. Phase 1 complete (2026-06-12): dry-run path with synthetic flows, `WebProxySessionReport` + `ProxyFlow` + `BudgetUsage` types, `to_scan_report_data_proxy()` bridge with `proxy-intercept-flow` + `web-traffic-summary` finding categories (auto-bridged via `report convert`), 16 CLI flags (listen, upstream, upstream-tls, ca-file, cert-file, key-file, redact, redact-re, upstream-headers, upstream-timeout, max-rows, max-bytes, dry-run, json, detect-ports, listen-port), `--allow-web-proxy` gate, `TrafficInterception` risk tier. Phase 2 (real MITM server, flow capture, CA install): deferred. Standalone defense-lab (no MCP/agent). See `docs/WEB_PROXY.md`, `architecture/web_proxy.md`. |
