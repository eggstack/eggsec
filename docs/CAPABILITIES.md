...
| `eggsec db pentest ...` | defense-lab (Phase 1) | Direct (non-web) Postgres/MySQL security assessment for authorized lab/defense use. Phase 1 (complete 2026-06-12): Postgres + MySQL checks, lab manifest, budgets, dry-run, `--allow-db-pentest` gate, local `DbPentestReport`/`DbFinding`, optional `to_scan_report_data_db` bridge (auto-bridged via `report convert`). No TUI/pipeline/MCP in Phase 1. Standalone defense-lab (see docs/DATABASE_PENTEST.md, architecture/defense_lab.md, `plans/database-pentesting-phase1-foundation-handoff-plan.md` + `plans/non-web-database-pentesting-loadout-design-plan.md`). |
...
| `db-pentest` | Standalone defense-lab direct Postgres/MySQL security assessment (Phase 1: postgres/mysql checks + manifest + bridge; requires `--allow-db-pentest` for non-dry runs; dry-run always safe; local types + optional bridge to unified reports via report convert; no TUI/pipeline/MCP in Phase 1). See `plans/database-pentesting-phase1-foundation-handoff-plan.md` (executed) + `plans/non-web-database-pentesting-loadout-design-plan.md`. |
